...
class SmartRedirectHandler(_urllib.request.HTTPRedirectHandler):
    ...
    def _ask_redirect_choice(self, redcode, redurl, method):
        with kb.locks.redirect:
            ...
            if (
                ...
                and kb.resendPostOnRedirect is None
            ):
                ...
                kb.resendPostOnRedirect = readInput(
                    msg, default=("Y" if not kb.originalPage else "N"), boolean=True
                )
    ...
    def _redirect_request(self, req, fp, code, msg, headers, newurl):
        return _urllib.request.Request(
            newurl.replace(" ", "%20"),
            data=req.data,
            headers=req.headers,
            origin_req_host=(
                req.get_origin_req_host()
                if hasattr(req, "get_origin_req_host")
                else req.origin_req_host
            ),
        )
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        ...
        content = None
        redurl = (
            self._get_header_redirect(headers) if not conf.ignoreRedirects else None
        )
        ...
        if headers:
            logHeaders = "\r\n".join(
                "%s: %s"
                % (
                    getUnicode(key.capitalize() if hasattr(key, "capitalize") else key),
                    getUnicode(value),
                )
        ...
        if redurl and kb.choices.redirect == REDIRECTION.YES:
            ...
            if headers and HTTP_HEADER.SET_COOKIE in headers:
                ...
                for part in getUnicode(req.headers.get(HTTP_HEADER.COOKIE, "")).split(
                    delimiter
                ) + (
    ...
    def _infinite_loop_check(self, req):
        if hasattr(req, "redirect_dict") and (
            req.redirect_dict.get(req.get_full_url(), 0) >= MAX_SINGLE_URL_REDIRECTIONS
            or len(req.redirect_dict) >= MAX_TOTAL_REDIRECTIONS
        ):
