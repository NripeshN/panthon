...
def comparison(page, headers, code=None, getRatioValue=False, pageLength=None):
    _ = _adjust(
        _comparison(page, headers, code, getRatioValue, pageLength), getRatioValue
    )
    return _
...
def _adjust(condition, getRatioValue):
    if not any((conf.string, conf.notString, conf.regexp, conf.code)):
        ...
        retVal = (
            condition if not getRatioValue else (MAX_RATIO if condition else MIN_RATIO)
        )
...
def _comparison(page, headers, code, getRatioValue, pageLength):
    ...
    if kb.testMode:
        threadData.lastComparisonHeaders = (
            listToStrValue(
                _ for _ in headers.headers if not _.startswith("%s:" % URI_HTTP_HEADER)
            )
            if headers
            else ""
        )
        ...
        rawResponse = "%s%s" % (
            (
                listToStrValue(
                    ...
                    if not _.startswith("%s:" % URI_HTTP_HEADER)
                )
                if headers
                else ""
            ),
        ...
        if conf.notString:
            if conf.notString in rawResponse:
                ...
                if kb.errorIsNone and (
                    wasLastResponseDBMSError() or wasLastResponseHTTPError()
                ):
                    return None
                else:
                    return True
    ...
    if page:
        ...
        if (
            ...
            if not (
                ...
                and getLastRequestHTTPError() in (conf.ignoreCode or [])
            ):
                return None
    ...
    if kb.nullConnection and pageLength:
        ...
        if isinstance(seqMatcher.a, six.binary_type) and isinstance(
            ...
            seqMatcher.a = getBytes(
                seqMatcher.a, kb.pageEncoding or DEFAULT_PAGE_ENCODING, "ignore"
            )
        ...
        if any(_ is None for _ in (page, seqMatcher.a)):
            ...
            or seqMatcher.a
            and page
            and any(len(_) > MAX_DIFFLIB_SEQUENCE_LENGTH for _ in (seqMatcher.a, page))
        ):
            ...
            if conf.titles:
                ...
                seq1 = (
                    getFilteredPageContent(seqMatcher.a, True)
                    if conf.textOnly
                    else seqMatcher.a
                )
            ...
            if key in kb.cache.comparison:
                ...
                ratio = round(
                    (
                        ...
                        if not kb.heavilyDynamic
                        else seqMatcher.ratio()
                    ),
                    3,
                )
    ...
    if kb.matchRatio is None:
        if ratio >= LOWER_RATIO_BOUND and ratio <= UPPER_RATIO_BOUND:
            kb.matchRatio = ratio
            logger.debug(
                "setting match ratio for current parameter to %.3f" % kb.matchRatio
            )
