...
def forgeHeaders(items=None, base=None):
    """Prepare HTTP Cookie, HTTP User-Agent and HTTP Referer headers to use
    when performing the HTTP requests."""
    ...
    for key, value in _.items():
        ...
        if key.upper() not in (
            ...
            try:
                ...
                UnicodeEncodeError
            ):  # don't do the hack on non-ASCII header names (they have to be properly encoded later on)
                pass
            else:
                success = True
        if not success:
        ...
        if HTTP_HEADER.COOKIE in headers:
            for cookie in conf.cj:
                if (
                    ...
                    and not (conf.hostname or "").endswith(cookie.domain)
                ):
                    continue
                ...
                if ("%s=" % getUnicode(cookie.name)) in getUnicode(
                    ...
                    if conf.loadCookies:
                        ...
                        message = (
                            "you provided a HTTP %s header value, while "
                            % HTTP_HEADER.COOKIE
                        )
                        message += "target URL provides its own cookies within "
                        message += (
                            "HTTP %s header which intersect with yours. "
                        ...
                        def _(value):
                            return re.sub(
                                ...
                                (
                                    "%s=%s"
                                    % (
                                        getUnicode(cookie.name),
                                        getUnicode(cookie.value),
                                    )
                                ).replace("\\", r"\\"),
                                value,
                            )
...
def checkCharEncoding(encoding, warn=True):
    """Checks encoding name, repairs common misspellings and adjusts to proper
    namings used in codecs module.
    ...
    elif encoding in ("null", "{charset}", "charset", "*") or not re.search(
        r"\w", encoding
    ):
        return None
...
def getHeuristicCharEncoding(page):
    ...
    key = hash(page)
    retVal = (
        kb.cache.encoding[key]
        if key in kb.cache.encoding
        else detect(page[:HEURISTIC_PAGE_SIZE_THRESHOLD])["encoding"]
    )
...
def decodePage(page, contentEncoding, contentType, percentDecode=True):
        ...
        try:
            if contentEncoding == "deflate":
                ...
                size = struct.unpack("<l", page[-4:])[
                    0
                ]  # Reference: http://pydoc.org/get.cgi/usr/local/lib/python2.5/gzip.py
            ...
            if (
                b"<html" not in page
            ):  # in some cases, invalid "Content-Encoding" appears for plain HTML (should be ignored)
    ...
    if not conf.encoding:
        ...
        if (
            any((httpCharset, metaCharset))
            and (
                not all((httpCharset, metaCharset))
                or isinstance(page, six.binary_type)
                and all(_ in PRINTABLE_BYTES for _ in page)
            )
        ) or (httpCharset == metaCharset and all((httpCharset, metaCharset))):
            kb.pageEncoding = (
                httpCharset or metaCharset
            )  # Reference: http://bytes.com/topic/html-css/answers/154758-http-equiv-vs-true-header-has-precedence
        ...
        if not kb.disableHtmlDecoding:
            ...
            if b"&#" in page:
                page = re.sub(
                    b"&#x([0-9a-f]{1,2});",
                    lambda _: decodeHex(
                        _.group(1) if len(_.group(1)) == 2 else b"0%s" % _.group(1)
                    ),
                    page,
                )
                page = re.sub(
                    b"&#(\\d{1,3});",
                    lambda _: (
                        six.int2byte(int(_.group(1)))
                        if int(_.group(1)) < 256
                        else _.group(0)
                    ),
                    page,
                )
            ...
            if percentDecode:
                if b"%" in page:
                    page = re.sub(
                        b"%([0-9a-f]{2})", lambda _: decodeHex(_.group(1)), page
                    )
                    page = re.sub(
                        b"%([0-9A-F]{2})", lambda _: decodeHex(_.group(1)), page
                    )  # Note: %DeepSee_SQL in CACHE

            # e.g. &amp;
            page = re.sub(
                b"&([^;]+);",
                lambda _: (
                    six.int2byte(HTML_ENTITIES[getText(_.group(1))])
                    if HTML_ENTITIES.get(getText(_.group(1)), 256) < 256
                    else _.group(0)
                ),
                page,
            )
            ...
            page = re.sub(
                r"&([^;]+);",
                lambda _: (
                    _unichr(HTML_ENTITIES[_.group(1)])
                    if HTML_ENTITIES.get(_.group(1), 0) > 255
                    else _.group(0)
                ),
                page,
            )
...
def processResponse(page, responseHeaders, code=None, status=None):
    ...
    parseResponse(
        page,
        responseHeaders if kb.processResponseCounter < PARSE_HEADERS_LIMIT else None,
        status,
    )
    ...
    if not conf.skipWaf and kb.processResponseCounter < IDENTYWAF_PARSE_LIMIT:
        rawResponse = "%s %s %s\n%s\n%s" % (
            _http_client.HTTPConnection._http_vsn_str,
            code or "",
            status or "",
            ...
            if identYwaf.non_blind_check(rawResponse, silent=True):
                for waf in set(identYwaf.non_blind):
                    if waf not in kb.identifiedWafs:
                        ...
                        errMsg = "WAF/IPS identified as '%s'" % identYwaf.format_name(
                            waf
                        )
            ...
            if match and PLACE.POST in conf.parameters:
                ...
                if PLACE.POST in conf.paramDict and name in conf.paramDict[PLACE.POST]:
                    if conf.paramDict[PLACE.POST][name] in page:
                        ...
                        msg = (
                            "do you want to automatically adjust the value of '%s'?"
                            " [y/N]" % name
                        )
