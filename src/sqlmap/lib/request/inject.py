...
def _goDns(payload, expression):
    ...
    if (
        conf.dnsDomain
        and kb.dnsTest is not False
        and not kb.testMode
        and Backend.getDbms() is not None
    ):
...
def _goInference(
    ...
    if not (timeBasedCompare and kb.dnsTest):
        if (
            ...
            and not (timeBasedCompare and not kb.forceThreads)
        ):
            if field and re.search(
                r"\ASELECT\s+DISTINCT\((.+?)\)\s+FROM", expression, re.I
            ):
            ...
            if (
                field
                and conf.hexConvert
                or conf.binaryFields
                and field in conf.binaryFields
                or Backend.getIdentifiedDbms() in (DBMS.RAIMA,)
            ):
...
def _goInferenceFields(
    ...
    for field in expressionFieldsList:
        ...
        output = _goInference(
            payload, expressionReplaced, charsetType, firstChar, lastChar, dump, field
        )
...
def _goInferenceProxy(
    ...
    advantage of an blind SQL injection vulnerability on the affected parameter
    ...
    if (
        ...
        and hasattr(queries[Backend.getIdentifiedDbms()].limitregexp, "query")
    ):
            ...
            if test:
                ...
                try:
                    try:
                        ...
                        errMsg = (
                            "boundary limits (%d,%d) are too large. Please rerun "
        ...
        Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE
        and expression.upper().startswith("SELECT ")
        and " FROM " not in expression.upper()
    ):
...
def _goBooleanProxy(expression):
    ...
    if output is None:
        output = Request.queryPage(
            payload, timeBasedCompare=timeBasedCompare, raise404=False
        )
...
def getValue(
    ...
    """Called each time sqlmap inject a SQL query on the SQL injection affected
    parameter."""
    ...
    try:
        ...
        if conf.direct:
            value = direct(
                forgeCaseExpression if expected == EXPECTED.BOOL else expression
            )

        elif any(
            isTechniqueAvailable(_)
            for _ in getPublicTypeMembers(PAYLOAD.TECHNIQUE, onlyValues=True)
        ):
            ...
            if not conf.forceDns:
                if union and isTechniqueAvailable(PAYLOAD.TECHNIQUE.UNION):
                    ...
                    if expected == EXPECTED.BOOL:
                        ...
                        if not any(
                            _ in forgeCaseExpression for _ in ("SELECT", "CASE")
                        ):
                            forgeCaseExpression = (
                    ...
                    try:
                        value = _goUnion(
                            forgeCaseExpression if expected == EXPECTED.BOOL else query,
                            unpack,
                            dump,
                        )
                    ...
                    found = (
                        (value is not None)
                        or (value is None and expectingNone)
                        or count >= MAX_TECHNIQUES_PER_VALUE
                    )
                ...
                if (
                    ...
                    and any(
                        isTechniqueAvailable(_)
                        for _ in (PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)
                    )
                    and not found
                ):
                    setTechnique(
            ...
            if (
                time
                and (
                    isTechniqueAvailable(PAYLOAD.TECHNIQUE.TIME)
                    or isTechniqueAvailable(PAYLOAD.TECHNIQUE.STACKED)
                )
                and not found
            ):
                match = re.search(r"\bFROM\b ([^ ]+).+ORDER BY ([^ ]+)", expression)
                kb.responseTimeMode = (
                    "%s|%s" % (match.group(1), match.group(2)) if match else None
                )
    ...
    if (
        not any((kb.testMode, conf.dummy, conf.offline, conf.noCast, conf.hexConvert))
        and value is None
        and Backend.getDbms()
        and conf.dbmsHandler
        and kb.fingerprinted
    ):
        if conf.abortOnEmpty:
            ...
            warnMsg = (
                "in case of continuous data retrieval problems you are advised to try "
            )
            warnMsg += "a switch '--no-cast' "
            warnMsg += (
                "or switch '--hex'"
                if hasattr(queries[Backend.getIdentifiedDbms()], "hex")
                else ""
            )
            singleTimeWarnMessage(warnMsg)
    ...
    if isinstance(value, six.text_type) and "\\x" in value:
        try:
            candidate = eval(
                repr(value).replace("\\\\x", "\\x").replace("u'", "'", 1)
            ).decode(conf.encoding or UNICODE_ENCODING)
