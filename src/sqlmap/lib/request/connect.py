...
class Connect(object):
    ...
    def _getPageProxy(**kwargs):
        try:
            if (
                len(inspect.stack()) > sys.getrecursionlimit() // 2
            ):  # Note: https://github.com/sqlmapproject/sqlmap/issues/4525
    ...
    def _retryProxy(**kwargs):
        ...
        if (
            conf.proxyList
            and threadData.retriesCount >= conf.retries
            and not kb.locks.handlers.locked()
        ):
        ...
        if not kb.dnsMode and conn:
            ...
            if (
                ...
                if len(retVal) == MAX_CONNECTION_TOTAL_SIZE:
                    warnMsg = (
                        "large compressed response detected. Disabling compression"
                    )
                    singleTimeWarnMessage(warnMsg)
                ...
                while True:
                    ...
                    if len(part) == MAX_CONNECTION_READ_SIZE:
                        ...
                        singleTimeWarnMessage(warnMsg)
                        part = re.sub(
                            getBytes(r"(?si)%s.+?%s" % (kb.chars.stop, kb.chars.start)),
                            getBytes(
                                "%s%s%s"
                                % (
                                    kb.chars.stop,
                                    LARGE_READ_TRIM_MARKER,
                                    kb.chars.start,
                                )
                            ),
                            part,
                    ...
                    if len(retVal) > MAX_CONNECTION_TOTAL_SIZE:
                        warnMsg = (
                            "too large response detected. Automatically trimming it"
                        )
                        singleTimeWarnMessage(warnMsg)
                        break
    ...
    def getPage(**kwargs):
        ...
        kwargs.get("host", None) or conf.host
        ...
        ignoreTimeout = (
            kwargs.get("ignoreTimeout", False)
            or kb.ignoreTimeout
            or conf.ignoreTimeouts
        )
        ...
        if conf.dummy or conf.murphyRate and randomInt() % conf.murphyRate == 0:
            ...
            page, headers, code = (
                randomStr(int(randomInt()), alphabet=[_unichr(_) for _ in xrange(256)]),
                None,
                None if not conf.murphyRate else randomInt(3),
            )
        ...
        if conf.liveCookies:
            with kb.locks.liveCookies:
                if (
                    not checkFile(conf.liveCookies, raiseOnError=False)
                    or os.path.getsize(conf.liveCookies) == 0
                ):
                    warnMsg = (
                        "[%s] [WARNING] live cookies file '%s' is empty or"
                        " non-existent. Waiting for timeout (%d seconds)"
                    ...
                    for _ in xrange(LIVE_COOKIES_TIMEOUT):
                        if (
                            checkFile(conf.liveCookies, raiseOnError=False)
                            and os.path.getsize(conf.liveCookies) > 0
                        ):
                            valid = True
                            break
                        else:
        ...
        requestMsg += getUnicode(
            "%s%s" % (_.path or "/", ("?%s" % _.query) if _.query else "")
            if not any((refreshing, crawling, checking))
            else url
        )
        ...
        try:
            ...
            if not conf.requestFile or not target:
                ...
                if not getHeader(headers, HTTP_HEADER.ACCEPT_ENCODING):
                    headers[HTTP_HEADER.ACCEPT_ENCODING] = (
                ...
                conf.requestFile
                and getHeader(headers, HTTP_HEADER.USER_AGENT) == DEFAULT_USER_AGENT
            ):
            ...
            if (
                post is not None
                and not multipart
                and not getHeader(headers, HTTP_HEADER.CONTENT_TYPE)
            ):
            ...
            if (
                ...
                == POST_HINT_CONTENT_TYPES[POST_HINT.MULTIPART]
            ):
                warnMsg = (
                    "missing 'boundary parameter' in '%s' header. "
            ...
            for key, value in list(headers.items()):
                ...
                if isinstance(value, six.string_types):
                    ...
                    headers[getBytes(key) if six.PY2 else key] = getBytes(
                        value.strip("\r\n")
            ...
            if six.PY2:
                url = getBytes(
                    url
                )  # Note: Python3 requires text while Python2 has problems when mixing text with binary POST
            ...
            if webSocket:
                ...
                ws.connect(
                    url,
                    header=(
                        "%s: %s" % _ for _ in headers.items() if _[0] not in ("Host",)
                    ),
                    cookie=cookie,
                ...
                if (
                    target
                    and cmdLineOptions.method
                    or method
                    and method not in (HTTPMETHOD.GET, HTTPMETHOD.POST)
                ):
                ...
                if post is not None:
                    if (
                        not getRequestHeader(req, HTTP_HEADER.CONTENT_LENGTH)
                        and not chunked
                    ):
                        requestHeaders += "\r\n%s: %d" % (
                            string.capwords(HTTP_HEADER.CONTENT_LENGTH),
                            len(post),
                        )
                ...
                if not getRequestHeader(req, HTTP_HEADER.CONNECTION):
                    requestHeaders += "\r\n%s: %s" % (
                        ...
                        "close" if not conf.keepAlive else "keep-alive",
                    )
                ...
                if (
                    not kb.authHeader
                    and getRequestHeader(req, HTTP_HEADER.AUTHORIZATION)
                    and (conf.authType or "").lower() == AUTH_TYPE.BASIC.lower()
                ):
                    kb.authHeader = getUnicode(
                        getRequestHeader(req, HTTP_HEADER.AUTHORIZATION)
                    )
                ...
                if hasattr(conn, "redurl"):
                    page = (
                        (
                            ...
                            if kb.choices.redirect == REDIRECTION.NO
                ...
                if conn:
                    code = (
                        (code or conn.code)
                        if conn.code == kb.originalCode
                        else conn.code
                    )  # do not override redirection code (for comparison purposes)
                    responseHeaders = conn.info()
                    responseHeaders[URI_HTTP_HEADER] = (
                        conn.geturl() if hasattr(conn, "geturl") else url
                    )
                    ...
                    kb.serverHeader = responseHeaders.get(
                        HTTP_HEADER.SERVER, kb.serverHeader
                    )
                else:
                    code = None
                    responseHeaders = {}
                ...
                page = decodePage(
                    page,
                    responseHeaders.get(HTTP_HEADER.CONTENT_ENCODING),
                    responseHeaders.get(HTTP_HEADER.CONTENT_TYPE),
                    percentDecode=not crawling,
                )
                status = (
                    getUnicode(conn.msg)
                    if conn and getattr(conn, "msg", None)
                    else None
                )
            ...
            if not refreshing:
                refresh = (
                    responseHeaders.get(HTTP_HEADER.REFRESH, "")
                    .split("url=")[-1]
                    .strip()
                )
                ...
                if refresh:
                    if kb.alwaysRefresh is None:
                        ...
                        msg += (
                            "(redirect like response common to login pages) to '%s'. "
                            % refresh
                        )
            ...
            if conn and not conf.keepAlive:
                try:
                    ...
                    warnMsg = (
                        "problem occurred during connection closing ('%s')"
                        % getSafeExString(ex)
                    )
                    logger.warning(warnMsg)
            ...
            if conf.proxyList and not kb.threadException:
                warnMsg = (
                    "unable to connect to the target URL ('%s')" % getSafeExString(ex)
                )
            ...
            code = ex.code
            status = getUnicode(
                getattr(ex, "reason", None) or getSafeExString(ex).split(": ", 1)[-1]
            )
            ...
            logHTTPTraffic(
                requestMsg,
                "%s%s\r\n\r\n%s"
                % (responseMsg, logHeaders, (page or "")[:MAX_CONNECTION_READ_SIZE]),
                start,
                time.time(),
            )
            ...
            if conf.verbose <= 5:
                ...
                responseMsg += "%s\r\n\r\n%s" % (
                    logHeaders,
                    (page or "")[:MAX_CONNECTION_READ_SIZE],
                )
            ...
            if ex.code not in (conf.ignoreCode or []):
                if ex.code == _http_client.UNAUTHORIZED:
                ...
                elif chunked and ex.code in (
                    _http_client.METHOD_NOT_ALLOWED,
                    _http_client.LENGTH_REQUIRED,
                ):
                    warnMsg = "turning off HTTP chunked transfer encoding "
                    warnMsg += (
                        "as it seems that the target site doesn't support it (%d)"
                        % code
                    )
                    singleTimeWarnMessage(warnMsg)
                    ...
                    warnMsg += (
                        "you are advised to try a switch '--no-cast' and/or"
                        " '--no-escape'"
                    )
                    singleTimeWarnMessage(warnMsg)
                    ...
                    if ignoreTimeout:
                        ...
                        warnMsg = "unable to connect to the target URL (%d - %s)" % (
                            ex.code,
                            _http_client.responses[ex.code],
                        )
                        if (
                            threadData.retriesCount < conf.retries
                            and not kb.threadException
                        ):
            ...
            _urllib.error.URLError,
            socket.error,
            socket.timeout,
            _http_client.HTTPException,
            struct.error,
            binascii.Error,
            ProxyError,
            ...
            if checking:
                ...
                if kb.testMode and kb.testType not in (
                    ...
                    singleTimeWarnMessage(
                        "there is a possibility that the target (or WAF/IPS) is"
                ...
                if not conf.disablePrecon:
                    singleTimeWarnMessage(
                        "turning off pre-connect mechanism because of connection"
                        " reset(s)"
                    )
                ...
                if not conf.agent and not conf.randomAgent:
                    ...
                    warnMsg += (
                        "header with option '--user-agent' or switch '--random-agent'"
                    )
                ...
                errMsg = (
                    "websocket handshake status %s" % status.group(1)
                    if status
                    else "unknown"
                )
                raise SqlmapConnectionException(errMsg)
            ...
            with kb.locks.connError:
                ...
                if (
                    kb.connErrorCounter >= MAX_CONSECUTIVE_CONNECTION_ERRORS
                    and kb.choices.connError is None
                ):
                    message = (
                        "there seems to be a continuous problem with connection to the"
                        " target. "
                    )
            ...
            if "forcibly closed" in tbMsg:
                ...
                _ in tbMsg
                for _ in ("timed out", "IncompleteRead", "Interrupted system call")
            ):
                return None if not conf.ignoreTimeouts else "", None, None
            ...
            if isinstance(page, six.binary_type):
                if HTTP_HEADER.CONTENT_TYPE in (
                    responseHeaders or {}
                ) and not re.search(
        ...
        if not sys.version.startswith("3.11."):
            if conf.retryOn and re.search(conf.retryOn, page, re.I):
                if threadData.retriesCount < conf.retries:
                    warnMsg = (
                        "forced retry of the request because of undesired page content"
                    )
                    logger.warning(warnMsg)
                    return Connect._retryProxy(**kwargs)
        ...
        if not skipLogTraffic:
            if conn and getattr(conn, "redurl", None):
                ...
                requestMsg = re.sub(
                    r"(\n[A-Z]+ ).+?( HTTP/\d)",
                    r"\g<1>%s\g<2>" % getUnicode(_).replace("\\", "\\\\"),
                    requestMsg,
                    1,
                )
    ...
    def queryPage(
        ...
        """This method calls a function to get the target URL page content and
        returns its page ratio (0 <= ratio <= 1) or a boolean value
        ...
        if conf.httpHeaders:
            ...
            contentType = (
                max(
                    (
                        headers[_] or ""
                        if _.upper() == HTTP_HEADER.CONTENT_TYPE.upper()
                        else ""
                    )
                    for _ in headers
                )
            ...
            if (kb.postHint or conf.skipUrlEncode) and postUrlEncode:
                postUrlEncode = False
                if not (
                    conf.skipUrlEncode and contentType
                ):  # NOTE: https://github.com/sqlmapproject/sqlmap/issues/5092
                    conf.httpHeaders = [
        ...
        if payload:
            delimiter = conf.paramDel or (
                DEFAULT_GET_POST_DELIMITER
                if place != PLACE.COOKIE
                else DEFAULT_COOKIE_DELIMITER
            )
            ...
            if not disableTampering and kb.tamperFunctions:
                ...
                if hints:
                    ...
                    if HINT.PREPEND in hints:
                        if place == PLACE.URI:
                            match = re.search(
                                r"\w+\s*=\s*%s" % PAYLOAD_DELIMITER, value
                            ) or re.search(
                                r"[^?%s/]=\s*%s"
                                % (re.escape(delimiter), PAYLOAD_DELIMITER),
                                value,
                            )
                            if match:
                                value = value.replace(
                                    match.group(0),
                                    "%s%s%s"
                                    % (hints[HINT.PREPEND], delimiter, match.group(0)),
                                )
                        else:
            ...
            logger.log(
                ...
                safecharencode(
                    payload.replace("\\", BOUNDARY_BACKSLASH_MARKER)
                ).replace(BOUNDARY_BACKSLASH_MARKER, "\\"),
            )
            ...
            if place == PLACE.CUSTOM_POST and kb.postHint:
                if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                    ...
                    payload = (
                        payload.replace("'", REPLACEMENT_MARKER)
                        .replace('"', "'")
                ...
                if (
                    ...
                    if not skip:
                        if place in (
                            PLACE.POST,
                            PLACE.CUSTOM_POST,
                        ):  # potential problems in other cases (e.g. URL encoding of whole URI - including path)
                            value = urlencode(value, spaceplus=kb.postSpaceToPlus)
                        payload = urlencode(
                            payload, safe="%", spaceplus=kb.postSpaceToPlus
                        )
                        value = agent.replacePayload(value, payload)
                        postUrlEncode = False
            ...
            if conf.hpp:
                if not any(
                    conf.url.lower().endswith(_.lower())
                    for _ in (WEB_PLATFORM.ASP, WEB_PLATFORM.ASPX)
                ):
                ...
                if place in (PLACE.GET, PLACE.POST):
                    ...
                    match = re.search(
                        r"(?P<name>\w+)=%s(?P<value>.+?)%s" % (_, _), value
                    )
                    if match:
        ...
        if PLACE.GET in conf.parameters:
            get = (
                conf.parameters[PLACE.GET] if place != PLACE.GET or not value else value
            )
        elif (
            place == PLACE.GET
        ...
        if PLACE.POST in conf.parameters:
            post = (
                ...
                if place != PLACE.POST or not value
                else value
            )
        ...
        if PLACE.CUSTOM_POST in conf.parameters:
            post = (
                conf.parameters[PLACE.CUSTOM_POST].replace(kb.customInjectionMark, "")
        ...
        if PLACE.COOKIE in conf.parameters:
            cookie = (
                ...
                if place != PLACE.COOKIE or not value
                else value
            )

        if PLACE.USER_AGENT in conf.parameters:
            ua = (
                conf.parameters[PLACE.USER_AGENT]
                if place != PLACE.USER_AGENT or not value
                else value
            )

        if PLACE.REFERER in conf.parameters:
            referer = (
                conf.parameters[PLACE.REFERER]
                if place != PLACE.REFERER or not value
                else value
            )

        if PLACE.HOST in conf.parameters:
            host = (
                conf.parameters[PLACE.HOST]
                if place != PLACE.HOST or not value
                else value
            )
        ...
        if conf.csrfToken:
            ...
            def _adjustParameter(paramString, parameter, newValue):
                ...
                if match:
                    retVal = re.sub(
                        r"(?i)%s" % re.escape(match.group(0)),
                        ("%s=%s" % (parameter, newValue)).replace("\\", r"\\"),
                        paramString,
                    )
                else:
                    match = re.search(
            ...
            for attempt in xrange(conf.csrfRetries + 1):
                ...
                if attempt > 0:
                    warnMsg = "unable to find anti-CSRF token '%s' at '%s'" % (
                        conf.csrfToken._original,
                        conf.csrfUrl or conf.url,
                    )
                    warnMsg += ". sqlmap is going to retry the request"
                    logger.warning(warnMsg)

                page, headers, code = Connect.getPage(
                    url=conf.csrfUrl or conf.url,
                    data=conf.csrfData
                    or (conf.data if conf.csrfUrl == conf.url else None),
                    method=conf.csrfMethod
                    or (conf.method if conf.csrfUrl == conf.url else None),
                    cookie=conf.parameters.get(PLACE.COOKIE),
                    direct=True,
                    silent=True,
                ...
                page = urldecode(
                    page
                )  # for anti-CSRF tokens with special characters in their name (e.g. 'foo:bar=...')

                match = re.search(
                    r"(?i)<input[^>]+\bname=[\"']?(?P<name>%s)\b[^>]*\bvalue=[\"']?(?P<value>[^>'\"]*)"
                    % conf.csrfToken,
                    page or "",
                    re.I,
                )
                ...
                if not match:
                    match = re.search(
                        r"(?i)<input[^>]+\bvalue=[\"']?(?P<value>[^>'\"]*)[\"']?[^>]*\bname=[\"']?(?P<name>%s)\b"
                        % conf.csrfToken,
                        page or "",
                        re.I,
                    )
                    ...
                    if not match:
                        ...
                        if not match:
                            ...
                            if not match:
                                ...
                                if not match:
                                    match = re.search(
                                        r"<meta\s+name=[\"']?(?P<name>%s)[\"']?[^>]+\b(value|content)=[\"']?(?P<value>[^>\"']+)"
                                        % conf.csrfToken,
                                        page or "",
                                        re.I,
                                    )
                ...
                if match:
                    ...
                    match = re.search(
                        r"String\.fromCharCode\(([\d+, ]+)\)", token.value
                    )
                    if match:
                        token.value = "".join(
                            _unichr(int(_))
                            for _ in match.group(1).replace(" ", "").split(",")
                        )
                ...
                if not token:
                    if (
                        conf.csrfUrl
                        and conf.csrfToken
                        and conf.csrfUrl != conf.url
                        and code == _http_client.OK
                    ):
                        if headers and PLAIN_TEXT_CONTENT_TYPE in headers.get(
                            HTTP_HEADER.CONTENT_TYPE, ""
                        ):
                    ...
                    if (
                        ...
                        for _ in conf.cj:
                            if re.search(conf.csrfToken, _.name, re.I):
                                ...
                                if not any(
                                    re.search(conf.csrfToken, " ".join(_), re.I)
                                    for _ in (
                                        conf.paramDict.get(PLACE.GET, {}),
                                        conf.paramDict.get(PLACE.POST, {}),
                                    )
                                ):
                                    if post:
                                        post = "%s%s%s=%s" % (
        ...
        if conf.rParam:
            ...
            def _randomizeParameter(paramString, randomParameter):
                retVal = paramString
                match = re.search(
                    r"(\A|\b)%s=(?P<value>[^&;]*)" % re.escape(randomParameter),
                    paramString,
                )
                if match:
                    origValue = match.group("value")
                    newValue = (
                        randomizeParameterValue(origValue)
                        if randomParameter not in kb.randomPool
                        else random.sample(kb.randomPool[randomParameter], 1)[0]
                    )
                    retVal = re.sub(
                        r"(\A|\b)%s=[^&;]*" % re.escape(randomParameter),
                        "%s=%s" % (randomParameter, newValue),
                        paramString,
                    )
                else:
                    match = re.search(
                        r"(\A|\b)(%s\b[^\w]+)(?P<value>\w+)"
                        % re.escape(randomParameter),
                        paramString,
                    )
                    if match:
                        origValue = match.group("value")
                        ...
                        retVal = paramString.replace(
                            match.group(0), "%s%s" % (match.group(2), newValue)
                        )
                return retVal
        ...
        if conf.evalCode:
                ...
                for part in item.split(delimiter):
                    if "=" in part:
                        ...
                        value = urldecode(
                            value,
                            convall=True,
                            spaceplus=(item == post and kb.postSpaceToPlus),
                        )
                ...
                try:
                    ...
                    if ex.text:
                        ...
                        if original == replacement:
                            ...
                            conf.evalCode = conf.evalCode.replace(
                                getUnicode(ex.text.strip(), UNICODE_ENCODING),
                                replacement,
                            )
                    else:
                        break
            ...
            for name, value in variables.items():
                if name != "__builtins__" and originals.get(name, "") != value:
                    if isinstance(
                        value, (int, float, six.string_types, six.binary_type)
                    ):
                        found = False
                        value = getUnicode(value, UNICODE_ENCODING)

                        if kb.postHint == POST_HINT.MULTIPART:
                            boundary = "--%s" % re.search(
                                r"boundary=([^\s]+)", contentType
                            ).group(1)
                            if boundary:
                                parts = post.split(boundary)
                                match = re.search(
                                    r'\bname="%s"' % re.escape(name), post
                                )
                                ...
                                for i in xrange(len(parts)):
                                    ...
                                    if re.search(
                                        ...
                                        if match:
                                            ...
                                            second = re.sub(
                                                r"(?s).+?(\r?\n?\-*\Z)",
                                                r"%s\g<1>" % re.escape(value),
                                                second,
                                            )
                                            parts[i] = "%s%s" % (first, second)
                                post = boundary.join(parts)

                        elif kb.postHint and re.search(
                            r"\b%s\b" % re.escape(name), post or ""
                        ):
                            if kb.postHint in (POST_HINT.XML, POST_HINT.SOAP):
                                ...
                                match = re.search(
                                    r"['\"]%s['\"]:" % re.escape(name), post
                                )
                                if match:
                                    quote = match.group(0)[0]
                                    post = post.replace(
                                        "\\%s" % quote, BOUNDARY_BACKSLASH_MARKER
                                    )
                                    match = re.search(
                                        r"(%s%s%s:\s*)(\d+|%s[^%s]*%s)"
                                        % (
                                    ...
                                    post = post.replace(
                                        BOUNDARY_BACKSLASH_MARKER, "\\%s" % quote
                                    )
                        ...
                        if re.search(regex, (get or "")):
                            found = True
                            get = re.sub(
                                regex, r"\g<1>%s\g<3>" % value.replace("\\", r"\\"), get
                            )
        ...
        if timeBasedCompare and not conf.disableStats:
            if len(kb.responseTimes.get(kb.responseTimeMode, [])) < MIN_TIME_RESPONSES:
                ...
                if conf.tor:
                    warnMsg = (
                        "it's highly recommended to avoid usage of switch '--tor' for "
                    )
                    warnMsg += (
                        "time-based injections because of inherent high latency time"
                    )
                    singleTimeWarnMessage(warnMsg)

                warnMsg = "[%s] [WARNING] %stime-based comparison requires " % (
                    time.strftime("%X"),
                    "(case) " if kb.responseTimeMode else "",
                )
                ...
                while len(kb.responseTimes[kb.responseTimeMode]) < MIN_TIME_RESPONSES:
                    value = (
                        kb.responseTimePayload.replace(
                            RANDOM_INTEGER_MARKER, str(randomInt(6))
                        ).replace(RANDOM_STRING_MARKER, randomStr())
                        if kb.responseTimePayload
                        else kb.responseTimePayload
                    )
                ...
                if headers:
                    try:
                        if kb.nullConnection in (
                            NULLCONNECTION.HEAD,
                            NULLCONNECTION.SKIP_READ,
                        ) and headers.get(HTTP_HEADER.CONTENT_LENGTH):
                            pageLength = int(
                                headers[HTTP_HEADER.CONTENT_LENGTH].split(",")[0]
                            )
                        elif kb.nullConnection == NULLCONNECTION.RANGE and headers.get(
        ...
        if not ignoreSecondOrder:
            if conf.secondUrl:
            ...
            elif kb.secondReq and IPS_WAF_CHECK_PAYLOAD not in _urllib.parse.unquote(
                value or ""
            ):
                ...
                def _(value):
                    if kb.customInjectionMark in (value or ""):
                        if payload is None:
                            ...
                            try:
                                value = re.sub(
                                    r"\w*%s" % re.escape(kb.customInjectionMark),
                                    payload,
                                    value,
                                )
                            except re.error:
                ...
                page, headers, code = Connect.getPage(
                    url=_(kb.secondReq[0]),
                    post=_(kb.secondReq[2]),
                    method=kb.secondReq[1],
                    cookie=kb.secondReq[3],
                    silent=silent,
        ...
        if timeBasedCompare:
            ...
            if len(kb.responseTimes[kb.responseTimeMode]) > MAX_TIME_RESPONSES:
                kb.responseTimes[kb.responseTimeMode] = kb.responseTimes[
                    kb.responseTimeMode
                ][-MAX_TIME_RESPONSES // 2 :]

        if not response and removeReflection:
            page = removeReflectiveValues(page, payload)

        kb.maxConnectionsFlag = (
            re.search(MAX_CONNECTIONS_REGEX, page or "", re.I) is not None
        )
        ...
        if message:
            ...
            singleTimeWarnMessage(
                "potential permission problems detected ('%s')" % message
            )
        ...
        if getRatioValue:
            return comparison(
                page, headers, code, getRatioValue=False, pageLength=pageLength
            ), comparison(
