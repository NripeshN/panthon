...
def direct(query, content=True):
    ...
    if (
        ...
        and query.upper().startswith("SELECT ")
        and " FROM " not in query.upper()
    ):
    ...
    for sqlTitle, sqlStatements in SQL_STATEMENTS.items():
        for sqlStatement in sqlStatements:
            if (
                query.lower().startswith(sqlStatement)
                and sqlTitle != "SQL SELECT statement"
            ):
                select = False
                break
        ...
        if conf.binaryFields:
            for field in conf.binaryFields:
                ...
                if re.search(r"\b%s\b" % re.escape(field), query):
                    query = re.sub(
                        r"\b%s\b" % re.escape(field),
                        agent.hexConvertField(field),
                        query,
                    )
    ...
    if not select and re.search(r"(?i)\bEXEC ", query) is None:
        timeout(
            func=conf.dbmsConnector.execute,
            args=(query,),
        ...
        output
        and ("%soutput" % conf.tablePrefix) not in query
        and ("%sfile" % conf.tablePrefix) not in query
    ):
        output, state = timeout(
            func=conf.dbmsConnector.select,
            args=(query,),
            duration=conf.timeout,
            default=None,
        )
