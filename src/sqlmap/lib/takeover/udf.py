...
class UDF(object):
    ...
    def udfCheckNeeded(self):
        if (
            not any((conf.fileRead, conf.commonFiles))
            or (
                any((conf.fileRead, conf.commonFiles))
                and not Backend.isDbms(DBMS.PGSQL)
            )
        ) and "sys_fileread" in self.sysUdfs:
    ...
    def udfInjectCore(self, udfDict):
        ...
        if len(self.udfToCreate) > 0:
            ...
            written = self.writeFile(
                self.udfLocalFile, self.udfRemoteFile, "binary", forceCheck=True
            )
    ...
    def udfInjectCustom(self):
        ...
        if not self.udfLocalFile.endswith(".dll") and not self.udfLocalFile.endswith(
            ".so"
        ):
        ...
        while True:
            ...
            while True:
                ...
                if choice == "Q":
                    ...
                    isDigit(choice) and int(choice) > 0 and int(choice) <= len(udfList)
                ):
                    choice = int(choice)
                    break
                else:
