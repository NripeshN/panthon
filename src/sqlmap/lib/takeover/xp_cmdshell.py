...
class XP_cmdshell(object):
    """This class defines methods to deal with Microsoft SQL Server xp_cmdshell
    extended procedure for plugins."""
    ...
    def _xpCmdshellConfigure2000(self, mode):
        ...
        if mode == 1:
            ...
            cmd = getSQLSnippet(
                DBMS.MSSQL, "disable_xp_cmdshell_2000", ENABLE=str(mode)
            )
    ...
    def _xpCmdshellTest(self):
        ...
        if output == "1":
            ...
            errMsg = (
                "it seems that the temporary directory ('%s') used for "
    ...
    def xpCmdshellForgeCmd(self, cmd, insertIntoTable=None):
        ...
        try:
            self._forgedCmd += "SET @%s=%s;" % (
                self._randStr,
                "0x%s" % encodeHex(cmd, binary=False),
            )
        except UnicodeError:
    ...
    def xpCmdshellEvalCmd(self, cmd, first=None, last=None):
        ...
        if conf.direct:
            ...
            if conf.dbmsCred:
                inject.goStacked(
                    "BULK INSERT %s FROM '%s' WITH (CODEPAGE='RAW',"
                    " FIELDTERMINATOR='%s', ROWTERMINATOR='%s')"
            ...
            if (output is None) or len(output) == 0 or output[0] is None:
                ...
                count = inject.getValue(
                    "SELECT COUNT(id) FROM %s" % self.cmdTblName,
                    resumeValue=False,
                    union=False,
                    error=False,
                ...
                if isNumPosStrValue(count):
                    for index in getLimitRange(count):
                        ...
                        output.append(
                            inject.getValue(
                                query, union=False, error=False, resumeValue=False
                            )
                        )
