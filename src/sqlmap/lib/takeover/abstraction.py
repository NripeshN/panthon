...
class Abstraction(Web, UDF, XP_cmdshell):
    ...
    def shell(self):
        ...
        autoCompletion(
            AUTOCOMPLETE_TYPE.OS, OS.WINDOWS if Backend.isOs(OS.WINDOWS) else OS.LINUX
        )
    ...
    def initEnv(self, mandatory=True, detailed=False, web=False, forceInit=False):
        ...
        if web:
            ...
            if (
                any((conf.osCmd, conf.osShell))
                and Backend.isDbms(DBMS.PGSQL)
                and self.checkCopyExec()
            ):
                success = True
            elif Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL):
