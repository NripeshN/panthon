...
class Web(object):
    ...
    def webInit(self):
        ...
        if (
            self.webBackdoorUrl is not None
            and self.webStagerUrl is not None
            and self.webPlatform is not None
        ):
            return
        ...
        while True:
            ...
            if not isDigit(choice):
                ...
                logger.warning(
                    "invalid value, it must be between 1 and %d" % len(choices)
                )
        ...
        if not kb.absFilePaths:
            ...
            if readInput(message, default="Y", boolean=True):
                ...
                for match in re.finditer(
                    ...
                    url = "%s%s" % (
                        conf.url.replace(conf.path, match.group(4)),
                        "wp-content/wp-db.php",
                    )
                    if url not in been:
                        try:
                            page, _, _ = Request.getPage(
                ...
                for place in (PLACE.GET, PLACE.POST):
                    if place in conf.parameters:
                        value = re.sub(
                            r"(\A|&)(\w+)=", r"\g<2>[]=", conf.parameters[place]
                        )
                        if "[]" in value:
        ...
        stagerContent = getText(
            decloak(
                os.path.join(
                    paths.SQLMAP_SHELL_PATH, "stagers", "stager.%s_" % self.webPlatform
                )
            )
        )
        ...
        for directory in directories:
            ...
            for match in re.finditer("/", directory):
                ...
                debugMsg = (
                    "trying to see if the file is accessible from '%s'"
                    % self.webStagerUrl
                )
            ...
            if self.webPlatform == WEB_PLATFORM.ASP:
                ...
                if self.webUpload(
                    ...
                    content=backdoorContent.replace(
                        SHELL_WRITABLE_DIR_TAG, backdoorDirectory
                    ).replace(SHELL_RUNCMD_EXE_TAG, _),
                ):
                    self.webUpload(
                        _,
                        backdoorDirectory,
