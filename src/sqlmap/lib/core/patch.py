...
def dirtyPatches():
    ...
    if six.PY3:
        if not hasattr(_http_client.HTTPConnection, "__send_output"):
            _http_client.HTTPConnection.__send_output = (
                _http_client.HTTPConnection._send_output
            )
    ...
    if hasattr(_http_client, "LineAndFileWrapper"):
        ...
        _http_client.LineAndFileWrapper._readline = (
            _http_client.LineAndFileWrapper.readline
        )
    ...
    try:
        ...
        if six.PY3:
            ...
            os.urandom = lambda size: "".join(
                chr(random.randint(0, 255)) for _ in xrange(size)
            )
