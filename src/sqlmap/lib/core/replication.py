...
class Replication(object):
    """This class holds all methods/classes used for database replication
    purposes."""
    ...
    class DataType(object):
        """Using this class we define auxiliary objects used for representing
        sqlite data types."""
    ...
    class Table(object):
                    ...
                    if not typeless:
                        self.execute(
                            ...
                            % (
                                ...
                                ",".join(
                                    '"%s" %s'
                                    % (unsafeSQLIdentificatorNaming(colname), coltype)
                        ...
                        self.execute(
                            ...
                            % (
                                self.name,
                                ",".join(
                                    '"%s"' % unsafeSQLIdentificatorNaming(colname)
        ...
        def insert(self, values):
            """This function is used for inserting row(s) into current
            table."""

            if len(values) == len(self.columns):
                self.execute(
                    'INSERT INTO "%s" VALUES (%s)'
                    % (self.name, ",".join(["?"] * len(values))),
            ...
            try:
                try:
                    ...
                    self.parent.cursor.execute(
                        sql, cleanReplaceUnicode(parameters or [])
                    )
            except sqlite3.OperationalError as ex:
                errMsg = (
                    "problem occurred ('%s') while accessing sqlite database "
                    % getSafeExString(ex, UNICODE_ENCODING)
                )
        ...
        def beginTransaction(self):
            """Great speed improvement can be gained by using explicit
            transactions around multiple inserts.
        ...
        def select(self, condition=None):
            """This function is used for selecting row(s) from current
            table."""
    ...
    def createTable(self, tblname, columns=None, typeless=False):
        """This function creates Table instance with current connection
        settings."""
        return Replication.Table(
            parent=self, name=tblname, columns=columns, typeless=typeless
        )
