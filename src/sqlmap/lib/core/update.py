...
def update():
    ...
    if TYPE == "pip":
        ...
        if success:
            logger.info(
                ...
                % (
                    ...
                    or extractRegexResult(r"\((?P<result>\d+\.\d+\.\d+)\)", output),
                )
            )
        else:
            logger.error(
        ...
        warnMsg = (
            "not a git repository. It is recommended to clone the"
            " 'sqlmapproject/sqlmap' repository "
        )
        ...
        if VERSION == getLatestRevision():
            logger.info(
                "already at the latest revision '%s'" % (getRevisionNumber() or VERSION)
            )
            return

        message = (
            "do you want to try to fetch the latest 'zipball' from repository and"
            " extract it (experimental) ? [y/N]"
        )
                    ...
                    try:
                        ...
                        if os.path.isfile(filepath):
                            with openFile(filepath, "rb") as f:
                                version = re.search(
                                    r"(?m)^VERSION\s*=\s*['\"]([^'\"]+)", f.read()
                                ).group(1)
                                logger.info(
                                    "updated to the latest version '%s#dev'" % version
                                )
                                success = True
                    except Exception as ex:
                        logger.error(
                            "update could not be completed ('%s')" % getSafeExString(ex)
                        )
                    else:
                        if not success:
        ...
        if success:
            ...
            if "Not a git repository" in output:
                errMsg = (
                    "not a valid git repository. Please checkout the"
                    " 'sqlmapproject/sqlmap' repository "
                )
                errMsg += (
                    "from GitHub (e.g. 'git clone --depth 1 %s sqlmap')"
                    % GIT_REPOSITORY
                )
                logger.error(errMsg)
            else:
