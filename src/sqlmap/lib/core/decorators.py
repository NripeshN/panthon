    ...
    def _f(*args, **kwargs):
        try:
            key = (
                int(
                    hashlib.md5(
                        "|".join(str(_) for _ in (f, args, kwargs)).encode(
                            UNICODE_ENCODING
                        )
                    ).hexdigest(),
                    16,
            ...
            ValueError
        ):  # https://github.com/sqlmapproject/sqlmap/issues/4281 (NOTE: non-standard Python behavior where hexdigest returns binary value)
...
def stackedmethod(f):
    """Method using pushValue/popValue functions (fallback function for stack
    realignment)
