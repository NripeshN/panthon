...
def _setRequestParams():
    ...
    if conf.data is not None:
        ...
        def process(match, repl):
            ...
            if not (
                ...
                not in (removePostHintPrefix(_) for _ in conf.testParameter)
            ) and match.group("name") == match.group("name").strip("\\"):
                retVal = repl
                while True:
        ...
        if kb.processUserMarks is None and kb.customInjectionMark in conf.data:
            message = "custom injection marker ('%s') found in %s " % (
        ...
        if re.search(JSON_RECOGNITION_REGEX, conf.data):
            ...
            if choice == "Q":
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    conf.data = re.sub(
                        r'("(?P<name>[^"]+)"\s*:\s*".*?)"(?<!\\")',
                        functools.partial(
                            process, repl=r'\g<1>%s"' % kb.customInjectionMark
                        ),
                        conf.data,
                    )
                    conf.data = re.sub(
                    ...
                    conf.data = re.sub(
                        r'("(?P<name>[^"]+)"\s*:\s*)(-?\d[\d\.]*)\b',
                        functools.partial(
                            process, repl=r"\g<1>\g<3>%s" % kb.customInjectionMark
                        ),
                        conf.data,
                    )
                    conf.data = re.sub(
                    ...
                    for match in re.finditer(
                        ...
                        if not (
                            ...
                            _ = match.group(2)
                            if (
                                kb.customInjectionMark not in _
                            ):  # Note: only for unprocessed (simple) forms - i.e. non-associative arrays (e.g. [1,2,3])
                                _ = re.sub(
                                    r'("[^"]+)"',
                                    r'\g<1>%s"' % kb.customInjectionMark,
                                    _,
                                )
                                _ = re.sub(
                                    r"(\A|,|\s+)(-?\d[\d\.]*\b)",
                                    r"\g<0>%s" % kb.customInjectionMark,
                                    _,
                                )
                                conf.data = conf.data.replace(
                                    match.group(0),
            ...
            if choice == "Q":
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    if '"' in conf.data:
                        ...
                        conf.data = re.sub(
                            r'((?P<name>"[^"]+"|\w+)\s*:\s*)(-?\d[\d\.]*\b)',
                            functools.partial(
                                process, repl=r"\g<0>%s" % kb.customInjectionMark
                            ),
                            conf.data,
                        )
                    else:
            ...
            if choice == "Q":
                ...
                kb.postHint = (
                    POST_HINT.SOAP if "soap" in conf.data.lower() else POST_HINT.XML
                )
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    conf.data = getattr(conf.data, UNENCODED_ORIGINAL_VALUE, conf.data)
                    conf.data = conf.data.replace(
                    ...
                    conf.data = re.sub(
                        r"(<(?P<name>[^>]+)( [^<]*)?>)([^<]+)(</\2)",
                        functools.partial(
                            process, repl=r"\g<1>\g<4>%s\g<5>" % kb.customInjectionMark
                        ),
                        conf.data,
                    )
            ...
            if choice == "Q":
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    conf.data = re.sub(
                        ...
                        % ("\r\n" if "\r\n" in conf.data else "\n"),
                        functools.partial(
                            process, repl=r"\g<1>%s\g<4>" % kb.customInjectionMark
                        ),
                        conf.data,
                    )
        ...
        if not kb.postHint:
            ...
            if (
                kb.customInjectionMark not in conf.data
            ):  # in case that no usable parameter values has been found
                conf.parameters[PLACE.POST] = conf.data

    kb.processUserMarks = (
        True
        if (kb.postHint and kb.customInjectionMark in (conf.data or ""))
        else kb.processUserMarks
    )

    if (
        re.search(URI_INJECTABLE_REGEX, conf.url, re.I)
        and not any(place in conf.parameters for place in (PLACE.GET, PLACE.POST))
        and not kb.postHint
        and kb.customInjectionMark not in (conf.data or "")
        and conf.url.startswith("http")
    ):
    ...
    for place, value in (
        ...
        _ = (
            re.sub(PROBLEMATIC_CUSTOM_INJECTION_PATTERNS, "", value or "")
        ...
        if kb.customInjectionMark in _:
            if kb.processUserMarks is None:
                ...
                message = (
                    "custom injection marker ('%s') found in option "
                    % kb.customInjectionMark
                )
                ...
                if choice == "Q":
                    ...
                    if kb.processUserMarks:
                        ...
                        if "=%s" % kb.customInjectionMark in _:
                            warnMsg = (
                                "it seems that you've provided empty parameter"
                                " value(s) "
                            )
                            warnMsg += (
                                "for testing. Please, always use only valid parameter"
                                " values "
                            )
            ...
            if not kb.processUserMarks:
                ...
                if place == PLACE.URI:
                    value = conf.url = conf.url.replace(
                        "+", "%20"
                    )  # NOTE: https://github.com/sqlmapproject/sqlmap/issues/5123
                ...
                if place == PLACE.CUSTOM_HEADER:
                        ...
                        if kb.customInjectionMark in re.sub(
                            ...
                            for i in xrange(len(parts) - 1):
                                conf.paramDict[place][
                                    "%s #%d%s" % (header, i + 1, kb.customInjectionMark)
                                ] = "%s,%s" % (
                                    header,
                                    "".join(
                                        "%s%s"
                    ...
                    for i in xrange(len(parts) - 1):
                        ...
                        conf.paramDict[place][name] = "".join(
                            "%s%s"
                            % (parts[j], kb.customInjectionMark if i == j else "")
                            for j in xrange(len(parts))
                        )
    ...
    if conf.csrfToken:
        if (
            not any(
                ...
                for _ in (
                    ...
                    conf.paramDict.get(PLACE.COOKIE, {}),
                )
            )
            and not re.search(r"\b%s\b" % conf.csrfToken, conf.data or "")
            and conf.csrfToken not in set(_[0].lower() for _ in conf.httpHeaders)
            and conf.csrfToken not in conf.paramDict.get(PLACE.COOKIE, {})
            and not all(
                re.search(conf.csrfToken, _, re.I)
                for _ in conf.paramDict.get(PLACE.URI, {}).values()
            )
        ):
        ...
        for place in (PLACE.GET, PLACE.POST, PLACE.COOKIE):
            ...
            for parameter in conf.paramDict.get(place, {}):
                if any(
                    parameter.lower().count(_) for _ in CSRF_TOKEN_PARAMETER_INFIXES
                ):
                    message = "%sparameter '%s' appears to hold anti-CSRF token. " % (
                        "%s " % place if place != parameter else "",
                        parameter,
                    )
                    message += (
                        "Do you want sqlmap to automatically update it in further"
                        " requests? [y/N] "
                    )
...
def _setHashDB():
    ...
    if not conf.hashDBFile:
        conf.hashDBFile = conf.sessionFile or os.path.join(
            conf.outputPath, SESSION_SQLITE_FILE
        )
...
def _resumeHashDBValues():
    ...
    kb.absFilePaths = (
        hashDBRetrieve(HASHDB_KEYS.KB_ABS_FILE_PATHS, True) or kb.absFilePaths
    )
    kb.brute.tables = (
        hashDBRetrieve(HASHDB_KEYS.KB_BRUTE_TABLES, True) or kb.brute.tables
    )
    kb.brute.columns = (
        hashDBRetrieve(HASHDB_KEYS.KB_BRUTE_COLUMNS, True) or kb.brute.columns
    )
    kb.chars = hashDBRetrieve(HASHDB_KEYS.KB_CHARS, True) or kb.chars
    kb.dynamicMarkings = (
        hashDBRetrieve(HASHDB_KEYS.KB_DYNAMIC_MARKINGS, True) or kb.dynamicMarkings
    )
    kb.xpCmdshellAvailable = (
        hashDBRetrieve(HASHDB_KEYS.KB_XP_CMDSHELL_AVAILABLE) or kb.xpCmdshellAvailable
    )
        ...
        if (
            ...
            if not conf.technique or intersect(conf.technique, injection.data.keys()):
                if intersect(conf.technique, injection.data.keys()):
                    injection.data = dict(
                        _ for _ in injection.data.items() if _[0] in conf.technique
                    )
...
def _setResultsFile():
    """Create results file for storing results of running in a multiple target
    mode."""
    ...
    if not conf.resultsFP:
        conf.resultsFile = conf.resultsFile or os.path.join(
            paths.SQLMAP_OUTPUT_PATH, time.strftime(RESULTS_FILE_FORMAT).lower()
        )
        found = os.path.exists(conf.resultsFile)

        try:
            conf.resultsFP = openFile(
                conf.resultsFile, "a", UNICODE_ENCODING, buffering=0
            )
            ...
            try:
                ...
                handle, conf.resultsFile = tempfile.mkstemp(
                    prefix=MKSTEMP_PREFIX.RESULTS, suffix=".csv"
                )
                os.close(handle)
                conf.resultsFP = openFile(
                    conf.resultsFile, "w+", UNICODE_ENCODING, buffering=0
                )
        ...
        if not found:
            conf.resultsFP.writelines(
                "Target URL,Place,Parameter,Technique(s),Note(s)%s" % os.linesep
            )
...
def _createTargetDirs():
    ...
    conf.outputPath = os.path.join(
        getUnicode(paths.SQLMAP_OUTPUT_PATH),
    ...
    try:
        with openFile(os.path.join(conf.outputPath, "target.txt"), "w+") as f:
            f.write(
                getUnicode(kb.originalUrls.get(conf.url) or conf.url or conf.hostname)
            )
            f.write(" (%s)" % (HTTPMETHOD.POST if conf.data else HTTPMETHOD.GET))
            f.write(
                "  # %s"
                % getUnicode(
                    subprocess.list2cmdline(sys.argv), encoding=sys.stdin.encoding
                )
            )
            if conf.data:
        ...
        warnMsg = (
            "something went wrong while saving target data ('%s')" % getSafeExString(ex)
        )
        logger.warning(warnMsg)
...
def _restoreMergedOptions():
    """Restore merged options (command line, configuration file and default
    values) that could be possibly changed during the testing of previous
    target."""
...
def initTargetEnv():
    ...
    match = re.search(
        INJECT_HERE_REGEX, "%s %s %s" % (conf.url, conf.data, conf.httpHeaders)
    )
