...
def _search(dork):
    """This method performs the effective search on Google providing the google
    dork and the Google session cookie."""
    ...
    requestHeaders[HTTP_HEADER.USER_AGENT] = dict(conf.httpHeaders).get(
    ...
    try:
        req = _urllib.request.Request(
            "https://www.google.com/ncr", headers=requestHeaders
        )
    ...
    url = (  # NOTE: if consent fails, try to use the "http://"
    ...
    if not retVal:
        ...
        try:
            req = _urllib.request.Request(
                url, data=getBytes(data), headers=requestHeaders
            )
            conn = _urllib.request.urlopen(req)
            ...
            try:
                ...
                page = decodePage(
                    page,
                    ex.headers.get("Content-Encoding"),
        ...
        if (
            not retVal
            and "issue with the Tor Exit Node you are currently using" in page
        ):
