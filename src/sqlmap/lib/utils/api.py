...
class Database(object):
    ...
    def init(self):
        self.execute(
            "CREATE TABLE logs(id INTEGER PRIMARY KEY AUTOINCREMENT, taskid INTEGER,"
            " time TEXT, level TEXT, message TEXT)"
        )
        self.execute(
            "CREATE TABLE data(id INTEGER PRIMARY KEY AUTOINCREMENT, taskid INTEGER,"
            " status INTEGER, content_type INTEGER, value TEXT)"
        )
        self.execute(
            "CREATE TABLE errors(id INTEGER PRIMARY KEY AUTOINCREMENT, taskid INTEGER,"
            " error TEXT)"
        )
...
class Task(object):
    ...
    def engine_start(self):
        ...
        if os.path.exists("sqlmap.py"):
            self.process = Popen(
                [sys.executable or "python", "sqlmap.py", "--api", "-c", configFile],
                shell=False,
            ...
            os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), "sqlmap.py")
        ):
            self.process = Popen(
                [sys.executable or "python", "sqlmap.py", "--api", "-c", configFile],
                shell=False,
                cwd=os.path.join(os.path.abspath(os.path.dirname(sys.argv[0]))),
                close_fds=not IS_WIN,
            )
        else:
            self.process = Popen(
...
def check_authentication():
    ...
    try:
        ...
        if creds.count(":") != 1:
            ...
            if username.strip() != (DataStore.username or "") or password.strip() != (
...
def task_list(token=None):
    ...
    logger.debug(
        ...
        % (token, "admin" if is_admin(token) else request.remote_addr)
    )
...
def scan_stop(taskid):
    ...
    if (
        ...
        or DataStore.tasks[taskid].engine_process() is None
        or DataStore.tasks[taskid].engine_has_terminated()
    ):
...
def scan_status(taskid):
    ...
    if DataStore.tasks[taskid].engine_process() is None:
        ...
        status = (
            "terminated"
            if DataStore.tasks[taskid].engine_has_terminated() is True
            else "running"
        )
...
def version(token=None):
    ...
    logger.debug(
        "Fetched version (%s)" % ("admin" if is_admin(token) else request.remote_addr)
    )
...
def server(
    ...
    try:
        ...
        if adapter.lower() not in server_names:
            ...
            errMsg = (
                "Server support for adapter '%s' is not installed on this system "
                % adapter
            )
            errMsg += (
                "(Note: you can try to install it with 'apt install python-%s' or"
                " 'pip%s install %s')" % (adapter, "3" if six.PY3 else "", adapter)
            )
...
def _client(url, options=None):
    ...
    try:
        ...
        if DataStore.username or DataStore.password:
            headers["Authorization"] = "Basic %s" % encodeBase64(
...
def client(
    ...
    dbgMsg += (
        "\n\t$ taskid=$(curl http://%s:%d/task/new 2>1 | grep -o -I '[a-f0-9]\\{16\\}')"
        " && echo $taskid" % (host, port)
    )
    dbgMsg += (
    ...
    try:
        ...
        if (
            not isinstance(ex, _urllib.error.HTTPError)
    ...
    while True:
        try:
            ...
            command = _input(
                "api%s> " % (" (%s)" % setColor(taskid, color) if taskid else "")
            ).strip()
        ...
        if command in ("data", "log", "status", "stop", "kill"):
            ...
            if not res["success"]:
                logger.error(
                    "Failed to create new task ('%s')" % res.get("message", "")
                )
                continue
            taskid = res["taskid"]
            ...
            msg += (
                "new ARGS       Start a new scan task with provided arguments (e.g."
                " 'new -u \"http://testphp.vulnweb.com/artists.php?artist=1\"')\n"
            )
            msg += (
                "use TASKID     Switch current context to different task (e.g. 'use"
                " c04d8c5c7582efb4')\n"
            )
