...
class ProgressBar(object):
    ...
    def update(self, newAmount=0):
        ...
        allFull = self._width - len(
            "100%% [] %s/%s  (ETA 00:00)" % (self._max, self._max)
        )
    ...
    def progress(self, newAmount):
        """This method saves item delta time and shows updated progress bar
        with calculated eta."""
    ...
    def draw(self, eta=None):
        ...
        dataToStdout(
            ...
            % (
                ...
                self._max,
                "  (ETA %s)"
                % (self._convertSeconds(int(eta)) if eta is not None else "??:??"),
            )
        )
