...
class HTTPCollector(object):
    ...
    def collectRequest(
        self, requestMessage, responseMessage, startTime=None, endTime=None
    ):
...
class RawPair(object):
    def __init__(
        self, request, response, startTime=None, endTime=None, extendedArguments=None
    ):
    ...
    def toEntry(self):
        return Entry(
            request=Request.parse(self.request),
    ...
    def toDict(self):
        out = {
            ...
            "startedDateTime": (
                "%s%s"
                % (
                    datetime.datetime.fromtimestamp(self.startTime).isoformat(),
                    time.strftime("%z"),
                )
                if self.startTime
                else None
...
class Request(object):
    def __init__(
        self, method, path, httpVersion, headers, postBody=None, raw=None, comment=None
    ):
        self.method = method
        self.path = path
    ...
    def toDict(self):
        out = {
            ...
            "url": self.url,
            "headers": [
                dict(name=key.capitalize(), value=value)
...
class Response(object):
    ...
    def __init__(
        self, httpVersion, status, statusText, headers, content, raw=None, comment=None
    ):
        self.raw = raw
        self.httpVersion = httpVersion
        self.status = status
    ...
    def parse(cls, raw):
        ...
        if altered.startswith(b"HTTP response [") or altered.startswith(
            b"HTTP redirect ["
        ):
            stream = io.BytesIO(raw)
            first_line = stream.readline()
            parts = cls.extract_status.search(first_line)
            status_line = "HTTP/1.0 %s %s" % (
                getText(parts.group(1)),
                getText(parts.group(2)),
            )
            remain = stream.read()
