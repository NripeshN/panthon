...
class ReqHandler(_BaseHTTPServer.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if (
            ".." not in os.path.relpath(path, HTML_DIR)
            and os.path.isfile(path)
            and not path.endswith(DISABLED_CONTENT_EXTENSIONS)
        ):
            ...
            content = (
                '<!DOCTYPE html><html lang="en"><head><title>404 Not'
                " Found</title></head><body><h1>Not Found</h1><p>The requested URL %s"
