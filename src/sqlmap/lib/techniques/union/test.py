...
def _findUnionCharCount(
    ...
    def _orderByTechnique(lowerCount=None, upperCount=None):
        def _orderByTest(cols):
            ...
            payload = agent.payload(
                newValue=query, place=place, parameter=parameter, where=where
            )
            ...
            return (
                not any(
                    ...
                    and not re.search(_, kb.pageTemplate or "", re.I)
                    for _ in (
                        "(warning|error):",
                        "order (by|clause)",
                        "unknown column",
                ...
                and comparison(page, headers, code)
                or re.search(
        ...
        if _orderByTest(1 if lowerCount is None else lowerCount) and not _orderByTest(
            ...
            lowCols, highCols = 1 if lowerCount is None else lowerCount, (
                ORDER_BY_STEP if upperCount is None else upperCount
            )
            found = None
            while not found:
    ...
    try:
        ...
        if kb.orderByColumns is None and (
            lowerCount == 1 or conf.uCols
        ):  # Note: ORDER BY is not bullet-proof
            found = (
                _orderByTechnique(lowerCount, upperCount)
                if conf.uCols
                else _orderByTechnique()
            )
        ...
        if not isNullValue(kb.uChar):
            for regex in (kb.uChar.strip("'"), r">\s*%s\s*<" % kb.uChar.strip("'")):
                contains = [
                    count
                    for count, content in pages.items()
                    if re.search(regex, content or "", re.IGNORECASE) is not None
                ]
                if len(contains) == 1:
                    retVal = contains[0]
                    break
        ...
        if not retVal:
            ...
            if all(_ == min_ and _ != max_ for _ in ratios):
                ...
                if deviation is not None:
                    lower, upper = (
                        average(ratios) - UNION_STDEV_COEFF * deviation,
                        average(ratios) + UNION_STDEV_COEFF * deviation,
                    )
...
def _fuzzUnionCols(place, parameter, prefix, suffix):
    ...
    if (
        ...
        and not re.search(FUZZ_UNION_ERROR_REGEX, kb.pageTemplate or "")
        and kb.orderByColumns
    ):
        ...
        for candidate in itertools.product(choices, repeat=kb.orderByColumns):
            if retVal:
                ...
                candidate = [
                    _.replace(FUZZ_UNION_COLUMN.INTEGER, str(randomInt())).replace(
                        FUZZ_UNION_COLUMN.STRING, "'%s'" % randomStr(20)
                    )
                    for _ in candidate
                ]
            ...
            query = agent.prefixQuery(
                ...
                % (
                    ",".join(candidate),
                    FROM_DUMMY_TABLE.get(Backend.getIdentifiedDbms(), ""),
                ),
                prefix=prefix,
            )
            query = agent.suffixQuery(query, suffix=suffix, comment=comment)
        ...
        for position in positions:
            ...
            if content and phrase in content:
                ...
                if where == PAYLOAD.WHERE.ORIGINAL:
                    ...
                    phrase2 = (
                        "%s%s%s" % (kb.chars.start, randQuery2, kb.chars.stop)
                    ).lower()
                    ...
                    if not all(_ in content for _ in (phrase, phrase2)):
                        ...
                        if (
                            content.count(phrase) > 0
                            and content.count(phrase) < LIMITED_ROWS_TEST_NUMBER
                        ):
                            warnMsg = (
                                "output with limited number of rows detected. Switching"
                                " to partial mode"
                            )
...
def _unionTestByCharBruteforce(comment, place, parameter, value, prefix, suffix):
    """This method tests if the target URL is affected by an union SQL
    injection vulnerability.

    The test is done up to 50 columns on the target database table
    """
    ...
    if conf.uColsStop == conf.uColsStart:
        ...
        count = _findUnionCharCount(
            comment, place, parameter, value, prefix, suffix, where
        )
    ...
    if count:
        ...
        if not all((validPayload, vector)) and not all(
            (conf.uChar, conf.dbms, kb.unionTemplate)
        ):
            if (
                Backend.getIdentifiedDbms()
                and kb.orderByColumns
                and kb.orderByColumns < FUZZ_UNION_MAX_COLUMNS
            ):
                if kb.fuzzUnionTest is None:
                    ...
                    if kb.fuzzUnionTest:
                        kb.unionTemplate = _fuzzUnionCols(
        ...
        if (
            orderBy is None
            and kb.orderByColumns is not None
            and not all((validPayload, vector))
        ):  # discard ORDER BY results (not usable - e.g. maybe invalid altogether)
...
def unionTest(comment, place, parameter, value, prefix, suffix):
    ...
    The test is done up to 3*50 times
    """
