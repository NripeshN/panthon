...
class Databases(object):
    """This class defines databases' enumeration functionalities for
    plugins."""
    ...
    def getDbs(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            ...
            warnMsg = (
                "user names are going to be used on %s " % Backend.getIdentifiedDbms()
            )
            warnMsg += "for enumeration as the counterpart to database "
            warnMsg += "names on other DBMSes"
            logger.warning(warnMsg)
    ...
    def getTables(self, bruteForce=None):
        ...
        if bruteForce:
            ...
            message = "do you want to use common table existence check? %s " % (
                ...
                in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB)
                else "[y/N/q]"
            )
        ...
        if (
            ...
            for query, condition in (
                (rootQuery.inband.query, getattr(rootQuery.inband, "condition", None)),
                (
                    getattr(rootQuery.inband, "query2", None),
                    getattr(rootQuery.inband, "condition2", None),
                ),
            ):
                        ...
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                if Backend.getIdentifiedDbms() in (
                                    ...
                                    query = _.query % (
                                        unsafeSQLIdentificatorNaming(db.upper()),
                                        unsafeSQLIdentificatorNaming(table.upper()),
                                    )
                                else:
                                    query = _.query % (
                                        unsafeSQLIdentificatorNaming(db),
                                ...
                                if not isNoneValue(comment):
                                    infoMsg = (
                                        "retrieved comment '%s' for table '%s'"
                                        % (comment, unsafeSQLIdentificatorNaming(table))
                                    )
        ...
        if not kb.data.cachedTables and isInferenceAvailable() and not conf.direct:
            for db in dbs:
                ...
                for _query, _count in (
                    (rootQuery.blind.query, rootQuery.blind.count),
                    (
                        getattr(rootQuery.blind, "query2", None),
                        getattr(rootQuery.blind, "count2", None),
                    ),
                ):
                    if _query is None:
                        break
    ...
    def getColumns(
        ...
        if bruteForce:
            ...
            if kb.choices.columnExists is None:
                ...
                kb.choices.columnExists = readInput(
                    message, default="Y" if "Y" in message else "N"
                ).upper()

            if kb.choices.columnExists == "N":
                if dumpMode and colList:
                    kb.data.cachedColumns[safeSQLIdentificatorNaming(conf.db)] = {
                        safeSQLIdentificatorNaming(tbl, True): dict(
            ...
            for tbl in tblList:
                ...
                if Backend.isDbms(DBMS.SQLITE):
                    if dumpMode and colList:
                        ...
                        kb.data.cachedColumns[conf.db][
                            safeSQLIdentificatorNaming(conf.tbl, True)
                        ] = dict((_, None) for _ in colList)
                    else:
                        ...
                        if not isNoneValue(columnData):
                            ...
                            if name:
                                ...
                                if len(columnData) == 1:
                                    ...
                                    key = (
                                        int(columnData[1])
                                        if isinstance(columnData[1], six.string_types)
                                    ...
                                    if Backend.isDbms(DBMS.FIREBIRD):
                                        ...
                                        columnData[1] = ALTIBASE_TYPES.get(
                                            key, columnData[1]
                                        )
                                    elif Backend.isDbms(DBMS.INFORMIX):
                                        notNull = False
                ...
                for index in getLimitRange(count):
                    ...
                    if not isNoneValue(column):
                        ...
                        if not onlyColNames:
                            ...
                            key = (
                                int(colType)
                                if hasattr(colType, "isdigit") and colType.isdigit()
                                else colType
                            )
    ...
    def getCount(self):
        ...
        if (
            ...
            not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD, DBMS.MCKOI, DBMS.EXTREMEDB)
        ):
    ...
    def getStatements(self):
        ...
        if not kb.data.cachedStatements and isInferenceAvailable() and not conf.direct:
            ...
            if Backend.isDbms(DBMS.MYSQL) and Backend.isFork(FORK.DRIZZLE):
                query = re.sub(
                    "INFORMATION_SCHEMA", "DATA_DICTIONARY", query, flags=re.I
                )
            ...
            for index in indexRange:
                ...
                if Backend.getIdentifiedDbms() in (
                    ...
                    identifier = unArrayizeValue(
                        inject.getValue(
                            query, union=False, error=False, expected=EXPECTED.INT
                        )
                    )
        ...
        if not kb.data.cachedStatements:
            ...
            kb.data.cachedStatements = [
                _.replace(REFLECTED_VALUE_MARKER, "<payload>")
                for _ in kb.data.cachedStatements
            ]
