...
class Filesystem(object):
    """This class defines generic OS file system functionalities for
    plugins."""
    ...
    def _checkFileLength(self, localFile, remoteFile, fileRead=False):
        if Backend.isDbms(DBMS.MYSQL):
            ...
            inject.goStacked(
                "INSERT INTO %s(%s) SELECT %s FROM OPENROWSET(BULK '%s', SINGLE_BLOB)"
                " AS %s(%s)"
                % (
                    self.fileTblName,
                    self.tblField,
    ...
    def fileToSqlQueries(self, fcEncodedList):
        """Called by MySQL and PostgreSQL plugins to write a file on the back-
        end DBMS underlying file system."""
    ...
    def writeFile(self, localFile, remoteFile, fileType=None, forceCheck=False):
        ...
        if conf.direct or isStackingAvailable():
            ...
            written = self.linesTerminatedWriteFile(
