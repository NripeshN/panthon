...
class Filesystem(GenericFilesystem):
    ...
    def stackedWriteFile(self, localFile, remoteFile, fileType=None, forceCheck=False):
        ...
        addFuncQuery = (
            "CREATE PROCEDURE %s (IN paramString VARCHAR, IN paramArrayOfByte"
            " VARBINARY(%s)) " % (func_name, max_bytes)
        )
        ...
        message = (
            "the local file '%s' has been written on the back-end DBMS" % localFile
        )
