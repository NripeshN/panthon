    ...
    def checkDbms(self):
        ...
        * http://dev.mysql.com/doc/refman/6.0/en/news-6-0-x.html (manual has been withdrawn)
        ...
        if not conf.extensiveFp and Backend.isDbmsWithin(MYSQL_ALIASES):
            ...
            if Backend.isVersionGreaterOrEqualThan(
                "5"
            ) or inject.checkBooleanExpression("DATABASE() LIKE SCHEMA()"):
            ...
            # Determine if it is MySQL >= 5.0.0
                ...
                # Check if it is MySQL >= 5.1.2 and < 5.5.0
                    ...
                    if inject.checkBooleanExpression(
                        "[RANDNUM]=(SELECT [RANDNUM] FROM"
                        " information_schema.GLOBAL_STATUS LIMIT 0, 1)"
                    ):
                        Backend.setVersionList([">= 5.1.12", "< 5.5.0"])
                    elif inject.checkBooleanExpression(
                        "[RANDNUM]=(SELECT [RANDNUM] FROM"
                        " information_schema.PROCESSLIST LIMIT 0, 1)"
                    ):
                        Backend.setVersionList([">= 5.1.7", "< 5.1.12"])
                    elif inject.checkBooleanExpression(
                        "[RANDNUM]=(SELECT [RANDNUM] FROM information_schema.PARTITIONS"
                        " LIMIT 0, 1)"
                    ):
                        Backend.setVersion("= 5.1.6")
                    elif inject.checkBooleanExpression(
                        "[RANDNUM]=(SELECT [RANDNUM] FROM information_schema.PLUGINS"
                        " LIMIT 0, 1)"
                    ):
                ...
                # Check if it is MySQL >= 5.0.0 and < 5.1.2
                    ...
                    "[RANDNUM]=(SELECT [RANDNUM] FROM DUAL WHERE"
                    " [RANDNUM1]!=[RANDNUM2])"
                ):
