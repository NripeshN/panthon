...
class Filesystem(GenericFilesystem):
    def nonStackedReadFile(self, rFile):
        ...
        result = inject.getValue(
            "HEX(LOAD_FILE('%s'))" % rFile, charsetType=CHARSET_TYPE.HEXADECIMAL
        )
    ...
    def stackedReadFile(self, remoteFile):
        ...
        inject.goStacked(
            "SELECT HEX(LOAD_FILE('%s')) INTO DUMPFILE '%s'" % (remoteFile, tmpFile)
        )
        ...
        length = inject.getValue(
            "SELECT LENGTH(%s) FROM %s" % (self.tblField, self.fileTblName),
            resumeValue=False,
        ...
        if not isNumPosStrValue(length):
            ...
            if length > chunkSize:
                ...
                result = inject.getValue(
                    "SELECT %s FROM %s" % (self.tblField, self.fileTblName),
                    resumeValue=False,
                    charsetType=CHARSET_TYPE.HEXADECIMAL,
                )
    ...
    def linesTerminatedWriteFile(
        ...
        query = getSQLSnippet(
            DBMS.MYSQL, "write_file_limit", OUTFILE=remoteFile, HEXSTRING=fcEncodedStr
        )
        ...
        Request.queryPage(
            payload, content=False, raise404=False, silent=True, noteResponseTime=False
        )
    ...
    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        ...
        inject.goStacked(
            "SET GLOBAL max_allowed_packet = %d" % (1024 * 1024)
        )  # 1MB (Note: https://github.com/sqlmapproject/sqlmap/issues/3230)
        ...
        inject.goStacked(
            ...
            % (self.tblField, self.fileTblName, remoteFile),
            silent=True,
        )
