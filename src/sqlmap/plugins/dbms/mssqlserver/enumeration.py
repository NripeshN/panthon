...
class Enumeration(GenericEnumeration):
    ...
    def getTables(self):
        ...
        infoMsg += "%s: %s" % (
            "s" if len(dbs) > 1 else "",
            ", ".join(
                db if isinstance(db, six.string_types) else db[0] for db in sorted(dbs)
            ),
        )
        logger.info(infoMsg)
                ...
                for index in xrange(int(count)):
                    _ = safeStringFormat(
                        (
                            ...
                            if query == rootQuery.blind.count
                            else (
                                rootQuery.blind.query2
                                if query == rootQuery.blind.count2
                                else rootQuery.blind.query3
                            )
                        ).replace("%s", db),
                        index,
                    )
    ...
    def searchColumn(self):
        ...
        for column in colList:
            ...
            if conf.db:
                ...
                infoMsgDb = " not in system database%s '%s'" % (
                        ...
                        for foundTbl in values:
                            ...
                            if colConsider == "1":
                                ...
                                if (
                                    ...
                                    and foundTbl in kb.data.cachedColumns[db]
                                    and not isNoneValue(
