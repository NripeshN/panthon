...
class MultipartPostHandler(_urllib.request.BaseHandler):
        ...
        if isinstance(data, dict):
            ...
            try:
                ...
                raise SqlmapDataException(
                    "not a valid non-string sequence or mapping object '%s'" % traceback
                )
    ...
    def multipart_encode(self, vars, files, boundary=None, buf=None):
        ...
        for key, fd in files:
            file_size = (
                fd.len if hasattr(fd, "len") else os.fstat(fd.fileno())[stat.ST_SIZE]
            )
            filename = (
            ...
            try:
                contenttype = (
                    mimetypes.guess_type(filename)[0] or b"application/octet-stream"
                )
            except:
