...
class _baseDes(object):
    ...
    def setPadMode(self, mode):
        """Sets the type of padding mode, pyDes.PAD_NORMAL or
        pyDes.PAD_PKCS5."""
        self._padmode = mode
        ...
        if padmode == PAD_NORMAL:
            ...
            if not pad:
                raise ValueError(
                    ...
                    + str(self.block_size)
                    + " bytes in length. Use padmode=PAD_PKCS5 or set the pad"
                    " character."
                )
...
class des(_baseDes):
    ...
    Supports ECB (Electronic Code Book) and CBC (Cypher Block Chaining)
    modes.
    ...
    mode -> Optional argument for encryption type, can be either
    pyDes.ECB         (Electronic Code Book), pyDes.CBC (Cypher Block
    Chaining) IV   -> Optional Initial Value bytes, must be supplied if
...
class triple_des(_baseDes):
    ...
    24 bytes long mode -> Optional argument for encryption type, can be
    either pyDes.ECB         (Electronic Code Book), pyDes.CBC (Cypher
    Block Chaining) IV   -> Optional Initial Value bytes, must be
    supplied if using CBC mode.         Must be 8 bytes in length. pad
    -> Optional argument, set the pad character (PAD_NORMAL) to use
    ...
    def setKey(self, key):
        ...
        if len(key) != self.key_size:
            if len(key) == 16:  # Use DES-EDE2 mode
                ...
                raise ValueError(
                    "Invalid triple DES key size. Key must be either 16 or 24 bytes"
                    " long"
                )
        if self.getMode() == CBC:
            if not self.getIV():
        ...
        if self.key_size == 16:
            ...
            self.__key3 = des(
                key[16:], self._mode, self._iv, self._padding, self._padmode
            )
        _baseDes.setKey(self, key)
