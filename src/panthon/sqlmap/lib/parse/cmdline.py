...
try:
    ...
    def get_groups(parser):
        return getattr(parser, "option_groups", None) or getattr(
...
def cmdLineParser(argv=None):
    ...
    usage = "%s%s [options]" % (
        "%s " % os.path.basename(sys.executable) if not IS_WIN else "",
        '"%s"' % _ if " " in _ else _,
    )
    ...
    try:
        ...
        target.add_argument(
            "-d", dest="direct", help="Connection string for direct database connection"
        )
        ...
        target.add_argument(
            "-m", dest="bulkFile", help="Scan multiple targets given in a textual file "
        )

        target.add_argument(
            "-r", dest="requestFile", help="Load HTTP request from a file"
        )

        target.add_argument(
            "-g", dest="googleDork", help="Process Google dork results as target URLs"
        )

        target.add_argument(
            "-c", dest="configFile", help="Load options from a configuration INI file"
        )
        ...
        request.add_argument(
            "-A", "--user-agent", dest="agent", help="HTTP User-Agent header value"
        )
        ...
        request.add_argument(
            "--referer", dest="referer", help="HTTP Referer header value"
        )
        ...
        request.add_argument(
            "--proxy-file", dest="proxyFile", help="Load proxy list from a file"
        )
        ...
        request.add_argument(
            "--tor", dest="tor", action="store_true", help="Use Tor anonymity network"
        )

        request.add_argument(
            "--tor-port", dest="torPort", help="Set Tor proxy port other than default"
        )
        ...
        request.add_argument(
            "--safe-post", dest="safePost", help="POST data to send to a safe URL"
        )

        request.add_argument(
            "--safe-req", dest="safeReqFile", help="Load safe HTTP request from a file"
        )
        ...
        request.add_argument(
            ...
            dest="csrfRetries",
            type=int,
            help="Retries for anti-CSRF token retrieval (default %d)"
            % defaults.csrfRetries,
        )
        ...
        injection.add_argument(
            "--skip", dest="skip", help="Skip testing for given parameter(s)"
        )
        ...
        injection.add_argument(
            "--dbms", dest="dbms", help="Force back-end DBMS to provided value"
        )
        ...
        injection.add_argument(
            "--prefix", dest="prefix", help="Injection payload prefix string"
        )

        injection.add_argument(
            "--suffix", dest="suffix", help="Injection payload suffix string"
        )
        ...
        enumeration.add_argument(
            "--users", dest="getUsers", action="store_true", help="Enumerate DBMS users"
        )
        ...
        enumeration.add_argument(
            "--dbs", dest="getDbs", action="store_true", help="Enumerate DBMS databases"
        )
        ...
        enumeration.add_argument("-D", dest="db", help="DBMS database to enumerate")

        enumeration.add_argument(
            "-T", dest="tbl", help="DBMS database table(s) to enumerate"
        )

        enumeration.add_argument(
            "-C", dest="col", help="DBMS database table column(s) to enumerate"
        )

        enumeration.add_argument(
            "-X", dest="exclude", help="DBMS database identifier(s) to not enumerate"
        )

        enumeration.add_argument("-U", dest="user", help="DBMS user to enumerate")
        ...
        enumeration.add_argument(
            "--pivot-column", dest="pivotColumn", help="Pivot column name"
        )

        enumeration.add_argument(
            "--where", dest="dumpWhere", help="Use WHERE condition while table dumping"
        )
        ...
        enumeration.add_argument(
            "--sql-query", dest="sqlQuery", help="SQL statement to be executed"
        )
        ...
        # Brute force options
        brute = parser.add_argument_group(
        ...
        udf.add_argument(
            "--shared-lib", dest="shLib", help="Local path of the shared library"
        )
        ...
        takeover.add_argument(
            "--os-cmd", dest="osCmd", help="Execute an operating system command"
        )
        ...
        windows.add_argument("--reg-key", dest="regKey", help="Windows registry key")

        windows.add_argument(
            "--reg-value", dest="regVal", help="Windows registry key value"
        )

        windows.add_argument(
            "--reg-data", dest="regData", help="Windows registry key value data"
        )

        windows.add_argument(
            "--reg-type", dest="regType", help="Windows registry key value type"
        )
        ...
        general.add_argument(
            "-s", dest="sessionFile", help="Load session from a stored (.sqlite) file"
        )

        general.add_argument(
            "-t", dest="trafficFile", help="Log all HTTP traffic into a textual file"
        )
        ...
        general.add_argument(
            "--dump-file", dest="dumpFile", help="Store dumped data to a custom file"
        )
        ...
        general.add_argument(
            "--har", dest="harFile", help="Log all HTTP traffic into a HAR file"
        )
        ...
        general.add_argument(
            ...
            dest="repair",
            action="store_true",
            help="Redump entries having unknown character marker (%s)"
            % INFERENCE_UNKNOWN_CHAR,
        )

        general.add_argument(
            "--save", dest="saveConfig", help="Save options to a configuration INI file"
        )

        general.add_argument(
            "--scope", dest="scope", help="Regexp for filtering targets"
        )
        ...
        general.add_argument(
            "--table-prefix",
            dest="tablePrefix",
            help='Prefix used for temporary tables (default: "%s")'
        ...
        miscellaneous.add_argument(
            "--update", dest="updateAll", action="store_true", help="Update sqlmap"
        )
        ...
        parser.add_argument(
            "--crack", dest="hashFile", help=SUPPRESS
        )  # "Load and crack hashes from a file (standalone)"

        parser.add_argument("--dummy", dest="dummy", action="store_true", help=SUPPRESS)

        parser.add_argument("--yuge", dest="yuge", action="store_true", help=SUPPRESS)

        parser.add_argument("--murphy-rate", dest="murphyRate", type=int, help=SUPPRESS)

        parser.add_argument("--debug", dest="debug", action="store_true", help=SUPPRESS)

        parser.add_argument(
            "--deprecations", dest="deprecations", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--disable-multi", dest="disableMulti", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--disable-precon", dest="disablePrecon", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--disable-stats", dest="disableStats", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--profile", dest="profile", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--localhost", dest="localhost", action="store_true", help=SUPPRESS
        )

        parser.add_argument("--force-dbms", dest="forceDbms", help=SUPPRESS)

        parser.add_argument(
            "--force-dns", dest="forceDns", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--force-partial", dest="forcePartial", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--force-pivoting", dest="forcePivoting", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--ignore-stdin", dest="ignoreStdin", action="store_true", help=SUPPRESS
        )
        ...
        parser.add_argument("--gui", dest="gui", action="store_true", help=SUPPRESS)

        parser.add_argument(
            "--smoke-test", dest="smokeTest", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--vuln-test", dest="vulnTest", action="store_true", help=SUPPRESS
        )

        parser.add_argument(
            "--disable-json", dest="disableJson", action="store_true", help=SUPPRESS
        )

        # API options
        parser.add_argument("--api", dest="api", action="store_true", help=SUPPRESS)
        ...
        if hasattr(parser, "formatter"):
            ...
            def _(self, *args):
                ...
                if len(retVal) > MAX_HELP_OPTION_LENGTH:
                    retVal = (
                        "%%.%ds.."
                        % (MAX_HELP_OPTION_LENGTH - parser.formatter.indent_increment)
                    ) % retVal
                return retVal

            parser.formatter._format_option_strings = (
            ...
            def _format_action_invocation(self, action):
                ...
                if len(retVal) > MAX_HELP_OPTION_LENGTH:
                    retVal = (
                        "%%.%ds.." % (MAX_HELP_OPTION_LENGTH - self._indent_increment)
                    ) % retVal
                return retVal

            parser.formatter_class.__format_action_invocation = (
        ...
        if hasattr(parser, "get_option"):
            ...
            for action in get_actions(parser):
                if action.option_strings == ["-h", "--help"]:
                    action.help = action.help.capitalize().replace(
        ...
        if "--gui" in argv:
            ...
            while True:
                ...
                if not command:
                    ...
                    dataToStdout(
                        "[i] valid example: '-u http://www.site.com/vuln.php?id=1"
                        " --banner'\n"
                    )
                else:
                    saveHistory(AUTOCOMPLETE_TYPE.SQLMAP)
        ...
        for i in xrange(len(argv)):
            ...
            if argv[i] == "-hh":
                ...
                i == 1
                and re.search(r"\A(http|www\.|\w[\w.-]+\.\w{2,})", argv[i]) is not None
            ):
                argv[i] = "--url=%s" % argv[i]
            elif len(argv[i]) > 1 and all(
                ord(_) in xrange(0x2018, 0x2020)
                for _ in ((argv[i].split("=", 1)[-1].strip() or " ")[0], argv[i][-1])
            ):
                dataToStdout(
                ...
                if index is None:
                    ...
                    argv[i] = ""
            elif argv[i] in ("-H", "--header") or any(
                argv[i].startswith("%s=" % _) for _ in ("-H", "--header")
            ):
                if "=" in argv[i]:
                ...
                re.match(r"\A\d+!\Z", argv[i])
                and argv[max(0, i - 1)] == "--threads"
                or re.match(r"\A--threads.+\d+!\Z", argv[i])
            ):
                argv[i] = argv[i][:-1]
                ...
                and argv[i].split("=")[0] in longOptions
                and re.search(r"\A-{1,2}\w", argv[i - 1]) is None
            ):
                dataToStdout(
        ...
        for verbosity in (_ for _ in argv if re.search(r"\A\-v+\Z", _)):
            try:
                if (
                    argv.index(verbosity) == len(argv) - 1
                    or not argv[argv.index(verbosity) + 1].isdigit()
                ):
        ...
        if not any(
            ...
            errMsg = (
                "missing a mandatory option (-d, -u, -l, -m, -r, -g, -c, --wizard,"
                " --shell, --update, --purge, --list-tampers or --dependencies). "
            )
