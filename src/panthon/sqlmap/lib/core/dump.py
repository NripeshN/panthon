...
class Dump(object):
    """This class defines methods used to parse and output the results of SQL
    injection actions."""
    ...
    def setOutputFile(self):
        ...
        try:
            self._outputFP = openFile(
                self._outputFile, "ab" if not conf.flushSession else "wb"
            )
        except IOError as ex:
        ...
        if isListLike(data):
            ...
            if "\n" in _:
                ...
                self._write(
                    ...
                    % (
                        header,
                        ("'%s'" % _) if isinstance(data, six.string_types) else _,
                    )
                )
    ...
    def currentDb(self, data):
        if Backend.getIdentifiedDbms() in (
            ...
            self.string(
                "current database (equivalent to schema on %s)"
            ...
            self.string(
                "current database (equivalent to owner on %s)"
                % Backend.getIdentifiedDbms(),
                data,
                content_type=CONTENT_TYPE.CURRENT_DB,
            )
    ...
    def users(self, users):
        self.lister(
            "database management system users", users, content_type=CONTENT_TYPE.USERS
        )
    ...
    def dbTables(self, dbTables):
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            for tables in dbTables.values():
                for table in tables:
                    ...
                    maxlength = max(
                        maxlength,
                        getConsoleLength(
                            unsafeSQLIdentificatorNaming(getUnicode(table))
                        ),
                    )
            ...
            for db, tables in dbTables.items():
                ...
                self._write(
                    "Database: %s" % unsafeSQLIdentificatorNaming(db)
                ...
                for table, columns in tables.items():
                    ...
                    for column in colList:
                        ...
                        if colType is not None:
                            ...
                            self._write(
                                "| %s%s | %s%s |" % (column, blank1, colType, blank2)
                            )
                        else:
        ...
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            for ctables in dbTables.values():
                for tables in ctables.values():
                    for table in tables:
                        maxlength1 = max(
                            maxlength1, getConsoleLength(getUnicode(table))
                        )
    ...
    def dbTableValues(self, tableValues):
        ...
        try:
            ...
            try:
                dumpDbPath = os.path.join(
                    conf.dumpPath, normalizeUnicode(unsafeSQLIdentificatorNaming(db))
                )
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            replication = Replication(
                os.path.join(
                    conf.dumpPath, "%s.sqlite3" % unsafeSQLIdentificatorNaming(db)
                )
            )
            ...
            if not os.path.isdir(dumpDbPath):
                try:
                    ...
                    _ = re.sub(
                        r"[^\w]",
                        UNSAFE_DUMP_FILEPATH_REPLACEMENT,
                        unsafeSQLIdentificatorNaming(db),
                    )
                    dumpDbPath = os.path.join(
                        conf.dumpPath,
                        "%s-%s" % (_, hashlib.md5(getBytes(db)).hexdigest()[:8]),
                    )
                    ...
                    if not os.path.isdir(dumpDbPath):
                        try:
                            ...
                            warnMsg += (
                                "Using temporary directory '%s' instead" % tempDir
                            )
                            logger.warning(warnMsg)

                            dumpDbPath = tempDir

            dumpFileName = conf.dumpFile or os.path.join(
                dumpDbPath,
                re.sub(
                    r"[\\/]",
                    UNSAFE_DUMP_FILEPATH_REPLACEMENT,
            ...
            if not checkFile(dumpFileName, False):
                try:
                    ...
                    if (
                        len(_) < len(table)
                        or IS_WIN
                        and table.upper() in WINDOWS_RESERVED_NAMES
                    ):
                        _ = re.sub(
                            r"[^\w]",
                            UNSAFE_DUMP_FILEPATH_REPLACEMENT,
                        ...
                        dumpFileName = os.path.join(
                            ...
                            % (
                                _,
                                hashlib.md5(getBytes(table)).hexdigest()[:8],
                ...
                if not appendToFile:
                    if conf.dumpFormat == DUMP_FORMAT.CSV:
                        if field == fields:
                            ...
                            dataToDumpFile(
                                dumpFP, "%s%s" % (safeCSValue(column), conf.csvDel)
                            )
                    elif conf.dumpFormat == DUMP_FORMAT.HTML:
                        dataToDumpFile(
                            dumpFP,
        ...
        for i in xrange(count):
            ...
            for column in columns:
                if column != "__infos__":
                    ...
                    self._write(
                        "| %s%s" % (value, blank), newline=False, console=console
                    )
                    ...
                    if len(value) > MIN_BINARY_DISK_DUMP_SIZE and r"\x" in value:
                        try:
                            ...
                            if any(
                                mimetype.startswith(_) for _ in ("application", "image")
                            ):
                                if not os.path.isdir(dumpDbPath):
                                    os.makedirs(dumpDbPath)
                                ...
                                filepath = os.path.join(
                                    dumpDbPath, "%s-%d.bin" % (_, randomInt(8))
                                )
                                warnMsg = (
                                    "writing binary ('%s') content to file '%s' "
                                    % (mimetype, filepath)
                                )
                                logger.warning(warnMsg)
                    ...
                    if conf.dumpFormat == DUMP_FORMAT.CSV:
                        if field == fields:
                            ...
                            dataToDumpFile(
                                dumpFP, "%s%s" % (safeCSValue(value), conf.csvDel)
                            )
                    elif conf.dumpFormat == DUMP_FORMAT.HTML:
                        dataToDumpFile(
                            dumpFP,
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            ...
            logger.info(
                "table '%s.%s' dumped to SQLITE database '%s'"
    ...
    def rFile(self, fileData):
        self.lister(
            "files saved to", fileData, sort=False, content_type=CONTENT_TYPE.FILE_READ
        )
