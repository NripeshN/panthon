...
class Fingerprint(GenericFingerprint):
    ...
    def _versionCheck(self):
        ...
        for version in reversed(xrange(5, 15)):
            result = inject.checkBooleanExpression(
                "(SELECT COUNT(*) FROM sysibm.sysversions WHERE versionnumber BETWEEN"
                " %d000000 AND %d999999)>0" % (version, version)
            )
            ...
            if result:
                ...
                for version in reversed(xrange(0, 20)):
                    result = inject.checkBooleanExpression(
                        "(SELECT COUNT(*) FROM sysibm.sysversions WHERE versionnumber"
                        " BETWEEN %d%02d0000 AND %d%02d9999)>0"
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        query = (
            "(SELECT LENGTH(OS_NAME) FROM SYSIBMADM.ENV_SYS_INFO WHERE OS_NAME LIKE"
            " '%WIN%')>0"
        )
