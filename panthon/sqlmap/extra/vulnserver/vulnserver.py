    ...
    def do_REQUEST(self):
        ...
        if self.url == "/":
            if not any(_ in self.params for _ in ("id", "query")):
                ...
                self.send_header(
                    "Content-type", "text/html; charset=%s" % UNICODE_ENCODING
                )
                self.send_header("Connection", "close")
                self.end_headers()
                self.wfile.write(
                    b"<!DOCTYPE"
                    b" html><html><head><title>vulnserver</title></head><body><h3>GET:</h3><a"
                    b" href='/?id=1'>link</a><hr><h3>POST:</h3><form method='post'>ID:"
                    b" <input type='text' name='id'><input type='submit'"
                    b" value='Submit'></form></body></html>"
                )
            else:
                code, output = OK, ""
                ...
                try:
                    ...
                    code = INTERNAL_SERVER_ERROR
                    output = "%s: %s" % (
                        re.search(r"'([^']+)'", str(type(ex))).group(1),
                        ex,
                    )
                ...
                if self.raw_requestline.startswith(b"HEAD"):
                    ...
                    self.wfile.write(
                        ...
                        else output.encode(UNICODE_ENCODING)
                    )
        else:
...
if __name__ == "__main__":
    try:
        init()
        run(
            sys.argv[1] if len(sys.argv) > 1 else LISTEN_ADDRESS,
