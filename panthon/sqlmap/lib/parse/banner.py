        ...
        if name == "signature":
            for version in (self._version, self._versionAlt):
                if (
                    version
                    and self._banner
                    and re.search(r" %s[\.\ ]+" % re.escape(version), self._banner)
                ):
            ...
            self._versionAlt = (
                "%s.0.%s.0" % (match.group("major"), match.group("build"))
                if match
                else None
            )
...
def bannerParser(banner):
    """This function calls a class to extract information from the given DBMS
    banner based upon the data in XML file."""
