...
def _oneShotErrorUse(expression, field=None, chunkTest=False):
    ...
    if retVal and PARTIAL_VALUE_MARKER in retVal:
        ...
        logger.info(
            "resuming partial value: '%s'" % _formatPartialContent(partialValue)
        )
    ...
    if (
        ...
        and kb.errorChunkLength is None
        and not chunkTest
        and not kb.testMode
    ):
        ...
        while current >= MIN_ERROR_CHUNK_LENGTH:
            ...
            if Backend.isDbms(DBMS.ORACLE):
                ...
                testQuery = "%s('%s',%d)" % (
                    "REPEAT" if Backend.isDbms(DBMS.MYSQL) else "REPLICATE",
                    testChar,
                    current,
                )
                testQuery = "SELECT %s" % (
                    agent.hexConvertField(testQuery) if conf.hexConvert else testQuery
                )
            ...
            if (result or "").startswith(testChar):
                if result == testChar * current:
                    ...
                    current = (
                        candidate
                        if candidate != current and candidate not in seen
                        else current - 1
                    )
            else:
                current = current // 2
            ...
            while True:
                ...
                if field:
                    ...
                    if (
                        ...
                        and kb.errorChunkLength
                        and not chunkTest
                    ):
                        extendedField = re.search(
                            r"[^ ,]*%s[^ ,]*" % re.escape(field), expression
                        ).group(0)
                ...
                injExpression = (
                    expression.replace(field, nulledCastedField, 1)
                    if field
                    else expression
                )
                ...
                if any(
                    ...
                    if (
                        output
                        and kb.errorChunkLength
                        and len(output) >= kb.errorChunkLength
                        and not chunkTest
                    ):
                    ...
                    if (
                        output
                        and conf.verbose in (1, 2)
                        and not any((conf.api, kb.bruteMode))
                    ):
                        if kb.fileReadMode:
                            dataToStdout(
                                _formatPartialContent(output)
                                .replace(r"\n", "\n")
                                .replace(r"\t", "\t")
                            )
                        elif offset > 1:
...
def _errorFields(
    ...
    for field in expressionFieldsList:
        ...
        output = (
            NULL
            if emptyFields and field in emptyFields
...
def _errorReplaceChars(value):
    ...
    if value:
        retVal = (
            retVal.replace(kb.chars.space, " ")
            .replace(kb.chars.dollar, "$")
            .replace(kb.chars.at, "@")
            .replace(kb.chars.hash_, "#")
        )
...
def _formatPartialContent(value):
    """Prepares (possibly hex-encoded) partial content for safe console
    output."""
...
def errorUse(expression, dump=False):
    ...
    if (
        ...
        or (
            ...
            and (
                "(CASE" not in expression.upper()
                or ("(CASE" in expression.upper() and "WHEN use" in expression)
            )
        )
        ...
        if limitCond:
            ...
            if isNumPosStrValue(count) and int(count) > 1:
                if (
                    ...
                    and (stopLimit - startLimit) > SLOW_ORDER_COUNT_THRESHOLD
                ):
                ...
                if kb.dumpTable and (
                    ...
                    for field in expressionFieldsList:
                        if (
                            _oneShotErrorUse(
                                "SELECT COUNT(%s) FROM %s" % (field, kb.dumpTable)
                            )
                            == "0"
                        ):
                            ...
                            with kb.locks.value:
                                ...
                                for index in xrange(
                                    ...
                                    if (
                                        ...
                                        and threadData.shared.buffered[index][0] >= num
                                    ):
                                        break
                                ...
                                while (
                                    threadData.shared.buffered
                                    and threadData.shared.lastFlushed + 1
    ...
    if not kb.bruteMode:
        debugMsg = "performed %d quer%s in %.2f seconds" % (
            ...
            "y" if kb.counters[getTechnique()] == 1 else "ies",
            duration,
        )
        logger.debug(debugMsg)
