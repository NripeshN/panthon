...
def mssql_new_passwd(password, salt, uppercase=False):  # since version '2012'
    ...
    unistr = b"".join(
        (
            (_.encode(UNICODE_ENCODING) + b"\0")
            if ord(_) < 256
            else _.encode(UNICODE_ENCODING)
        )
        for _ in password
    )
...
def oracle_old_passwd(password, username, uppercase=True):  # prior to version '11g'
    ...
    unistr = b"".join(
        (
            (b"\0" + _.encode(UNICODE_ENCODING))
            if ord(_) < 256
            else _.encode(UNICODE_ENCODING)
        )
        for _ in (username + password).upper()
    )
...
def ssha256_passwd(password, salt, **kwargs):
    ...
    return "{SSHA256}%s" % getText(
        base64.b64encode(sha256(password + salt).digest() + salt)
    )
...
def ssha512_passwd(password, salt, **kwargs):
    ...
    return "{SSHA512}%s" % getText(
        base64.b64encode(sha512(password + salt).digest() + salt)
    )
...
def unix_md5_passwd(password, salt, magic="$1$", **kwargs):
    ...
    hash_ = _encode64(
        (int(ord(final[0:1])) << 16)
        | (int(ord(final[6:7])) << 8)
        | (int(ord(final[12:13]))),
        4,
    )
    hash_ = hash_ + _encode64(
        (int(ord(final[1:2])) << 16)
        | (int(ord(final[7:8])) << 8)
        | (int(ord(final[13:14]))),
        4,
    )
    hash_ = hash_ + _encode64(
        (int(ord(final[2:3])) << 16)
        | (int(ord(final[8:9])) << 8)
        | (int(ord(final[14:15]))),
        4,
    )
    hash_ = hash_ + _encode64(
        (int(ord(final[3:4])) << 16)
        | (int(ord(final[9:10])) << 8)
        | (int(ord(final[15:16]))),
        4,
    )
    hash_ = hash_ + _encode64(
        (int(ord(final[4:5])) << 16)
...
def vbulletin_passwd(password, salt, **kwargs):
    ...
    return "%s:%s" % (
        md5(
            binascii.hexlify(md5(getBytes(password)).digest()) + getBytes(salt)
        ).hexdigest(),
        salt,
    )
...
def phpass_passwd(password, salt, count, prefix, **kwargs):
    ...
    def _encode64(input_, count):
        ...
        while i < count:
            ...
            if i < count:
                value = value | (
                    (input_[i] if isinstance(input_[i], int) else ord(input_[i])) << 8
                )
            ...
            if i < count:
                value = value | (
                    (input_[i] if isinstance(input_[i], int) else ord(input_[i])) << 16
                )
    ...
    retVal = "%s%s%s%s" % (
        prefix,
        ITOA64[int(math.log(count, 2))],
        salt,
        _encode64(hash_, len(hash_)),
    )
    ...
    if kb.data.dumpedTable:
        ...
        for column in columns:
            if column != "__infos__" and table[column]["values"]:
                if all(
                    INVALID_UNICODE_CHAR_FORMAT.split("%")[0] in (value or "")
        ...
        if binary_fields:
            ...
            warnMsg = (
                "potential binary fields detected ('%s'). In case of any problems you"
                " are " % _
            )
            warnMsg += (
                "advised to rerun table dump with '--fresh-queries"
                ' --binary-fields="%s"\'' % _
            )
        ...
        for i in xrange(count):
            ...
            for column in columns:
                ...
                if (
                    column in binary_fields
                    and re.search(HASH_BINARY_COLUMNS_REGEX, column) is not None
                ):
                    previous = value
        ...
        if attack_dict:
            ...
            choice = readInput(
                message, default="N" if conf.multipleTargets else "Y"
            ).upper()
                    ...
                    if not (
                        ...
                        if value and value.lower() in lut:
                            table[column]["values"][i] = "%s (%s)" % (
                                ...
                                getUnicode(
                                    lut[value.lower()] or HASH_EMPTY_PASSWORD_MARKER
                                ),
                            )
                            table[column]["length"] = max(
                                table[column]["length"], len(table[column]["values"][i])
                            )
...
def hashRecognition(value):
    ...
    if (
        value and len(value) >= 8 and " " not in value
    ):  # Note: pre-filter condition (for optimization purposes)
        ...
        if kb.cache.hashRegex is None:
            ...
            for name, regex in getPublicTypeMembers(HASH):
                ...
                if (
                    isOracle
                    and regex == HASH.MYSQL_OLD
                    or isMySQL
                    and regex == HASH.ORACLE_OLD
                ):
                    continue
        ...
        if isinstance(value, six.string_types):
            ...
            if match:
                algorithm, _ = [
                    _ for _ in match.groupdict().items() if _[1] is not None
                ][0]
...
def _bruteProcessVariantA(
    ...
    wordlist = Wordlist(
        wordlists, proc_id, getattr(proc_count, "value", 0), custom_wordlist
    )
        ...
        for word in wordlist:
            ...
            try:
                ...
                if current in hashes:
                    ...
                    and count % HASH_MOD_ITEM_DISPLAY == 0
                    or hash_regex == HASH.ORACLE_OLD
                    or hash_regex == HASH.CRYPT_GENERIC
                    and IS_WIN
                ):
                    rotator += 1
    ...
    for hash_regex in hash_regexes:
                ...
                if re.match(hash_regex, hash_):
                    try:
                        ...
                        if hash_regex in (
                            ...
                            item = [
                                (user, encodeHex(decodeBase64(hash_, binary=True))),
                                {},
                            ]
                        elif hash_regex in (
                            HASH.MYSQL,
                            ...
                            if hash_.startswith(
                                "0x"
                            ):  # Reference: https://docs.microsoft.com/en-us/sql/t-sql/functions/hashbytes-transact-sql?view=sql-server-2017
                                hash_ = hash_[2:]
                            item = [(user, hash_), {}]
                        elif hash_regex in (HASH.SSHA,):
                            item = [
                                (user, hash_),
                                {"salt": decodeBase64(hash_, binary=True)[20:]},
                            ]
                        elif hash_regex in (HASH.SSHA256,):
                            item = [
                                (user, hash_),
                                {"salt": decodeBase64(hash_, binary=True)[32:]},
                            ]
                        elif hash_regex in (HASH.SSHA512,):
                            item = [
                                (user, hash_),
                                {"salt": decodeBase64(hash_, binary=True)[64:]},
                            ]
        ...
        if not kb.wordlists:
            while not kb.wordlists:
                ...
                message += (
                    "[1] default dictionary file '%s' (press Enter)\n" % dictPaths[0]
                )
        ...
        infoMsg = (
            "starting dictionary-based cracking (%s)"
            % __functions__[hash_regex].__name__
        )
        logger.info(infoMsg)
                ...
                try:
                    if _multiprocessing:
                        if _multiprocessing.cpu_count() > 1:
                            infoMsg = (
                                "starting %d processes " % _multiprocessing.cpu_count()
                            )
                    ...
                    warnMsg = (
                        "user aborted during dictionary-based attack phase (Ctrl+C was"
                        " pressed)"
                    )
                    logger.warning(warnMsg)
            ...
            for (user, hash_), kwargs in attack_info:
                ...
                for suffix in suffix_list:
                    ...
                    try:
                        ...
                        processException = True
                        warnMsg = (
                            "user aborted during dictionary-based attack phase (Ctrl+C"
                            " was pressed)"
                        )
                        logger.warning(warnMsg)
