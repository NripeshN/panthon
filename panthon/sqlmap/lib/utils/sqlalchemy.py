...
class SQLAlchemy(GenericConnector):
    def __init__(self, dialect=None):
        ...
        if conf.dbmsUser:
            self.address = self.address.replace(
                "'%s':" % conf.dbmsUser, "%s:" % _urllib.parse.quote(conf.dbmsUser)
            )
            self.address = self.address.replace(
                "%s:" % conf.dbmsUser, "%s:" % _urllib.parse.quote(conf.dbmsUser)
            )
        ...
        if conf.dbmsPass:
            self.address = self.address.replace(
                ":'%s'@" % conf.dbmsPass, ":%s@" % _urllib.parse.quote(conf.dbmsPass)
            )
            self.address = self.address.replace(
                ":%s@" % conf.dbmsPass, ":%s@" % _urllib.parse.quote(conf.dbmsPass)
            )
            ...
            try:
                ...
                if self.dialect == "sqlite":
                    engine = _sqlalchemy.create_engine(
                        self.address, connect_args={"check_same_thread": False}
                    )
                    ...
                    try:
                        ...
                        if int(pymssql.__version__[0]) < 2:
                            raise SqlmapConnectionException(
                                "SQLAlchemy connection issue (obsolete version of"
                                " pymssql ('%s') is causing problems)"
                                % pymssql.__version__
                            )
                    except ImportError:
                        pass
                elif (
                    "invalid literal for int() with base 10: '0b"
                    in traceback.format_exc()
                ):
                    raise SqlmapConnectionException(
                        "SQLAlchemy connection issue"
                        " ('https://bitbucket.org/zzzeek/sqlalchemy/issues/3975')"
                    )
                else:
                    pass
    ...
    def execute(self, query):
        ...
        try:
            ...
            _sqlalchemy.exc.OperationalError,
            _sqlalchemy.exc.ProgrammingError,
        ) as ex:
            logger.log(
                logging.WARN if conf.dbmsHandler else logging.DEBUG,
                "(remote) %s" % getSafeExString(ex),
