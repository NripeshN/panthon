...
def pivotDumpTable(table, colList, count=None, blind=True, alias=None):
    ...
    if count is None:
        ...
        count = (
            ...
            if blind
            else inject.getValue(query, blind=False, time=False, expected=EXPECTED.INT)
        )
    ...
    def _(column, pivotValue):
        if column == colList[0]:
            query = dumpNode.query.replace(
                "'%s'" if unescaper.escape(pivotValue, False) != pivotValue else "%s",
                "%s",
            ) % (
                agent.preprocessField(table, column),
                table,
                agent.preprocessField(table, column),
            ...
            query = dumpNode.query2.replace(
                ...
                (
                    unescaper.escape(pivotValue, False)
                    if SINGLE_QUOTE_MARKER not in dumpNode.query2
                    else pivotValue
                ),
            )
        ...
        return unArrayizeValue(
            inject.getValue(
                query, blind=blind, time=blind, union=not blind, error=not blind
            )
        )
    ...
    try:
        for i in xrange(count):
            ...
            for column in colList:
                ...
                lengths[column] = max(
                    lengths[column],
                    len(DUMP_REPLACEMENTS.get(getUnicode(value), getUnicode(value))),
                )
