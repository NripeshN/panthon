...
class HashDB(object):
        ...
        if threadData.hashDBCursor is None:
            try:
                ...
                threadData.hashDBCursor.execute(
                    "CREATE TABLE IF NOT EXISTS storage (id INTEGER PRIMARY KEY, value"
                    " TEXT)"
                )
                connection.commit()
    ...
    def hashKey(key):
        key = getBytes(
            key if isinstance(key, six.text_type) else repr(key),
            errors="xmlcharrefreplace",
        )
        retVal = (
            int(hashlib.md5(key).hexdigest(), 16) & 0x7FFFFFFFFFFFFFFF
    ...
    def retrieve(self, key, unserialize=False):
            ...
            if not retVal:
                for _ in xrange(HASHDB_RETRIEVE_RETRIES):
                    try:
                        ...
                        if any(
                            _ in getSafeExString(ex)
                            for _ in ("locked", "no such table")
                        ):
                            warnMsg = (
                                "problem occurred while accessing session file '%s'"
                                " ('%s')" % (self.filepath, getSafeExString(ex))
                            )
                            ...
                            errMsg = (
                                "error occurred while accessing session file '%s'"
                                " ('%s'). " % (self.filepath, getSafeExString(ex))
                            )
                            errMsg += (
                                "If the problem persists please rerun with"
                                " '--flush-session'"
                            )
                            raise SqlmapConnectionException(errMsg)
                    else:
        ...
        if retVal and unserialize:
            try:
                ...
                retVal = None
                warnMsg = (
                    "error occurred while unserializing value for session key '%s'. "
                    % key
                )
    ...
    def write(self, key, value, serialize=False):
        if key:
            ...
            self._write_cache[hash_] = (
                getUnicode(value) if not serialize else serializeObject(value)
            )
            ...
            for hash_, value in _.items():
                ...
                while True:
                    try:
                        ...
                        UnicodeError,
                        OverflowError,
                    ):  # e.g. surrogates not allowed (Issue #3851)
                        break
                    except sqlite3.DatabaseError as ex:
                        if not os.path.exists(self.filepath):
                            debugMsg = (
                                "session file '%s' does not exist" % self.filepath
                            )
