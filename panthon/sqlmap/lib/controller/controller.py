...
def _formatInjection(inj):
    ...
    for stype, sdata in inj.data.items():
        ...
        if stype == PAYLOAD.TECHNIQUE.UNION:
            count = (
                re.sub(r"(?i)(\(.+\))|(\blimit[^a-z]+)", "", sdata.payload).count(",")
                + 1
            )
    ...
    if extractRegexResult(EMPTY_FORM_FIELDS_REGEX, value):
        ...
        if readInput(message, default="Y", boolean=True):
            for match in re.finditer(EMPTY_FORM_FIELDS_REGEX, retVal):
                ...
                if not any(_ in item for _ in IGNORE_PARAMETERS) and not re.search(
                    ASP_NET_CONTROL_REGEX, item
                ):
                    newValue = (
                        randomStr()
                        if not re.search(r"^id|id$", item, re.I)
                        else randomInt()
                    )
                    if item[-1] == DEFAULT_GET_POST_DELIMITER:
                        retVal = retVal.replace(
                            item,
                            "%s%s%s"
                            % (item[:-1], newValue, DEFAULT_GET_POST_DELIMITER),
                        )
                    else:
...
def _saveToHashDB():
    ...
    injections.extend(
        _
        for _ in kb.injections
        if _ and _.place is not None and _.parameter is not None
    )
    ...
    hashDBWrite(
        ...
        kb.absFilePaths | (_ if isinstance(_, set) else set()),
        True,
    )
...
def _saveToResultsFile():
    ...
    try:
        for key, value in results.items():
            ...
            line = "%s,%s,%s,%s,%s%s" % (
                safeCSValue(kb.originalUrls.get(conf.url) or conf.url),
                place,
                parameter,
                "".join(techniques[_][0].upper() for _ in sorted(value)),
                notes,
                os.linesep,
            )
        ...
        errMsg = "unable to write to the results file '%s' ('%s'). " % (
            conf.resultsFile,
            getSafeExString(ex),
        )
...
def start():
    ...
    stability and all GET, POST, Cookie and User-Agent parameters to check if
    they are dynamic and SQL injection affected."""
        ...
        try:
            ...
            if conf.data:
                ...
                conf.data = re.sub(
                    r"\b(__\w+)=([^&]+)",
                    lambda match: "%s=%s"
                    % (match.group(1), urlencode(match.group(2), safe="%")),
                    conf.data,
                )
            ...
            conf.httpHeaders = [
                ...
                if conf.httpHeaders[i][0].upper()
                not in (__[0].upper() for __ in conf.httpHeaders[i + 1 :])
            ]
            ...
            if conf.multipleTargets:
                if conf.forms and conf.method:
                    message = "[%d/%s] Form:\n%s %s" % (
                        targetCount,
                        len(kb.targets) if isListLike(kb.targets) else "?",
                        conf.method,
                        targetUrl,
                    )
                else:
                ...
                if conf.data is not None:
                    message += "\n%s data: %s" % (
                        (conf.method if conf.method != HTTPMETHOD.GET else None)
                        or HTTPMETHOD.POST,
                        (
                            urlencode(conf.data or "")
                            if re.search(r"\A\s*[<{]", conf.data or "") is None
                            else conf.data
                        ),
                    )
                    ...
                    if choice == "N":
                        ...
                        if conf.method != HTTPMETHOD.GET:
                            message = "Edit %s data [default: %s]%s: " % (
                                ...
                                (
                                    urlencode(conf.data or "")
                                    if re.search(r"\A\s*[<{]", conf.data or "None")
                                    is None
                                    else conf.data
                                ),
                                (
                            ...
                            conf.data = (
                                urldecode(conf.data)
                                if conf.data
                                and urlencode(DEFAULT_GET_POST_DELIMITER, None)
                                not in conf.data
                                else conf.data
                            )
            ...
            if conf.rParam and kb.originalPage:
                ...
                for match in re.finditer(
                    ...
                    name, _ = match.groups()
                    options = tuple(
                        re.findall(r"<option[^>]+\bvalue\s*=\s*[\"']([^\"']+)", _)
                    )
                    if options:
            ...
            if (
                len(kb.injections) == 0
                or (len(kb.injections) == 1 and kb.injections[0].place is None)
            ) and (kb.injection.place is None or kb.injection.parameter is None):
                if (
                    not any((conf.string, conf.notString, conf.regexp))
                ...
                for place in parameters:
                    ...
                    # --level >= 3
                    skip = place == PLACE.USER_AGENT and (
                        kb.testOnlyCustom or conf.level < 3
                    )
                    skip |= place == PLACE.REFERER and (
                        kb.testOnlyCustom or conf.level < 3
                    )

                    # --param-filter
                    skip |= (
                        len(conf.paramFilter) > 0
                        and place.upper() not in conf.paramFilter
                    )

                    # Test Host header only if
                    # --level >= 5
                    skip |= place == PLACE.HOST and (
                        kb.testOnlyCustom or conf.level < 5
                    )

                    # Test Cookie header only if --level >= 2
                    skip |= place == PLACE.COOKIE and (
                        kb.testOnlyCustom or conf.level < 2
                    )

                    skip |= place == PLACE.USER_AGENT and intersect(
                        USER_AGENT_ALIASES, conf.skip, True
                    ) not in ([], None)
                    skip |= place == PLACE.REFERER and intersect(
                        REFERER_ALIASES, conf.skip, True
                    ) not in ([], None)
                    skip |= place == PLACE.COOKIE and intersect(
                        PLACE.COOKIE, conf.skip, True
                    ) not in ([], None)
                    skip |= place == PLACE.HOST and intersect(
                        PLACE.HOST, conf.skip, True
                    ) not in ([], None)

                    skip &= not (
                        place == PLACE.USER_AGENT
                        and intersect(USER_AGENT_ALIASES, conf.testParameter, True)
                    )
                    skip &= not (
                        place == PLACE.REFERER
                        and intersect(REFERER_ALIASES, conf.testParameter, True)
                    )
                    skip &= not (
                        place == PLACE.HOST
                        and intersect(HOST_ALIASES, conf.testParameter, True)
                    )
                    skip &= not (
                        place == PLACE.COOKIE
                        and intersect((PLACE.COOKIE,), conf.testParameter, True)
                    )
                    ...
                    for parameter, value in paramDict.items():
                        ...
                        if kb.processUserMarks:
                            if testSqlInj and place not in (
                                PLACE.CUSTOM_POST,
                                PLACE.CUSTOM_HEADER,
                            ):
                                if kb.processNonCustom is None:
                                    ...
                                    message += (
                                        "Do you want to process them too? [Y/n/q] "
                                    )
                                    choice = readInput(message, default="Y").upper()
                        ...
                        if paramKey in kb.testedParams:
                            ...
                            infoMsg = (
                                "skipping previously processed %sparameter '%s'"
                                % (
                            ...
                            _ in conf.testParameter
                            for _ in (parameter, removePostHintPrefix(parameter))
                        ):
                            pass
                            ...
                            parameter in conf.skip
                            or kb.postHint
                            and parameter.split(" ")[-1] in conf.skip
                        ):
                            testSqlInj = False
                            ...
                            infoMsg = (
                                "skipping anti-CSRF token parameter '%s'" % parameter
                            )
                            logger.info(infoMsg)
                        ...
                        # Ignore session-like parameters for --level < 4
                            ...
                            PAYLOAD.TECHNIQUE.BOOLEAN in conf.technique
                            or conf.skipStatic
                        ):
                            ...
                            if not check:
                                warnMsg = (
                                    "%sparameter '%s' does not appear to be dynamic"
                                    % (
                                        (
                                            "%s " % paramType
                                            if paramType != parameter
                        ...
                        if testSqlInj:
                            try:
                                ...
                                if check != HEURISTIC_TEST.POSITIVE:
                                    if conf.smart or (
                                        kb.ignoreCasted
                                        and check == HEURISTIC_TEST.CASTED
                                    ):
                                        infoMsg = "skipping %sparameter '%s'" % (
                                            (
                                                "%s " % paramType
                                ...
                                infoMsg = (
                                    "testing for SQL injection on %sparameter '%s'"
                                    % (
                                        (
                                            "%s " % paramType
                                            if paramType != parameter
                                ...
                                if getattr(injection, "place", None) is not None:
                                    if (
                                        NOTE.FALSE_POSITIVE_OR_UNEXPLOITABLE
                                        in injection.notes
                                    ):
                                ...
                                if not injectable:
                                    warnMsg = (
                                        "%sparameter '%s' does not seem to be"
                                        " injectable"
                                        % (
                                            (
                                                "%s " % paramType
            ...
            if len(kb.injections) == 0 or (
                ...
                if kb.vainRun and not conf.multipleTargets:
                    ...
                    if kb.originalPage:
                        ...
                        if (
                            not conf.forms
                            and re.search(r"<form", kb.originalPage) is not None
                        ):
                            advice.append("--forms")
                        if (
                            not conf.crawlDepth
                            and re.search(r"href=[\"']/?\w", kb.originalPage)
                            is not None
                        ):
                    ...
                    if conf.level < 5 or conf.risk < 3:
                        errMsg += (
                            " Try to increase values for '--level'/'--risk' options "
                        )
                    ...
                    if not conf.textOnly and kb.originalPage:
                        ...
                        if kb.dynamicMarkings:
                            errMsg += (
                                " You can give it a go with the switch '--text-only' "
                            )
                    ...
                    if conf.string:
                        ...
                        errMsg += (
                            "valid value for option '--string' as perhaps the string"
                            " you "
                        )
                        ...
                        errMsg += " Also, you can try to rerun by providing a "
                        errMsg += (
                            "valid value for option '--regexp' as perhaps the regular "
                        )
                    ...
                    if not conf.tamper:
                        errMsg += (
                            " If you suspect that there is some kind of protection"
                            " mechanism "
                        )
