...
class Format(object):
    ...
    def getOs(target, info):
        """Formats the back-end operating system fingerprint value and return
        its values formatted as a human readable string.
    ...
    def isDbms(dbms):
        if (
            ...
            singleTimeWarnMessage(
                "identified ('%s') and fingerprinted ('%s') DBMSes differ. If you"
                " experience problems in enumeration phase please rerun with"
                " '--flush-session'" % (Backend.getIdentifiedDbms(), Backend.getDbms())
            )
            ...
            if condition:
                ...
                if not conf.multipleTargets and not (
                    ...
                    if (
                        ...
                        warnMsg = (
                            "it appears that you have provided tainted parameter"
                            " values "
                        )
                        ...
                        message = (
                            "are you really sure that you want to continue (sqlmap"
                            " could have problems)? [y/N] "
                        )
                    ...
                    for regex in (
                        ...
                        if match:
                            try:
                                ...
                                if candidates:
                                    message = (
                                        "it appears that provided value for %sparameter"
                                        " '%s' "
                                        % (
                                    ...
                                    message += (
                                        "is JSON deserializable. Do you want to inject"
                                        " inside? [y/N] "
                                    )
...
def filePathToSafeString(filePath):
    """Returns string representation of a given filepath safe for a single
    filename usage.
...
def boldifyMessage(message, istty=None):
    """Sets ANSI bold marking on entire message if parts found in predefined
    BOLD_PATTERNS.
    ...
    if data:
        while True:
            ...
            try:
                with open(
                    retVal, "w+b"
                ) as f:  # has to stay as non-codecs because data is raw ASCII encoded data
...
def setTechnique(technique):
    """Thread-safe setting of currently used technique (Note: dealing with
    cases of per-thread technique switching)"""
...
def isDigit(value):
    """Checks if provided (string) value consists of digits (Note: Python's
    isdigit() is problematic)
...
def parsePasswordHash(password):
    """In case of Microsoft SQL Server password hash value is expanded to its
    components.
...
def parseTargetDirect():
    """Parse target dbms and set some attributes into the configuration
    singleton.
...
def parseTargetUrl():
    ...
    try:
        ...
        errMsg += (
            "Please be sure that you don't have any leftover characters (e.g. '[' or"
            " ']') "
        )
...
def expandAsteriskForColumns(expression):
    """If the user provided an asterisk rather than the column(s) name, sqlmap
    will retrieve the columns itself and reprocess the SQL query string
    (expression)"""
...
def parseFilePaths(page):
    """Detects (possible) absolute system paths inside the provided page
    content.
...
def getCharset(charsetType=None):
    """Returns list with integers representing characters of a given charset
    type appropriate for inference techniques.
...
def safeFilepathEncode(filepath):
    """Returns filepath in (ASCII) format acceptable for OS handling (e.g.
    reading)
...
def getFilteredPageContent(page, onlyText=True, split=" "):
    """Returns filtered page content without script, style and/or comments or
    all HTML tags.
...
def posixToNtSlashes(filepath):
    """Replaces all occurrences of Posix slashes in provided filepath with NT
    backslashes.
...
def ntToPosixSlashes(filepath):
    """Replaces all occurrences of NT backslashes in provided filepath with
    Posix slashes.
...
def initCommonOutputs():
    """Initializes dictionary containing common output values used by "good
    samaritan" feature.
...
def wasLastResponseDBMSError():
    """Returns True if the last web request resulted in a (recognized) DBMS
    error page."""
...
def wasLastResponseHTTPError():
    """Returns True if the last web request resulted in an erroneous HTTP code
    (like 500)"""
...
def findLocalPort(ports):
    """Find the first opened localhost port from a given list of ports (e.g.
    for Tor port checks)"""
...
def isNumPosStrValue(value):
    """Returns True if value is a string (or integer) with a positive integer
    representation.
...
def parseSqliteTableSchema(value):
    ...
    if value:
        ...
        for match in re.finditer(
            (
                r"(?:\A|,)\s*(([\"'`]).+?\2|\w+)(?:\s+(INT|INTEGER|TINYINT|SMALLINT|MEDIUMINT|BIGINT|UNSIGNED"
                r" BIG INT|INT2|INT8|INTEGER|CHARACTER|VARCHAR|VARYING"
                r" CHARACTER|NCHAR|NATIVE"
                r" CHARACTER|NVARCHAR|TEXT|CLOB|LONGTEXT|BLOB|NONE|REAL|DOUBLE|DOUBLE"
                r" PRECISION|FLOAT|REAL|NUMERIC|DECIMAL|BOOLEAN|DATE|DATETIME|NUMERIC)\b)?"
            ),
...
def checkIntegrity():
    ...
    baseTime = (
        os.path.getmtime(paths.SQLMAP_SETTINGS_PATH) + 3600
...
def unhandledExceptionMessage():
    ...
    errMsg += (
        "reproduce the bug. Developers will try to reproduce the bug, fix it"
        " accordingly "
    )
...
def createGithubIssue(errMsg, excMsg):
    """Automatically create a Github issue with unhandled exception
    information."""
...
def maskSensitiveData(msg):
    ...
    for match in re.finditer(
        (
            r"(?im)[ -]-(u|url|data|cookie|auth-\w+|proxy|host|referer|headers?|H)("
            r" |=)(.*?)(?= -?-[a-z]|$)"
        ),
        retVal,
    ):
            ...
            if regex != payload:
                if all(
                    ...
                    if thread.is_alive():
                        ...
                        if not suppressWarning:
                            debugMsg = (
                                "turning off reflection removal mechanism (because of"
                                " timeouts)"
                            )
                            logger.debug(debugMsg)
                    else:
                ...
                if retVal != content:
                    ...
                    if re.search(
                        ...
                        warnMsg = (
                            "frames detected containing attacked parameter values."
                            " Please be sure to "
                        )
                        warnMsg += (
                            "test those separately in case that attack on this page"
                            " fails"
                        )
                        singleTimeWarnMessage(warnMsg)
                    ...
                    if (
                        ...
                        if not suppressWarning:
                            debugMsg = (
                                "turning off reflection removal mechanism (for"
...
def safeSQLIdentificatorNaming(name, isTable=False):
    """Returns a safe representation of SQL identificator name (internal data
    format)
    ...
    if isinstance(name, six.string_types):
        ...
        if (
            ...
            if (
                conf.db or ""
            ).lower() != "information_schema":  # NOTE: https://github.com/sqlmapproject/sqlmap/issues/5192
...
def randomizeParameterValue(value):
    """Randomize a parameter value based on occurrences of alphanumeric
    characters.
...
def findPageForms(content, url, raise_=False, addToTargets=False):
    """Parses given page content for possible forms (Note: still not
    ...
    def _(value):
        ...
        if value and isinstance(value, six.string_types):
            ...
            if len(value) % 2 != 0:
                ...
                singleTimeWarnMessage(
                    "there was a problem decoding value '%s' from expected hexadecimal"
                    " form" % value
                )
            else:
...
def decloakToTemp(filename):
    """Decloaks content of a given file to a temporary file with similar name
    and extension.
...
def prioritySortColumns(columns):
    """Sorts given column names by length in ascending order while those
    containing string 'id' go first.
...
def pollProcess(process, suppress_errors=False):
    """Checks for process status (prints .
...
def safeVariableNaming(value):
    """Returns escaped safe-representation of a given variable name that can be
    used in Python evaluated code.
...
def chunkSplitPostData(data):
    """Convert POST data to chunked transfer-encoded data (Note: splitting done
    by SQL keywords)
