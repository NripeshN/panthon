...
def _setMultipleTargets():
    ...
    if checkFile(conf.logFile, False):
        for target in parseRequestFile(conf.logFile):
            ...
            key = re.sub(
                r"(\w+=)[^%s ]*" % (conf.paramDel or DEFAULT_GET_POST_DELIMITER),
                r"\g<1>",
                "%s %s" % (url, data),
            )
            if key not in seen:
...
def _setRequestFromFile():
    """This function checks if the way to make a HTTP request is through
    supplied textual file, parses it and saves the information into the
    knowledge base."""
...
def _doSearch():
    """This function performs search dorking, parses results and saves the
    testable hosts into the knowledge base."""
    ...
    def retrieve():
        ...
        for link in links:
            ...
            if re.search(r"(.*?)\?(.+)", link) or conf.forms:
                ...
                if (
                    kb.data.onlyGETs is None
                    and conf.data is None
                    and not conf.googleDork
                ):
                    message = (
                        "do you want to scan only results containing GET parameters?"
                        " [Y/n] "
                    )
...
def _setBulkMultipleTargets():
    ...
    for line in getFileItems(conf.bulkFile):
        ...
        if (
            re.match(r"[^ ]+\?(.+)", line, re.I)
            or kb.customInjectionMark in line
            or conf.data
        ):
            found = True
            ...
            try:
                ...
                if conf.verbose in (1, 2):
                    status = "%d/%d links visited (%d%%)" % (
                        i + 1,
                        len(targets),
                        round(100.0 * (i + 1) / len(targets)),
                    )
                    dataToStdout(
                        "\r[%s] [INFO] %s" % (time.strftime("%X"), status), True
                    )
    ...
    if conf.msfPath:
        for path in (conf.msfPath, os.path.join(conf.msfPath, "bin")):
            if any(
                os.path.exists(
                    normalizePath(
                        os.path.join(path, "%s%s" % (_, ".bat" if IS_WIN else ""))
                    )
                )
    ...
    if not msfEnvPathExists:
        ...
        for envPath in envPaths:
            ...
            if any(
                os.path.exists(
                    normalizePath(
                        os.path.join(envPath, "%s%s" % (_, ".bat" if IS_WIN else ""))
                    )
                )
                for _ in ("msfcli", "msfconsole")
            ):
                ...
                if all(
                    os.path.exists(
                        normalizePath(
                            os.path.join(
                                envPath, "%s%s" % (_, ".bat" if IS_WIN else "")
                            )
                        )
                    )
                    for _ in ("msfvenom",)
...
def _setTechnique():
    validTechniques = sorted(
        getPublicTypeMembers(PAYLOAD.TECHNIQUE), key=lambda x: x[1]
    )
...
def _setTamperingFunctions():
    ...
    if conf.tamper:
        ...
        for script in re.split(PARAMETER_SPLITTING_REGEX, conf.tamper):
            ...
            try:
                if not script:
                    ...
                    os.path.join(
                        path, script if script.endswith(".py") else "%s.py" % script
                    )
                ):
                    script = os.path.join(
                        path, script if script.endswith(".py") else "%s.py" % script
                    ...
                    errMsg = (
                        "tamper script '%s' should have an extension '.py'" % script
                    )
                    raise SqlmapSyntaxException(errMsg)
            ...
            try:
                ...
                raise SqlmapSyntaxException(
                    "cannot import tamper module '%s' (%s)"
                    % (getUnicode(filename[:-3]), getSafeExString(ex))
                )

            priority = (
                PRIORITY.NORMAL
                if not hasattr(module, "__priority__")
                else module.__priority__
            )
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                if name == "tamper" and (
                    ...
                    and all(
                        _ in inspect.signature(function).parameters
                        for _ in ("payload", "kwargs")
                    )
                    or inspect.getargspec(function).args
                    and inspect.getargspec(function).keywords == "kwargs"
                ):
                    found = True
                    ...
                    try:
                        ...
                        errMsg += "for tamper module '%s' ('%s')" % (
                            getUnicode(filename[:-3]),
                            getSafeExString(ex),
                        )
                        raise SqlmapGenericException(errMsg)
        ...
        if resolve_priorities and priorities:
            priorities.sort(
                key=functools.cmp_to_key(lambda a, b: cmp(a[0], b[0])), reverse=True
            )
            kb.tamperFunctions = []
        ...
        for script in re.split(PARAMETER_SPLITTING_REGEX, conf.preprocess):
            ...
            try:
                ...
                if not os.path.exists(script):
                    ...
                    errMsg = (
                        "preprocess script '%s' should have an extension '.py'" % script
                    )
                    raise SqlmapSyntaxException(errMsg)
            except UnicodeDecodeError:
            ...
            try:
                ...
                raise SqlmapSyntaxException(
                    "cannot import preprocess module '%s' (%s)"
                    % (getUnicode(filename[:-3]), getSafeExString(ex))
                )
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                try:
                    if (
                        name == "preprocess"
                        and inspect.getargspec(function).args
                        and all(
                    ...
                    ValueError
                ):  # Note: https://github.com/sqlmapproject/sqlmap/issues/4357
                    pass
            ...
            if not found:
                ...
                try:
                    ...
                    handle, filename = tempfile.mkstemp(
                        prefix=MKSTEMP_PREFIX.PREPROCESS, suffix=".py"
                    )
                    os.close(handle)
                    ...
                    openFile(
                        os.path.join(os.path.dirname(filename), "__init__.py"), "w+b"
                    ).write("pass")
        ...
        for script in re.split(PARAMETER_SPLITTING_REGEX, conf.postprocess):
            ...
            try:
                ...
                if not os.path.exists(script):
                    ...
                    errMsg = (
                        "postprocess script '%s' should have an extension '.py'"
                        % script
                    )
                    raise SqlmapSyntaxException(errMsg)
            except UnicodeDecodeError:
            ...
            try:
                ...
                raise SqlmapSyntaxException(
                    "cannot import postprocess module '%s' (%s)"
                    % (getUnicode(filename[:-3]), getSafeExString(ex))
                )
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                if (
                    ...
                    and inspect.getargspec(function).args
                    and all(
                        _ in inspect.getargspec(function).args
                        for _ in ("page", "headers", "code")
                    )
                ):
                    found = True
            ...
            if not found:
                errMsg = (
                    "missing function 'postprocess(page, headers=None, code=None)' "
                )
                ...
                try:
                    ...
                    openFile(filename, "w+b").write(
                        "#!/usr/bin/env\n\ndef postprocess(page, headers=None,"
...
def _setDNSCache():
    """Makes a cached version of socket._getaddrinfo to avoid subsequent DNS
    requests."""
    ...
    with kb.locks.handlers:
        if conf.proxyList:
            ...
            if len(conf.proxyList) > 1:
                infoMsg = (
                    "loading proxy '%s' from a supplied proxy list file" % conf.proxy
                )
                logger.info(infoMsg)
        ...
        if conf.proxy:
            ...
            if not all((scheme, hasattr(PROXY_TYPE, scheme), hostname, port)):
                errMsg = (
                    "proxy value must be in format '(%s)://address:port'"
                    % "|".join(_[0].lower() for _ in getPublicTypeMembers(PROXY_TYPE))
                )
                raise SqlmapSyntaxException(errMsg)
            ...
            if scheme in (PROXY_TYPE.SOCKS4, PROXY_TYPE.SOCKS5):
                ...
                if scheme == PROXY_TYPE.SOCKS4:
                    warnMsg = (
                        "SOCKS4 does not support resolving (DNS) names (i.e. causing"
                        " DNS leakage)"
                    )
...
def _setSafeVisit():
    ...
    if (conf.safeFreq or 0) <= 0:
        errMsg = (
            "please provide a valid value (>0) for safe frequency ('--safe-freq') while"
            " using safe visit features"
        )
        raise SqlmapSyntaxException(errMsg)
...
def _setAuthCred():
    """Adds authentication credentials (if any) for current target to the
    password manager (used by connection handler)"""
    ...
    if kb.passwordMgr and all(
        ...
        kb.passwordMgr.add_password(
            None,
            "%s://%s:%d" % (conf.scheme, conf.hostname, conf.port),
            conf.authUsername,
            conf.authPassword,
        )
...
def _setHTTPAuthentication():
    """Check and set the HTTP(s) authentication method (Basic, Digest, Bearer,
    NTLM or PKI), username and password for first three methods, or PEM private
    key file for PKI authentication."""
    ...
    elif (conf.authType or "").lower() not in (
        AUTH_TYPE.BASIC,
        AUTH_TYPE.DIGEST,
        AUTH_TYPE.BEARER,
        AUTH_TYPE.NTLM,
    ...
    if not conf.authFile:
        ...
        if authType in (AUTH_TYPE.BASIC, AUTH_TYPE.DIGEST):
            ...
            conf.httpHeaders.append(
                (HTTP_HEADER.AUTHORIZATION, "Bearer %s" % conf.authCred.strip())
            )
            return
...
def _setHTTPExtraHeaders():
    if conf.headers:
        ...
        if conf.encoding:
            conf.httpHeaders.append(
                (HTTP_HEADER.ACCEPT_CHARSET, "%s;q=0.7,*;q=0.1" % conf.encoding)
            )
...
def _setHTTPUserAgent():
    """Set the HTTP User-Agent header. Depending on the user options it can be:
...
def _setHTTPTimeout():
    ...
    try:
        ...
        raise SqlmapValueException(
            "invalid value used for option '--timeout' ('%s')" % getSafeExString(ex)
        )
...
def _createHomeDirectories():
    ...
    for context in ("output", "history"):
        directory = paths[
            "SQLMAP_%s_PATH" % getUnicode(context).upper()
        ]  # NOTE: https://github.com/sqlmapproject/sqlmap/issues/4363
        try:
            ...
            warnMsg = "unable to %s %s directory " % (
                "create" if not os.path.isdir(directory) else "write to the",
                context,
            )
...
def _createTemporaryDirectory():
    ...
    if conf.tmpDir:
        ...
        try:
            ...
            warnMsg += (
                "system's temporary directory location(s) ('%s'). Please "
                % getSafeExString(ex)
            )
            warnMsg += (
                "make sure that there is enough disk space left. If problem persists, "
            )
    ...
    if (
        "sqlmap" not in (tempfile.tempdir or "")
        or conf.tmpDir
        and tempfile.tempdir == conf.tmpDir
    ):
        try:
            ...
            tempfile.tempdir = os.path.join(
                paths.SQLMAP_HOME_PATH,
                "tmp",
                "sqlmap%s%d" % (randomStr(6), os.getpid()),
            )
...
def _cleanupOptions():
    ...
    if conf.testParameter:
        ...
        conf.testParameter = [
            _.strip() for _ in re.split(PARAMETER_SPLITTING_REGEX, conf.testParameter)
        ]
    else:
        conf.testParameter = []
    ...
    if conf.ignoreCode:
        if conf.ignoreCode == IGNORE_CODE_WILDCARD:
            ...
            try:
                conf.ignoreCode = [
                    int(_) for _ in re.split(PARAMETER_SPLITTING_REGEX, conf.ignoreCode)
                ]
            except ValueError:
                errMsg = (
                    "option '--ignore-code' should contain a list of integer values or"
                    " a wildcard value '%s'" % IGNORE_CODE_WILDCARD
                )
                raise SqlmapSyntaxException(errMsg)
    else:
        conf.ignoreCode = []
    ...
    if conf.paramFilter:
        conf.paramFilter = [
            _.strip()
            for _ in re.split(PARAMETER_SPLITTING_REGEX, conf.paramFilter.upper())
        ]
    else:
        conf.paramFilter = []
        ...
        if all(_ in conf.rParam for _ in ("=", ",")):
            ...
            for part in original.split(";"):
                if "=" in part:
                    ...
                    kb.randomPool[left] = filterNone(
                        _.strip() for _ in right.split(",")
                    )
                else:
    ...
    if conf.csrfToken:
        ...
        try:
            ...
            if re.escape(conf.csrfToken) != conf.csrfToken:
                message = (
                    "provided value for option '--csrf-token' is a regular expression?"
                    " [y/N] "
                )
    ...
    if conf.exclude:
        ...
        if not regex:
            ...
            conf.exclude = r"\A%s\Z" % "|".join(
                re.escape(_) for _ in conf.exclude.split(",")
            )
        else:
...
def _setKnowledgeBaseAttributes(flushAll=True):
    ...
    kb.chars.start = "%s%s%s" % (
        ...
        randomStr(length=3, alphabet=KB_CHARS_LOW_FREQUENCY_ALPHABET),
        KB_CHARS_BOUNDARY_CHAR,
    )
    kb.chars.stop = "%s%s%s" % (
        KB_CHARS_BOUNDARY_CHAR,
    ...
    kb.chars.at, kb.chars.space, kb.chars.dollar, kb.chars.hash_ = (
        ...
        for _ in randomStr(length=4, lowercase=True)
    )
...
def _saveConfig():
    ...
    infoMsg = (
        "saved command line options to the configuration file '%s'" % conf.saveConfig
    )
    logger.info(infoMsg)
...
def _setTorHttpProxySettings():
    ...
    port = findLocalPort(
        DEFAULT_TOR_HTTP_PORTS if not conf.torPort else (conf.torPort,)
    )
...
def _setTorSocksProxySettings():
    ...
    port = findLocalPort(
        DEFAULT_TOR_SOCKS_PORTS if not conf.torPort else (conf.torPort,)
    )
    ...
    socks.setdefaultproxy(
        (
            socks.PROXY_TYPE_SOCKS5
            if conf.torType == PROXY_TYPE.SOCKS5
            else socks.PROXY_TYPE_SOCKS4
        ),
        LOCALHOST,
        port,
...
def _setHttpChunked():
    if conf.chunked and conf.data:
        if hasattr(_http_client.HTTPConnection, "_set_content_length"):
            _http_client.HTTPConnection._set_content_length = (
            ...
            if not hasattr(_http_client.HTTPConnection, "_putheader"):
                _http_client.HTTPConnection._putheader = (
...
def _checkTor():
    ...
    if not page or "Congratulations" not in page:
        errMsg = (
            "it appears that Tor is not properly set. Please try using options"
            " '--tor-type' and/or '--tor-port'"
        )
        raise SqlmapConnectionException(errMsg)
    else:
...
def _basicOptionValidation():
    if conf.limitStart is not None and not (
        isinstance(conf.limitStart, int) and conf.limitStart > 0
    ):
        errMsg = (
            "value for option '--start' (limitStart) must be an integer value greater"
            " than zero (>0)"
        )
        raise SqlmapSyntaxException(errMsg)

    if conf.limitStop is not None and not (
        isinstance(conf.limitStop, int) and conf.limitStop > 0
    ):
        errMsg = (
            "value for option '--stop' (limitStop) must be an integer value greater"
            " than zero (>0)"
        )
        raise SqlmapSyntaxException(errMsg)

    if conf.level is not None and not (
        isinstance(conf.level, int) and conf.level >= 1 and conf.level <= 5
    ):
        errMsg = "value for option '--level' must be an integer value from range [1, 5]"
        raise SqlmapSyntaxException(errMsg)

    if conf.risk is not None and not (
        isinstance(conf.risk, int) and conf.risk >= 1 and conf.risk <= 3
    ):
    ...
    if (
        ...
        and isinstance(conf.limitStop, int)
        and conf.limitStop < conf.limitStart
    ):
        warnMsg = (
            "usage of option '--start' (limitStart) which is bigger than value for"
            " --stop (limitStop) option is considered unstable"
        )
        logger.warning(warnMsg)

    if (
        isinstance(conf.firstChar, int)
        and conf.firstChar > 0
        and isinstance(conf.lastChar, int)
        and conf.lastChar < conf.firstChar
    ):
        errMsg = (
            "value for option '--first' (firstChar) must be smaller than or equal to"
            " value for --last (lastChar) option"
        )
        raise SqlmapSyntaxException(errMsg)

    if conf.proxyFile and not any(
        (conf.randomAgent, conf.mobile, conf.agent, conf.requestFile)
    ):
    ...
    if conf.chunked and not any((conf.data, conf.requestFile, conf.forms)):
        errMsg = (
            "switch '--chunked' requires usage of (POST) options/switches '--data',"
            " '-r' or '--forms'"
        )
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.crawlDepth:
        try:
            ...
            errMsg = "invalid value used for option '--crawl' ('%s')" % getSafeExString(
                ex
            )
            raise SqlmapSyntaxException(errMsg)
    ...
    if conf.paramExclude:
        if re.search(r"\A\w+,", conf.paramExclude):
            conf.paramExclude = r"\A(%s)\Z" % "|".join(
                re.escape(_).strip() for _ in conf.paramExclude.split(",")
            )
    ...
    if conf.predictOutput and (conf.threads > 1 or conf.optimize):
        errMsg = (
            "switch '--predict-output' is incompatible with option '--threads' and"
            " switch '-o'"
        )
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.threads > MAX_NUMBER_OF_THREADS and not conf.get("skipThreadCheck"):
        errMsg = (
            "maximum number of used threads is %d avoiding potential connection issues"
    ...
    if conf.forms and not any((conf.url, conf.googleDork, conf.bulkFile)):
        errMsg = (
            "switch '--forms' requires usage of option '-u' ('--url'), '-g' or '-m'"
        )
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.safeFreq and not any((conf.safeUrl, conf.safeReqFile)):
        errMsg = (
            "option '--safe-freq' requires usage of option '--safe-url' or '--safe-req'"
        )
        raise SqlmapSyntaxException(errMsg)

    if conf.safeReqFile and any((conf.safeUrl, conf.safePost)):
        errMsg = (
            "option '--safe-req' is incompatible with option '--safe-url' and option"
            " '--safe-post'"
        )
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.checkTor and not any((conf.tor, conf.proxy)):
        errMsg = (
            "switch '--check-tor' requires usage of switch '--tor' (or option '--proxy'"
            " with HTTP proxy address of Tor service)"
        )
        raise SqlmapSyntaxException(errMsg)

    if conf.torPort is not None and not (
        isinstance(conf.torPort, int) and conf.torPort >= 0 and conf.torPort <= 65535
    ):
    ...
    if conf.dumpFormat not in getPublicTypeMembers(DUMP_FORMAT, True):
        errMsg = (
            "option '--dump-format' accepts one of following values: %s"
            % ", ".join(getPublicTypeMembers(DUMP_FORMAT, True))
        )
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.uChar and not re.match(UNION_CHAR_REGEX, conf.uChar):
        errMsg = (
            "value for option '--union-char' must be an alpha-numeric value (e.g. 1)"
        )
        raise SqlmapSyntaxException(errMsg)
