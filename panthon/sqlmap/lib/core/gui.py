...
def runGui(parser):
    ...
    settings = {
        "TNotebook.Tab": {
            "configure": {"padding": [5, 1], "background": "#fdd57e"},
            "map": {
    ...
    helpmenu.add_command(
        label="Github pages", command=lambda: webbrowser.open(GIT_PAGE)
    )
    helpmenu.add_command(label="Wiki pages", command=lambda: webbrowser.open(WIKI_PAGE))
    helpmenu.add_command(
        label="Report issue", command=lambda: webbrowser.open(ISSUES_PAGE)
    )
    helpmenu.add_separator()
    helpmenu.add_command(
        label="About",
        command=lambda: _tkinter_messagebox.showinfo(
            "About", "Copyright (c) 2006-2023\n\n    (%s)" % DEV_EMAIL_ADDRESS
        ),
    )
    ...
    for group in parser.option_groups:
        ...
        for option in group.option_list:
            _tkinter.Label(
                frame, text="%s " % parser.formatter._format_option_strings(option)
            ...
            _tkinter.Label(frame, text=" %s" % option.help).grid(
