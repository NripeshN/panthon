...
def base64pickle(value):
    ...
    try:
        ...
        try:
            ...
            retVal = encodeBase64(
                pickle.dumps(str(value), PICKLE_PROTOCOL), binary=False
            )
...
def htmlUnescape(value):
    ...
    if value and isinstance(value, six.string_types):
        ...
        try:
            retVal = re.sub(
                r"&#x([^ ;]+);", lambda match: _unichr(int(match.group(1), 16)), retVal
            )
        ...
        if INVALID_UNICODE_PRIVATE_AREA:
            if unsafe:
                for char in xrange(0xF0000, 0xF00FF + 1):
                    value = value.replace(
                        _unichr(char), "%s%02x" % (SAFE_HEX_MARKER, char - 0xF0000)
                    )
            ...
            if unsafe:
                retVal = re.sub(
                    r"%s([0-9a-f]{2})" % SAFE_HEX_MARKER,
                    lambda _: decodeHex(_.group(1)),
                    retVal,
                )
        else:
            try:
            ...
            if unsafe:
                retVal = re.sub(
                    b"\\\\x([0-9a-f]{2})", lambda _: decodeHex(_.group(1)), retVal
                )
...
def getText(value, encoding=None):
    """Returns textual value of a given value (Note: not necessary Unicode on
    Python2)
...
def stdoutEncode(value):
    """Returns binary representation of a given Unicode value safe for writing
    to stdout."""
