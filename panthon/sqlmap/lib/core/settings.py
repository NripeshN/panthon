...
VERSION_STRING = "sqlmap/%s#%s" % (
    (
        ...
        if VERSION.count(".") > 2 and VERSION.split(".")[-1] == "0"
        else VERSION
    ),
    TYPE,
)
...
PERMISSION_DENIED_REGEX = (
    r"(?P<result>(command|permission|access)\s*(was|is)?\s*denied)"
)

# Regular expression used in recognition of generic protection mechanisms
GENERIC_PROTECTION_REGEX = (
    r"(?i)\b(rejected|blocked|protection|incident|denied|detected|dangerous|firewall)\b"
)

# Regular expression used to detect errors in fuzz(y) UNION test
FUZZ_UNION_ERROR_REGEX = (
    r"(?i)data\s?type|comparable|compatible|conversion|converting|failed|error"
)
...
GOOGLE_CONSENT_COOKIE = "CONSENT=YES+shp.gws-%s-0-RC1.%s+FX+740" % (
    time.strftime("%Y%m%d"),
...
DBMS_ALIASES = (
    (DBMS.MSSQL, MSSQL_ALIASES),
    (DBMS.MYSQL, MYSQL_ALIASES),
    (DBMS.PGSQL, PGSQL_ALIASES),
    (DBMS.ORACLE, ORACLE_ALIASES),
    (DBMS.SQLITE, SQLITE_ALIASES),
    (DBMS.ACCESS, ACCESS_ALIASES),
    (DBMS.FIREBIRD, FIREBIRD_ALIASES),
    (DBMS.MAXDB, MAXDB_ALIASES),
    (DBMS.SYBASE, SYBASE_ALIASES),
    (DBMS.DB2, DB2_ALIASES),
    (DBMS.HSQLDB, HSQLDB_ALIASES),
    (DBMS.H2, H2_ALIASES),
    (DBMS.INFORMIX, INFORMIX_ALIASES),
    (DBMS.MONETDB, MONETDB_ALIASES),
    (DBMS.DERBY, DERBY_ALIASES),
    (DBMS.VERTICA, VERTICA_ALIASES),
    (DBMS.MCKOI, MCKOI_ALIASES),
    (DBMS.PRESTO, PRESTO_ALIASES),
    (DBMS.ALTIBASE, ALTIBASE_ALIASES),
    (DBMS.MIMERSQL, MIMERSQL_ALIASES),
    (DBMS.CLICKHOUSE, CLICKHOUSE_ALIASES),
    (DBMS.CRATEDB, CRATEDB_ALIASES),
    (DBMS.CUBRID, CUBRID_ALIASES),
    (DBMS.CACHE, CACHE_ALIASES),
    (DBMS.EXTREMEDB, EXTREMEDB_ALIASES),
    (DBMS.FRONTBASE, FRONTBASE_ALIASES),
    (DBMS.RAIMA, RAIMA_ALIASES),
    (DBMS.VIRTUOSO, VIRTUOSO_ALIASES),
)
...
META_REFRESH_REGEX = (
    r"(?i)<meta"
    r' http-equiv="?refresh"?[^>]+content="?[^">]+;\s*(url=)?["\']?(?P<result>[^\'">]+)'
)
...
BURP_XML_HISTORY_REGEX = (
    r'<port>(\d+)</port>.*?<request base64="true"><!\[CDATA\[([^]]+)'
)
...
LEGAL_DISCLAIMER = (
    "Usage of sqlmap for attacking targets without prior mutual consent is illegal. It"
    " is the end user's responsibility to obey all applicable local, state and federal"
...
IPS_WAF_CHECK_PAYLOAD = (
    "AND 1=1 UNION ALL SELECT 1,NULL,'<script>alert(\"XSS\")</script>',table_name FROM"
    " information_schema.tables WHERE 2>1--/**/; EXEC xp_cmdshell('cat"
    " ../../../etc/passwd')#"
)
...
HASHDB_MILESTONE_VALUE = (  # python -c 'import random, string; print "".join(random.sample(string.ascii_letters, 10))'
    "OdqjeUpBLc"
)
...
BANNER = re.sub(
    ...
    % random.sample(HEURISTIC_CHECK_ALPHABET, 1)[0],
    BANNER,
)
...
EVENTVALIDATION_REGEX = (
    r'(?i)(?P<name>__EVENTVALIDATION[^"]*)[^>]+value="(?P<result>[^"]+)'
)
...
ARRAY_LIKE_RECOGNITION_REGEX = r"(\A|%s)(\w+)\[\d*\]=.+%s\2\[\d*\]=" % (
    DEFAULT_GET_POST_DELIMITER,
    DEFAULT_GET_POST_DELIMITER,
)
...
def _reversible(ex):
    if INVALID_UNICODE_PRIVATE_AREA:
        return (
            "".join(
                _unichr(int("000f00%2x" % (_ if isinstance(_, int) else ord(_)), 16))
                for _ in ex.object[ex.start : ex.end]
            ),
            ex.end,
        )
    else:
        return (
            "".join(
                INVALID_UNICODE_CHAR_FORMAT % (_ if isinstance(_, int) else ord(_))
                for _ in ex.object[ex.start : ex.end]
            ),
            ex.end,
        )
