...
class DNSQuery(object):
    ...
    def response(self, resolution):
        ...
        if self._query:
            ...
            retVal += (
                self._raw[4:6] + self._raw[4:6] + b"\x00\x00\x00\x00"
            )  # Questions and Answers Counts
            retVal += self._raw[
                12 : (12 + self._raw[12:].find(b"\x00") + 5)
            ...
            retVal += b"".join(
                struct.pack("B", int(_)) for _ in resolution.split(".")
            )  # 4 bytes of IP
...
class DNSServer(object):
    """Used for making fake DNS resolution responses based on received raw
    request.
    ...
    def _check_localhost(self):
        ...
        try:
            ...
            if response and b"google" in response:
                raise socket.error(
                    "another DNS service already running on '0.0.0.0:53'"
                )
        ...
        with self._lock:
            for _ in self._requests:
                if (
                    prefix is None
                    and suffix is None
                    or re.search(b"%s\\..+\\.%s" % (prefix, suffix), _, re.I)
                ):
    ...
    def run(self):
        """Runs a DNSServer instance as a daemon thread (killed by program
        exit)"""
