        ...
        if hasattr(ssl, "SSLContext"):
            for protocol in (_ for _ in _protocols if _ >= ssl.PROTOCOL_TLSv1):
                try:
                    ...
                    if protocol not in _contexts:
                        ...
                        if getattr(self, "cert_file", None) and getattr(
                            self, "key_file", None
                        ):
                            _contexts[protocol].load_cert_chain(
                        ...
                        try:
                            ...
                            pass
                    result = _contexts[protocol].wrap_socket(
                        sock,
                        do_handshake_on_connect=True,
                        server_hostname=(
                            self.host
                            if re.search(r"\A[\d.]+\Z", self.host or "") is None
                            else None
                        ),
                    )
                    if result:
            ...
            for protocol in _protocols:
                try:
                    ...
                    _ = ssl.wrap_socket(
                        sock,
                        keyfile=getattr(self, "key_file"),
                        certfile=getattr(self, "cert_file"),
...
class HTTPSHandler(_urllib.request.HTTPSHandler):
    def https_open(self, req):
        return self.do_open(
            HTTPSConnection if ssl else _http_client.HTTPSConnection, req
        )
