...
def _re_flatten(p):
    """Turn all capturing groups in a regular expression pattern into non-
    capturing groups."""
    if "(" not in p:
        return p
    return re.sub(
...
class Router(object):
    ...
    efficiently match WSGI requests against a number of routes and return the
    first target that satisfies the request. The target may be anything,
    usually a string, ID or callable object. A route consists of a path-rule
    and a HTTP method.

    The path-rule is either a static path (e.g. `/contact`) or a dynamic
    path that contains wildcards (e.g. `/wiki/<page>`). The wildcard
    syntax and details on the matching order are described in
    docs:`routing`.
    """
    ...
    def add_filter(self, name, func):
        ...
        The provided function is called with the configuration string as
        parameter and must return a (regexp, to_python, to_url) tuple.
        The first element is a string, the last two are callables or
        None.
        """
    ...
    def match(self, environ):
        """Return a (target, url_args) tuple or raise
...
class Route(object):
    ...
    It is also responsible for turning an URL path rule into a regular
    ...
    @cached_property
    def call(self):
        """The route callback with all plugins applied.

        This property is created on demand and then cached to speed up
        subsequent requests.
        """
    ...
    def reset(self):
        ...
        The next time :attr:`call` is accessed, all plugins are re-
        applied.
        """
    ...
    def get_undecorated_callback(self):
        ...
        If the callback is a decorated function, try to recover the
        original function.
        """
        func = self.callback
    ...
    def get_callback_args(self):
        ...
        If the callback is a decorated function, try to recover the
        original function before inspection.
        """
...
class Bottle(object):
    ...
    def add_hook(self, name, func):
        ...
        before_request     Executed once before each request. The
        request context is     available, but no routing has happened
        yet. after_request     Executed once after each request
        regardless of its outcome. app_reset     Called whenever
        :meth:`Bottle.reset` is called.
        """
    ...
    def install(self, plugin):
        """Add a plugin to the list of plugins and prepare it for being applied
        to all routes of this application.

        A plugin may be a simple decorator or an object that implements
        the :class:`Plugin` API.
        """
    ...
    def reset(self, route=None):
        ...
        If an ID or route object is given, only that specific route is
        affected.
        """
        if route is None:
            routes = self.routes
    ...
    def match(self, environ):
        ...
        The second value is a dictionary with parameters extracted from
        the URL. Raise :exc:`HTTPError` (404/405) on a non-match.
        """
    ...
    def error(self, code=500, callback=None):
        """Register an output handler for a HTTP error code. Can be used as a
...
class BaseRequest(object):
    ...
    def cookies(self):
        ...
        Signed cookies are NOT decoded. Use :meth:`get_cookie` if you
        expect signed cookies.
        """
    ...
    def files(self):
        ...
        The values are instances of :class:`FileUpload`.
        """
        files = FormsDict()
    ...
    @property
    def url(self):
        """The full request URI including hostname and scheme.
    ...
    def urlparts(self):
        ...
        The tuple contains (scheme, host, path, query_string and
        fragment), but the fragment is always empty because it is not
    ...
    def script_name(self):
        ...
        level (server or routing middleware) before the application was called.

        This script path is returned with leading and tailing slashes.
        """
    ...
    def content_length(self):
        """The request body length as an integer.

        The client is responsible to set this header. Otherwise, the
        real length of the body is unknown and -1 is returned. In this
        case, :attr:`body` will be empty.
        """
    ...
    @property
    def is_xhr(self):
        """True if the request was triggered by a XMLHttpRequest.

        This only works with JavaScript libraries that support the
    ...
    @property
    def auth(self):
        """HTTP authentication data as a (user, password) tuple.
    ...
    def remote_route(self):
        ...
        the client IP and followed by zero or more proxies.
    ...
    def remote_addr(self):
        ...
        Note that this information can be forged by malicious clients.
        """
...
class BaseResponse(object):
    ...
    def get_header(self, name, default=None):
        """Return the value of a previously defined header.

        If there is no header with that name, return a default value.
        """
    ...
    def iter_headers(self):
        """Yield (header, value) tuples, skipping headers that are not allowed
        with the current response status code."""
    ...
    def set_cookie(self, name, value, secret=None, digestmod=hashlib.sha256, **options):
        """Create a new cookie or replace an old one. If the `secret` parameter
        is set, create a `Signed Cookie` (described below).
    ...
    def delete_cookie(self, key, **kwargs):
        ...
        Be sure to use the same `domain` and `path` settings as used to
        create the cookie.
        """
        kwargs["max_age"] = -1
        kwargs["expires"] = 0
...
class LocalRequest(BaseRequest):
    ...
    There is usually only one global instance of this class
    (:data:`request`). If accessed during a request/response cycle, this
    instance always refers to the *current* request (even on a
...
class LocalResponse(BaseResponse):
    ...
    There is usually only one global instance of this class
    (:data:`response`). Its attributes are used to build the HTTP
...
class TemplatePlugin(object):
    ...
    `template` config parameter.

    If the parameter is a tuple, the second element must be a dict with
    additional options (e.g. `template_engine`) or default variables for
    the template.
    """
...
class MultiDict(DictMixin):
    ...
    There are special methods available to access the full list of
    values.
    """
...
class FormsDict(MultiDict):
    ...
    Additionally to the normal dict-like item access methods (which
    return unmodified data as native strings), this container also
    supports attribute-like access to its values. Attributes are
    automatically de- or recoded to match :attr:`input_encoding`
...
class WSGIHeaderDict(DictMixin):
    ...
    3.x unicode) and keys are case-insensitive. If the WSGI environment
    contains non-native string values, these are de- or encoded using a
    lossless 'latin1' character set.

    The API will remain stable even on changes to the relevant PEPs.
    Currently PEP 333, 444 and 3333 are supported. (PEP 444 is the only
    one that uses non-native strings.)
    """
...
class ConfigDict(dict):
    ...
    This dict-like class is heavily optimized for read access. All read-
    only methods as well as item access should be as fast as the built-
    in dict.
    """
    ...
    def _set_virtual(self, key, value):
        ...
        Do nothing if non-virtual value is present.
        """
    ...
    def _delete_virtual(self, key):
        ...
        Do nothing if key is not virtual.
        """
    ...
    def _make_overlay(self):
        ...
        missing in the overlay are copied from the source map. Both maps share
        the same meta entries.

        Entries that were copied from the source are called 'virtual'.
        You can not delete virtual keys, but overwrite them, which turns
        them into non-virtual entries. Setting keys on an overlay never
        affects its source, but may affect any number of child overlays.

        Other than collections.ChainMap or most other implementations,
        this approach does not resolve missing keys on demand, but
        instead actively copies all values from the source to the
        overlay and keeps track of virtual and non-virtual keys
        internally. This removes any lookup-overhead. Read-access is as
        fast as a build-in dict for both virtual and non-virtual keys.
...
class AppStack(list):
    ...
    Calling it returns the head of the stack.
    """
...
class ResourceManager(object):
    ...
    def add_path(self, path, base=None, index=None, create=False):
        """Add a new path to the list of search paths. Return False if the path
        does not exist.
    ...
    def lookup(self, name):
        ...
        returned. Symlinks are followed. The result is cached to speed
        up future lookups.
        """
...
class FileUpload(object):
    ...
    def filename(self):
        ...
        or dashes are removed. The filename is limited to 255
        characters.
        """
    ...
    def save(self, destination, overwrite=False, chunk_size=2**16):
        ...
        If *destination* is a directory, :attr:`filename` is added to the path.
...
def redirect(url, code=None):
    """Aborts execution and causes a 303 or 302 redirect, depending on the HTTP
    protocol version."""
    if not code:
...
def parse_range_header(header, maxlen=0):
    """Yield (start, end) ranges parsed from a HTTP Range header.
...
def _parse_http_header(h):
    """Parses a typical multi-valued and parametrised HTTP header (e.g. Accept
    headers) and returns a list of values and parameters.
...
def yieldroutes(func):
    """Return a generator for routes that match the signature (name, args) of
    the func parameter. This may yield more than one route if the function
    takes optional keyword arguments. The output is best described by example::

    a()         -> '/a' b(x, y)     -> '/b/<x>/<y>' c(x, y=5)   ->
    '/c/<x>' and '/c/<x>/<y>' d(x=5, y=6) -> '/d' and '/d/<x>' and
    '/d/<x>/<y>'
    """
...
class FapwsServer(ServerAdapter):
    ...
    See https://github.com/william-os4y/fapws3
    """
...
class GunicornServer(ServerAdapter):
    ...
    See http://gunicorn.org/configure.html for options.
    """
...
class AiohttpServer(AsyncioServerAdapter):
    ...
    https://pypi.python.org/pypi/aiohttp/ https://pypi.org/project/aiohttp-
    wsgi/"""
...
def load_app(target):
    ...
    See :func:`load` for the target parameter.
    """
    global NORUN
...
class FileCheckerThread(threading.Thread):
    """Interrupt main-thread as soon as a changed module file is detected, the
    lockfile gets deleted or gets too old."""
...
class BaseTemplate(object):
    ...
    def __init__(
        ...
        If the source parameter (str or buffer) is missing, the name
        argument is used to guess a template filename. Subclasses can
        assume that self.source and/or self.filename are set. Both are
        strings. The lookup, encoding and settings parameters are stored
        as instance variables. The lookup parameter stores a list
        containing directory paths. The encoding parameter should be
        used to decode byte strings or files. The settings parameter
        contains a dict for engine-specific settings.
        """
        self.name = name
    ...
    def render(self, *args, **kwargs):
        """Render the template with the specified local variables and return a
        single byte or unicode string.

        If it is a byte string, the encoding must match self.encoding.
        This method must be thread-safe! Local variables may be provided
...
def template(*args, **kwargs):
    ...
    You can use a name, a filename or a template string as first
    parameter. Template rendering arguments can be passed as
    dictionaries or directly (as keyword arguments).
    """
