...
try:
    ...
    try:
        ...
        sys.exit(
            "[!] wrong installation detected (missing modules). Visit"
            " 'https://github.com/sqlmapproject/sqlmap/#installation' for further"
            " details"
        )
    ...
    if "--deprecations" not in sys.argv:
        ...
        warnings.filterwarnings(
            action="ignore", message="'crypt'", category=DeprecationWarning
        )
    ...
    warnings.filterwarnings(
        action="ignore", message=".*using a very old release", category=UserWarning
    )
    warnings.filterwarnings(
        action="ignore",
...
def main():
    ...
    try:
                ...
                if conf.profile:
                    ...
                    try:
                        if conf.crawlDepth and conf.bulkFile:
                            ...
                            for i in xrange(len(targets)):
                                ...
                                try:
                                    ...
                                    infoMsg = (
                                        "starting crawler for target URL '%s' (%d/%d)"
                                    ...
                                    if target and not isinstance(
                                        ...
                                        errMsg = (
                                            "problem occurred while crawling '%s'"
                                            " ('%s')" % (target, getSafeExString(ex))
                                        )
                                        logger.error(errMsg)
                                    else:
                                        raise
                        ...
                        if "can't start new thread" in getSafeExString(ex):
                            errMsg = (
                                "unable to start new threads. Please check OS (u)limits"
                            )
                            logger.critical(errMsg)
                            raise SystemExit
                        else:
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            _ in excMsg for _ in ("tempfile.mkdtemp", "tempfile.mkstemp", "tempfile.py")
        ):
            errMsg = (
                "unable to write to the temporary directory '%s'. "
                % tempfile.gettempdir()
            )
            ...
            errMsg = (
                "permission error occurred while accessing file '%s'" % match.group(1)
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            "invalid maximum character passed to PyUnicode_New" in excMsg
            and re.search(r"\A3\.[34]", sys.version) is not None
        ):
            ...
            _ in excMsg
            for _ in ("OSError: [Errno 22] Invalid argument: '", "importlib")
        ):
            errMsg = "unable to read file '%s'" % extractRegexResult(
        ...
        elif "AttributeError: unable to access item" in excMsg and re.search(
            r"3\.11\.\d+a", sys.version
        ):
            errMsg = (
                "there is a known issue when sqlmap is run with ALPHA versions of"
                " Python 3.11. "
            )
            ...
            errMsg = (
                "there has been a problem while running the multiprocessing hash"
                " cracking. "
            )
            ...
            errMsg = (
                "wrong initialization of 'python-ntlm' detected (using Python2 syntax)"
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg += (
                " (Reference:"
                " 'https://github.com/sqlmapproject/sqlmap/issues/4572#issuecomment-775041086')"
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = (
                "code integrity check failed (turning off automatic issue creation). "
            )
            errMsg += (
                "You should retrieve the latest development version from official"
                " GitHub "
            )
            ...
            _ in "%s\n%s" % (errMsg, excMsg)
            for _ in ("tamper/", "waf/", "--engagement-dojo")
        ):
            logger.critical(errMsg)
            print()
            dataToStdout(excMsg)
            ...
            for _ in (
                ...
                (
                    "<built-in function compile> returned NULL without setting an"
                    " exception"
                ),
            ...
            _ in excMsg for _ in ("PermissionError: [WinError 5]", "multiprocessing")
        ):
            errMsg = (
                "there is a permission problem in running multiprocessing on this"
                " system. "
            )
            ...
            errMsg = (
                "you are trying to run (hidden) development tests inside the production"
                " environment"
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            _ in excMsg
            for _ in ("HTTPNtlmAuthHandler", "'str' object has no attribute 'decode'")
        ):
            errMsg = "package 'python-ntlm' has a known compatibility issue with the "
            errMsg += (
                "Python 3 (Reference:"
                " 'https://github.com/mullender/python-ntlm/pull/61')"
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = (
                "local session file seems to be malformed. Please rerun with"
                " '--flush-session'"
            )
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg += (
                "(Reference: 'https://stackoverflow.com/a/38841364' &"
        ...
        if kb.get("tempDir"):
            ...
            if not filterNone(
                ...
                if not any(filepath.endswith(_) for _ in (".lock", ".exe", ".so", "_"))
            ):  # ignore junk files
                try:
