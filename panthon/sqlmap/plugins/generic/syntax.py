            ...
            for item in re.findall(r"'[^']*'+", expression):
                ...
                if original:
                    if Backend.isDbms(DBMS.SQLITE) and "X%s" % item in expression:
                        continue
                    if (
                        re.search(r"\[(SLEEPTIME|RAND)", original) is None
                    ):  # e.g. '[SLEEPTIME]' marker
                        replacement = (
                            escaper(original) if not conf.noEscape else original
                        )

                        if replacement != original:
                            retVal = retVal.replace(item, replacement)
                        elif (
                            len(original) != len(getBytes(original))
                            and "n'%s'" % original not in retVal
                            and Backend.getDbms()
                            in (DBMS.MYSQL, DBMS.PGSQL, DBMS.ORACLE, DBMS.MSSQL)
                        ):
                            retVal = retVal.replace(
                                "'%s'" % original, "n'%s'" % original
                            )
        else:
