        ...
        for tbl in tblList:
            ...
            try:
                ...
                if (
                    ...
                    warnMsg = (
                        "unable to enumerate the columns for table '%s'"
                    ...
                    if Backend.getIdentifiedDbms() in (
                        ...
                        if not (
                            ...
                            if Backend.isDbms(DBMS.MSSQL) and not conf.forcePivoting:
                                warnMsg = (
                                    "in case of table dumping problems (e.g. column"
                                    " entry order) "
                                )
                                warnMsg += (
                                    "you are advised to rerun with '--force-pivoting'"
                                )
                            ...
                            if isNoneValue(entries) and not kb.dumpKeyboardInterrupt:
                                ...
                                if retVal:
                                    entries, _ = retVal
                                    entries = BigArray(
                                        _zip(*[entries[colName] for colName in colList])
                                    )
                        else:
                ...
                if (
                    ...
                    if Backend.getIdentifiedDbms() in (
                        ...
                        query = (
                            rootQuery.blind.count % ("%s.%s" % (conf.db, tbl))
                            if conf.db
                            else tbl
                        )
                    ...
                    if count == 0:
                        ...
                        if Backend.isDbms(DBMS.MSSQL) and not conf.forcePivoting:
                            warnMsg = (
                                "in case of table dumping problems (e.g. column entry"
                                " order) "
                            )
                            warnMsg += (
                                "you are advised to rerun with '--force-pivoting'"
        ...
        if kb.data.cachedTables:
            ...
            for db, tables in kb.data.cachedTables.items():
                ...
                for table in tables:
                    if (
                        conf.exclude
                        and re.search(conf.exclude, table, re.I) is not None
                    ):
