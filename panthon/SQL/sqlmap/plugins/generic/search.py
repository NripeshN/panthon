    ...
    def searchTable(self):
        ...
        for tbl in tblList:
            ...
            if dbCond and conf.exclude:
                whereDbsQuery += " AND %s NOT LIKE '%s'" % (
                    dbCond,
                    re.sub(r"\.[*+]", "%", conf.exclude._original),
                )
                ...
                for db in foundTbls:
                    ...
                    for index in indexRange:
                        ...
                        if " ORDER BY " in query:
                            query = query.replace(
                                " ORDER BY ", "%s ORDER BY " % (" AND %s" % tblQuery)
                            )
        ...
        for column in colList:
            ...
            if tblCond:
                if conf.tbl:
                    ...
                    if conf.exclude:
                        tbls = [
                            _ for _ in tbls if re.search(conf.exclude, _, re.I) is None
                        ]
                    whereTblsQuery = (
                        " AND ("
                        + " OR ".join(
                    ...
                    infoMsgTbl = " for table%s '%s'" % (
                        "s" if len(tbls) > 1 else "",
                        ", ".join(unsafeSQLIdentificatorNaming(tbl) for tbl in tbls),
                    )
                    ...
                    for db in dbData:
                        ...
                        for index in indexRange:
                            ...
                            if re.search(r"(?i)%s\Z" % METADB_SUFFIX, db or ""):
                                ...
                                query = query.replace(
                                    " ORDER BY ",
                                    " AND %s ORDER BY " % (colQuery + whereTblsQuery),
                                )
                            else:
