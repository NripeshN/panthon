    ...
    def isDba(self, user=None):
        ...
        if Backend.isDbms(DBMS.MYSQL):
            ...
            if Backend.isDbms(DBMS.MYSQL) and Backend.isFork(FORK.DRIZZLE):
                ...
                query = (
                    queries[Backend.getIdentifiedDbms()].is_dba.query
            ...
            if Backend.isDbms(DBMS.SYBASE):
                ...
                if Backend.isDbms(DBMS.MSSQL) and isNoneValue(values):
                    values = inject.getValue(
                        query.replace(
                            "master.dbo.fn_varbintohexstr", "sys.fn_sqlvarbasetostr"
                        ),
                        blind=False,
                        time=False,
                    )
                elif Backend.isDbms(DBMS.MYSQL) and (
                    isNoneValue(values)
                    or all(
                        len(value) == 2
                        and (isNullValue(value[1]) or isNoneValue(value[1]))
                        for value in values
                    )
                ):
                    values = inject.getValue(
                        query.replace("authentication_string", "password"),
                        blind=False,
                        time=False,
                    )
                    ...
                    for count in xrange(0, len(value or [])):
                        ...
                        # The other columns are the privileges
                            ...
                            # of privileges as string
                                ...
                                or Backend.getIdentifiedDbms()
                                in (DBMS.VERTICA, DBMS.MIMERSQL, DBMS.CUBRID)
                            ):
