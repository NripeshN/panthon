...
void usage(char *path) {
  ...
  printf("  -d milliseconds    delay between requests in milliseconds (default "
         "is %u)\n",
         DEFAULT_DELAY);
  printf("  -o milliseconds    timeout in milliseconds\n");
  printf("  -h                 this screen\n");
  printf("  -b num             maximal number of blanks (unanswered icmp "
         "requests)\n");
  printf("                     before quitting\n");
  printf("  -s bytes           maximal data buffer size in bytes (default is "
         "%u bytes)\n\n",
         DEFAULT_MAX_DATA_SIZE);
  printf("In order to improve the speed, lower the delay (-d) between requests "
         "or\n");
