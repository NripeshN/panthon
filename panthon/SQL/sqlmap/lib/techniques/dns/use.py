    ...
    if conf.dnsDomain and Backend.getIdentifiedDbms() in (
        ...
        if output is None:
            ...
            while True:
                count += 1
                prefix, suffix = (
                    "%s" % randomStr(length=3, alphabet=DNS_BOUNDARIES_ALPHABET)
                    for _ in xrange(2)
                )
                chunk_length = (
                    MAX_DNS_LABEL // 2
                    if Backend.getIdentifiedDbms()
                    in (DBMS.ORACLE, DBMS.MYSQL, DBMS.PGSQL)
                    else MAX_DNS_LABEL // 4 - 2
                )
                _, _, _, _, _, _, fieldToCastStr, _ = agent.getFields(expression)
                nulledCastedField = agent.nullAndCastField(fieldToCastStr)
                extendedField = re.search(
                    r"[^ ,]*%s[^ ,]*" % re.escape(fieldToCastStr), expression
                ).group(0)
                ...
                Request.queryPage(
                    forgedPayload, content=False, noteResponseTime=False, raise404=False
                )
                ...
                if _:
                    _ = extractRegexResult(
                        r"%s\.(?P<result>.+)\.%s" % (prefix, suffix), _, re.I
                    )
        ...
        warnMsg += (
            "is currently not available for DBMS %s" % Backend.getIdentifiedDbms()
        )
