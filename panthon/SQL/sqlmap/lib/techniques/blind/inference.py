...
def bisection(
    ...
    """Bisection algorithm that can be used to perform blind SQL injection on
    an affected host."""
    ...
    timeBasedCompare = getTechnique() in (
    ...
    if Backend.isDbms(DBMS.MCKOI):
        ...
        match = re.search(
            r"\ASELECT\b(\s+TOP\s*\([^)]+\)\s+)?(.+)\bFROM\b(.+)\Z", expression, re.I
        )
        if match:
            payload = payload.replace(
                INFERENCE_GREATER_CHAR,
                " FROM %s)%s" % (match.group(3).strip(), INFERENCE_GREATER_CHAR),
            )
            payload = payload.replace(
                "SUBSTRING",
                "(SELECT%sSUBSTRING" % (match.group(1) if match.group(1) else " "),
                1,
            )
    ...
    try:
        ...
        if partialValue:
            ...
            or (hasattr(conf.firstChar, "isdigit") and conf.firstChar.isdigit())
        ):
            firstChar = int(conf.firstChar) - 1
            if kb.fileReadMode:
                firstChar <<= 1
        elif (
            hasattr(firstChar, "isdigit")
            and firstChar.isdigit()
            or isinstance(firstChar, int)
        ):
        ...
        if re.search(r"(?i)(\b|CHAR_)(LENGTH|LEN|COUNT)\(", expression):
            lastChar = 0
        elif conf.lastChar is not None and (
            isinstance(conf.lastChar, int)
            or (hasattr(conf.lastChar, "isdigit") and conf.lastChar.isdigit())
        ):
            lastChar = int(conf.lastChar)
        elif (
            hasattr(lastChar, "isdigit")
            and lastChar.isdigit()
            or isinstance(lastChar, int)
        ):
        ...
        if (
            isinstance(length, six.string_types)
            and isDigit(length)
        ...
        def tryHint(idx):
            ...
            if (
                payload is not None
                and len(hintValue or "") > 0
                and len(hintValue) >= idx
            ):
                ...
                ).replace(markingValue, unescapedCharValue)
                result = Request.queryPage(
                    agent.replacePayload(payload, forgedPayload),
        ...
        def validateChar(idx, value):
            ...
            validationPayload = re.sub(
                ...
                r"\g<1>%s\g<2>" % INFERENCE_NOT_EQUALS_CHAR,
                payload,
            )
            ...
            if "'%s'" % CHAR_INFERENCE_MARK not in payload:
                ...
                forgedPayload = safeStringFormat(
                    validationPayload, (expressionUnescaped, idx)
                ).replace(markingValue, unescapedCharValue)
            ...
            if result and timeBasedCompare and getTechniqueData().trueCode:
                ...
                if not result:
                    warnMsg = (
                        "detected HTTP code '%s' in validation phase is differing from"
                        " expected '%s'"
            ...
            if continuousOrder:
                while len(charTbl) > 1:
                        ...
                        if not firstCheck:
                            try:
                                try:
                                    lastChar = [
                                        _
                                        for _ in threadData.shared.value
                                        if _ is not None
                                    ][-1]
                                except IndexError:
                                    lastChar = None
                            ...
                            not lastCheck and numThreads == 1
                        ):  # not usable in multi-threading environment
                    ...
                    if "'%s'" % CHAR_INFERENCE_MARK not in payload:
                        ...
                        forgedPayload = safeStringFormat(
                            payload, (expressionUnescaped, idx)
                        ).replace(markingValue, unescapedCharValue)
                        falsePayload = safeStringFormat(
                            payload, (expressionUnescaped, idx)
                        ).replace(markingValue, NULL)
                    ...
                    if not timeBasedCompare and getTechniqueData() is not None:
                        ...
                        if unexpectedCode:
                            if threadData.lastCode is not None:
                                warnMsg = (
                                    "unexpected HTTP code '%s' detected."
                                    % threadData.lastCode
                                )
                            else:
                            ...
                            warnMsg += (
                                " Will use (extra) validation step in similar cases"
                            )
                    ...
                    if len(charTbl) == 1:
                        ...
                        # Going beyond the original charset
                            ...
                            if retVal in originalTbl or (
                                ...
                                if (
                                    timeBasedCompare or unexpectedCode
                                    ...
                                    if (retried or 0) < MAX_REVALIDATION_STEPS:
                                        ...
                                        if timeBasedCompare:
                                            if (
                                                kb.adjustTimeDelay
                                                is not ADJUST_TIME_DELAY.DISABLE
                                            ):
                                                conf.timeSec += 1
                                                warnMsg = (
                                                    "increasing time delay to %d"
                                                    " second%s"
                                                    % (
                                                        conf.timeSec,
                                            ...
                                            if (
                                                kb.adjustTimeDelay
                                                is ADJUST_TIME_DELAY.YES
                                            ):
                                                dbgMsg = (
                                                    "turning off time auto-adjustment"
                                                    " mechanism"
                                                )
                                                logger.debug(dbgMsg)
                                                kb.adjustTimeDelay = (
                                                    ADJUST_TIME_DELAY.NO
                                                )
                                        ...
                                        errMsg = (
                                            "unable to properly validate last character"
                                            " value ('%s').."
                                    ...
                                    if timeBasedCompare:
                                        ...
                                        if (
                                            kb.adjustTimeDelay is ADJUST_TIME_DELAY.NO
                                            and threadData.validationRun
                                            > VALID_TIME_CHARS_RUN_THRESHOLD
                                        ):
                                            dbgMsg = (
                                                "turning back on time auto-adjustment"
                                                " mechanism"
                                            )
                                            logger.debug(dbgMsg)
                        ...
                        if kb.threadContinue:
                            if showEta:
                                ...
                                for i in xrange(startCharIndex, endCharIndex + 1):
                                    output += (
                                        "_"
                                        if currentValue[i] is None
                                        else filterControlChars(
                                            (
                                                currentValue[i]
                                                if len(currentValue[i]) == 1
                                                else " "
                                            ),
                                            replacement=" ",
                                        )
                                ...
                                if (
                                    endCharIndex - startCharIndex == conf.progressWidth
                                ) and (endCharIndex < length - 1):
                                    output = output[:-2] + ".."
                                ...
                                if conf.verbose in (1, 2) and not any(
                                    ...
                                    output += (
                                        status if _ != length else " " * len(status)
                                    )
            ...
            if None in value:
                ...
                if partialValue:
                    infoMsg = "\r[%s] [INFO] partially retrieved: %s" % (
                        time.strftime("%X"),
                        filterControlChars(partialValue),
                    )
            else:
                finalValue = "".join(value)
                infoMsg = "\r[%s] [INFO] retrieved: %s" % (
                    time.strftime("%X"),
                    filterControlChars(finalValue),
                )

            if (
                conf.verbose in (1, 2)
                and infoMsg
                and not any((showEta, conf.api, kb.bruteMode))
            ):
        ...
        # No multi-threading (--threads = 1)
            ...
            while True:
                ...
                if (
                    conf.predictOutput
                    and len(partialValue) > 0
                    and kb.partRun is not None
                ):
                    val = None
                    commonValue, commonPattern, commonCharset, otherCharset = (
                        goGoodSamaritan(partialValue, asciiTbl)
                    )
                    ...
                    if commonValue is not None:
                        ...
                        testValue = (
                            unescaper.escape("'%s'" % commonValue)
                            if "'" not in commonValue
                            else unescaper.escape("%s" % commonValue, quote=False)
                        )
                        ...
                        if result:
                            if showEta:
                                ...
                                dataToStdout(
                                    filterControlChars(commonValue[index - 1 :])
                                )
                    ...
                    if commonPattern is not None:
                        ...
                        subquery = queries[
                            Backend.getIdentifiedDbms()
                        ].substring.query % (expressionUnescaped, 1, len(commonPattern))
                        testValue = (
                            unescaper.escape("'%s'" % commonPattern)
                            if "'" not in commonPattern
                            else unescaper.escape("%s" % commonPattern, quote=False)
                        )

                        query = getTechniqueData().vector
                        query = agent.prefixQuery(
                            query.replace(
                                INFERENCE_MARKER, "(%s)=%s" % (subquery, testValue)
                            )
                        )
                        query = agent.suffixQuery(query)
                    ...
                    val = getChar(
                        index, asciiTbl, not (charsetType is None and conf.charset)
                    )
                ...
                if (
                    Backend.getIdentifiedDbms()
                    in (DBMS.FIREBIRD, DBMS.DB2, DBMS.MAXDB, DBMS.DERBY, DBMS.FRONTBASE)
                    and len(partialValue) > INFERENCE_BLANK_BREAK
                    and partialValue[-INFERENCE_BLANK_BREAK:].isspace()
                ):
                ...
                if lastChar > 0 and index >= lastChar:
                    ...
                    finalValue = (
                        finalValue.rstrip() if len(finalValue) > 1 else finalValue
                    )
                    partialValue = None
                    break
        ...
        if finalValue is not None:
            finalValue = (
                decodeDbmsHexValue(finalValue) if conf.hexConvert else finalValue
            )
            ...
            hashDBWrite(
                ...
                % (
                    (
                        ...
                        if not conf.hexConvert
                        else PARTIAL_HEX_VALUE_MARKER
                    ),
                    partialValue,
                ),
            )
