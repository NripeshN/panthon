    ...
    try:
        ...
        def crawlThread():
            ...
            while kb.threadContinue:
                ...
                try:
                    ...
                    errMsg = (
                        "connection exception detected ('%s'). skipping "
                        % getSafeExString(ex)
                    )
                ...
                if isinstance(content, six.text_type):
                    try:
                        ...
                        tags += re.finditer(
                            r'(?i)\s(href|src)=["\'](?P<href>[^>"\']+)', content
                        )
                        tags += re.finditer(
                            r'(?i)window\.open\(["\'](?P<href>[^)"\']+)["\']', content
                        )
                        ...
                        for tag in tags:
                            ...
                            if href:
                                if (
                                    threadData.lastRedirectURL
                                    and threadData.lastRedirectURL[0]
                        ...
                        if conf.forms:
                            threadData.shared.formsFound |= (
                                len(findPageForms(content, current, False, True)) > 0
                            )
        ...
        if kb.targets:
            if kb.normalizeCrawlingChoice is None:
                ...
                kb.normalizeCrawlingChoice = readInput(
