...
try:
    ...
    class CompleterNG(rlcompleter.Completer):
        def global_matches(self, text):
            ...
            Return a list of all names currently defined in
...
def readlineAvailable():
    ...
    By default it is not in Python default installation on Windows
    """
...
def loadHistory(completion=None):
    ...
    if os.path.exists(historyPath):
        try:
            ...
            if IS_WIN:
                warnMsg = (
                    "there was a problem loading the history file '%s'. " % historyPath
                )
                warnMsg += (
