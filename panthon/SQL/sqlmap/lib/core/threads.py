...
def exceptionHandledFunction(threadFunction, silent=False):
    try:
        ...
        if (
            not silent
            and kb.get("threadContinue")
            and not kb.get("multipleCtrlC")
            and not isinstance(ex, (SqlmapUserQuitException, SqlmapSkipTargetException))
        ):
            errMsg = (
                getSafeExString(ex)
                if isinstance(ex, SqlmapBaseException)
                else "%s: %s" % (type(ex).__name__, getSafeExString(ex))
            )
            logger.error(
                "thread %s: '%s'" % (threading.currentThread().getName(), errMsg)
            )

            if conf.get("verbose") > 1 and not isinstance(
                ex, SqlmapConnectionException
            ):
                traceback.print_exc()
...
def runThreads(
    ...
    try:
        if (
            ...
            while True:
                message = (
                    "please enter number of threads? [Enter for %d (current)] "
                    % numThreads
                )
                    ...
                    if isDigit(choice):
                        if int(choice) > MAX_NUMBER_OF_THREADS and not skipThreadCheck:
                            errMsg = (
                                ...
                                " potential connection issues" % MAX_NUMBER_OF_THREADS
                            )
