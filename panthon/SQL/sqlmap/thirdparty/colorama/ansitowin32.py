...
class AnsiToWin32(object):
    ...
    ANSI_OSC_RE = re.compile(
        "\001?\033\\]([^\a]*)(\a)\002?"
    )  # Operating System Command (Note: https://github.com/tartley/colorama/issues/247)
    ...
    def should_wrap(self):
        """True if this class is actually needed.

        If false, then the output stream will not be affected, nor will
        win32 calls be issued, so wrapping stdout is not actually
        required. This will generally be False on non-Windows platforms,
        unless optional functionality like autoreset has been requested
        using kwargs to init()
        """
