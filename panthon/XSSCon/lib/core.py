    ...
    def post_method(self):
        ...
        for form in forms:
            ...
            if form["method"].lower().strip() == "post":
                Log.warning(
                    "Target have form with POST method: "
                    + C
    ...
    def get_method_form(self):
        ...
        for form in forms:
            ...
            if form["method"].lower().strip() == "get":
                Log.warning(
                    "Target have form with GET method: " + C + urljoin(self.url, action)
                )
                Log.info("Collecting form input key.....")
    ...
    def get_method(self):
        ...
        for a in links:
            ...
            if (
                ...
                if query != "":
                    Log.warning(
                        "Found link with query: "
                        + G
                        + query
                        + N
                        + " Maybe a vuln XSS point"
                    )
                    ...
                    query_all = base.replace(
                        query, urlencode({x: self.payload for x in parse_qs(query)})
                    )
                    ...
                    if (
                        self.payload in _respon.text
                        or self.payload in self.session.get(query_all).text
                    ):
                        Log.high("Detected XSS (GET) at " + _respon.url)
                    else:
                        Log.info(
                            "This page is safe from XSS (GET) attack but not 100%"
                            " yet..."
                        )
